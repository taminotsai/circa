...
public class CircabcImporterActionExecuter extends ImporterActionExecuter
{
	...
	@Override
	 public void executeImpl(Action ruleAction, NodeRef actionedUponNodeRef)
	 {
		...
		try
		{
			...
			Boolean disableFileNotification = (Boolean)ruleAction.getParameterValue(PARAM_DISABLE_FILE_NOTIFICATION);
			...
			{
				throw new IllegalStateException("File is too big to be imported maximum size in bytes : " + String.valueOf(maxSizeInBytes) + " current size in bytes " + String.valueOf(fileSize) );
			}
	...
	protected void bulkImport(NodeRef actionedUponNodeRef, NodeRef importDest) throws IOException {
		if (myNodeService.exists(actionedUponNodeRef) == true)
        {
           ...
           ContentReader reader = myContentService.getReader(actionedUponNodeRef, ContentModel.PROP_CONTENT);
           if (reader != null)
           {
	...
	private void setLogRecord(LogRecord logRecord, NodeRef actionedUponNodeRef,
		...
		logRecord.setDocumentID((Long) myNodeService.getProperty(importDest, ContentModel.PROP_NODE_DBID ) );
		...
		logRecord.addInfo(" file : " + (String)myNodeService.getProperty(actionedUponNodeRef, ContentModel.PROP_NAME));
	...
	{
		long size =0L;
		ContentData content = (ContentData) myNodeService .getProperty(nodeRef, ContentModel.PROP_CONTENT);
		if (content != null)
		{
	...
	private void notifyUser(String userToNotify) {
			...
			// Send the message
      		try 
      		{
				mailService.send(from,to,null,subject,body,html);
			} 
	...
	protected void addParameterDefinitions(List<ParameterDefinition> paramList) 
	{
		...
		paramList.add(new ParameterDefinitionImpl(PARAM_DISABLE_FILE_NOTIFICATION, DataTypeDefinition.BOOLEAN, 
	...
	private void importDirectoryContentOverridable(String dir, NodeRef root)
    {
        ...
        for (File file : topdir.listFiles())
        {
            try
            {
            	...
            	NodeRef targetRef = myNodeService.getChildByName(root, ContentModel.ASSOC_CONTAINS, fileName);
                ...
                if (file.isFile())
                {
                    ...
                    if(targetRef == null)
                    {
                    	...
                    	FileInfo fileInfo = myFileFolderService.create(root, fileName, ContentModel.TYPE_CONTENT);
                    ...
                    ContentWriter writer = myContentService.getWriter(fileRef, ContentModel.PROP_CONTENT, true);
                ...
                else
                {
                    ...
                    {
                    	// create content node based on the file name
                    	FileInfo fileInfo = myFileFolderService.create(root, file.getName(), ContentModel.TYPE_FOLDER);
                    	targetRef = fileInfo.getNodeRef();

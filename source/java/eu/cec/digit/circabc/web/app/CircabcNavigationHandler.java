...
public class CircabcNavigationHandler extends AlfrescoNavigationHandler {
    ...
    @Override
    public void handleNavigation(final FacesContext context, final String fromAction, String outcome)
    {
        if(outcome != null)
        {
            if(isWai(outcome))
    ...
    protected String getViewIdFromStackObject(FacesContext context, Object topOfStack)
    {
        ...
        if (viewId.equals("/jsp/close.jsp") || (topOfStack instanceof String && ((String)topOfStack).equals("/jsp/close.jsp")))
        {
           return WAI_NAVIGATION_CONTAINER_PAGE;
        }
    ...
    protected void handleDialogOrWizardClose(FacesContext context, String fromAction, String outcome, boolean dialog)
    {
        ...
        if (getViewStack(context).empty() == true && (DialogManager.CANCEL_FROM_ACTION.equals(fromAction) || WizardManager.CANCEL_FROM_ACTION.equals(fromAction)))
        {
...
//            	 there isn't an overidden outcome so go back to the previous view
    ...
    protected boolean isWizardStep(String fromAction)
    {
       ...
       if (fromAction != null &&
           (fromAction.equals("#{WaiWizardManager.next}") || fromAction.equals("#{WaiWizardManager.back}")))
       {
          wizardStep = true;
       }
       else

...
import eu.cec.digit.circabc.service.profile.permissions.JoinPermissions;
...
public abstract class PermissionUtils extends AbstractSearchUtils
{
    ...
    private static final String CIRCABC_CACHE_NAME = "eu.cec.digit.circabc.service.dynamic.authority.circabcDynamicAuthority";
    //private static final String CAT_CACHE_NAME = "eu.cec.digit.circabc.service.dynamic.authority.categoryDynamicAuthority";
    //private static final String INF_CACHE_NAME = "eu.cec.digit.circabc.service.dynamic.authority.informationDynamicAuthority";
    //private static final String LIB_CACHE_NAME = "eu.cec.digit.circabc.service.dynamic.authority.libraryDynamicAuthority";
    //private static final String DIR_CACHE_NAME = "eu.cec.digit.circabc.service.dynamic.authority.directoryDynamicAuthority";
    //private static final String NEW_CACHE_NAME = "eu.cec.digit.circabc.service.dynamic.authority.newsgroupDynamicAuthority";
    //private static final String EVE_CACHE_NAME = "eu.cec.digit.circabc.service.dynamic.authority.eventDynamicAuthority";


    private static final List<String> PROFILES_LIST_TO_FILTER = new ArrayList<String>(3);
    static{
    	// fill the list of profiles to filter
		PROFILES_LIST_TO_FILTER.add(getProfileManagerServiceFactory().getCircabcRootProfileManagerService().getAllCircaUsersGroupName());
    ...
    public static List<SelectItem> getPermissions(final Node actionNode, final Log logger)
    {
    	if(actionNode.hasAspect(CircabcModel.ASPECT_LIBRARY))
    	{
    		...
    		final List<SelectItem> itemsAsList = new ArrayList<SelectItem>(libPermissions.size());
        	...
        	for(final String libPerm : libPermissions)
        	{
        		...
        		{
        			itemsAsList.add(
        					new SelectItem(libPerm.toString(), getPermissionLabel(libPerm.toString()))
        				);
        		}
        	}
        	...
        	{
        		logger.debug(itemsAsList.size() + " Permission(s) found for the node " + actionNode.getName() + " having aspect " + CircabcModel.ASPECT_LIBRARY + " . Permissions: " + itemsAsList);
        	}
    	...
    	else if(actionNode.hasAspect(CircabcModel.ASPECT_NEWSGROUP))
    	{
    		...
    		final List<SelectItem> itemsAsList = new ArrayList<SelectItem>(permissions.size());
        	...
        	if(logger.isDebugEnabled())
        	{
        		logger.debug(itemsAsList.size() + " Permission(s) found for the node " + actionNode.getName() + " having aspect " + CircabcModel.ASPECT_NEWSGROUP + " . Permissions: " + itemsAsList);
        	}
    	...
    	else if(actionNode.hasAspect(CircabcModel.ASPECT_SURVEY))
    	{
        	...
        	if(logger.isDebugEnabled())
        	{
        		logger.debug(itemsAsList.size() + " Permission(s) found for the node " + actionNode.getName() + " having aspect " + CircabcModel.ASPECT_SURVEY + " . Permissions: " + itemsAsList);
        	}
    	...
    	else if(actionNode.hasAspect(CircabcModel.ASPECT_INFORMATION))
    	{
        	...
        	if(logger.isDebugEnabled())
        	{
        		logger.debug(itemsAsList.size() + " Permission(s) found for the node " + actionNode.getName() + " having aspect " + CircabcModel.ASPECT_INFORMATION + " . Permissions: " + itemsAsList);
        	}
    	...
    	else if(actionNode.hasAspect(CircabcModel.ASPECT_EVENT))
    	{
        	...
        	if(logger.isDebugEnabled())
        	{
        		logger.debug(itemsAsList.size() + " Permission(s) found for the node " + actionNode.getName() + " having aspect " + CircabcModel.ASPECT_EVENT + " . Permissions: " + itemsAsList);
        	}
    	...
    	{
    		logger.error("The list of permissions can't be returned beacause no logic found for the the node " + actionNode + ". The node aspects are not taken in account yet.");

    		return Collections.<SelectItem>emptyList();
    	}
    }

    public static void resetCache(final Node actionNode, final CacheManager dynamicAuthorithiesCacheManager, final Log logger)
    {
    	...
    	{
    		//cacheName = INF_CACHE_NAME;
    		cacheName = CIRCABC_CACHE_NAME;
    	}
    	else if(actionNode.hasAspect(CircabcModel.ASPECT_EVENT))
    	{
    		//cacheName = EVE_CACHE_NAME;
    		cacheName = CIRCABC_CACHE_NAME;
    	}
    	else if(actionNode.hasAspect(CircabcModel.ASPECT_IGROOT) || actionNode.getType().equals(CircabcModel.TYPE_DIRECTORY_SERVICE))
    	{
    		//cacheName = DIR_CACHE_NAME;
    		cacheName = CIRCABC_CACHE_NAME;
    	}
    	else if(actionNode.hasAspect(CircabcModel.ASPECT_CATEGORY))
    	{
    		//cacheName = CAT_CACHE_NAME;
    		cacheName = CIRCABC_CACHE_NAME;
    	}
    	else
    	{
    ...
    public static List<Map> getInterestGroupAuthorities(final NodeRef anyNode, final List<String> withPermission)
    {
	    final FacesContext context = FacesContext.getCurrentInstance();

	    final NodeRef igRoot = getManagementService().getCurrentInterestGroup(anyNode);
		final ProfileManagerService igProfileManagerService = getProfileManagerServiceFactory().getIGRootProfileManagerService();
		final ProfileManagerService catProfileManagerService = getProfileManagerServiceFactory().getCategoryProfileManagerService();
		final ProfileManagerService rootProfileManagerService = getProfileManagerServiceFactory().getCircabcRootProfileManagerService();
		...
		final RetryingTransactionCallback<List<Map>> callback = new RetryingTransactionCallback<List<Map>>()
		{
			public List<Map> execute() throws Throwable
			{
				...
				try
				{
					...
					final Map<String, List<String>> permissionMap = new HashMap<String, List<String>>(8, 1.0f);
					final Map<String, Boolean> inheritanceMap = new HashMap<String, Boolean>();
					final Set<AccessPermission> permissions = getPermissionService().getAllSetPermissions(anyNode);
					...
					for (final Map.Entry<String, List<String>> entry : permissionMap.entrySet())
					{
								...
								|| getPersonService().personExists(entry.getKey()))
						{
							...
							if (nodeRef != null)
							{
								...
								node.put(KEY_USER_FULL_NAME, ((String) props.get("firstName")) + ' ' + ((String) props.get("lastName")));
						...
						else
						{
								...
								final Map<String, Object> node = new HashMap<String, Object>(5, 1.0f);
								boolean ignoreRow = false ;
								if (entry.getKey().startsWith(PermissionService.GROUP_PREFIX) == true)
								{
									Profile profileFromGroup = igProfileManagerService.getProfileFromGroup(igRoot, entry.getKey());

									if(profileFromGroup == null)
									{
										// perhaps the Cat Admin
										profileFromGroup = catProfileManagerService.getProfileFromGroup(getManagementService().getCurrentCategory(igRoot), entry.getKey());
									}

									if(profileFromGroup == null)
									{
										// perhaps the CircabcAdmin Admin
										profileFromGroup = rootProfileManagerService.getProfileFromGroup(getManagementService().getCircabcNodeRef(), entry.getKey());
									}

									ignoreRow = (profileFromGroup == null);
									node.put(KEY_DISPLAY_NAME, ignoreRow ? "" : profileFromGroup.getProfileDisplayName());
    ...
    public static SelectItem[] getDomainFilters(final boolean withFilter, final boolean accessProfile, final boolean applicant)
    {
    	final List<SelectItem> filters = new ArrayList<SelectItem>(11);

    	final String allUsersMessage = Application.getMessage(FacesContext.getCurrentInstance(), MSG_ALL_USER);

    	if(!withFilter)
    	{
    		filters.add(new SelectItem(PermissionUtils.ALLUSERS , allUsersMessage));

    		for (final Map.Entry<String, String> item : getUserService().getEcasUserDomains().entrySet())
			{
	 ...
	 */
	public static boolean isUserCanJoinNode(final String currentUser, final NodeRef nodeRef, final NodeService nodeService, final PermissionService permissionService)
	{
		final FacesContext fc = FacesContext.getCurrentInstance();
		final CircabcServiceRegistry services = Services.getCircabcServiceRegistry(fc);
		final ProfileManagerService profileManagerService = services.getProfileManagerServiceFactory().getProfileManagerService(nodeRef);
		...
		{
			final AccessStatus accesStatus = permissionService.hasPermission(nodeRef, JoinPermissions.JOIN.toString());
			...
			if(canJoin)
			{
				...
				final ProfileManagerServiceFactory profileManagerServiceFactory = services.getProfileManagerServiceFactory();
				final ProfileManagerService categoryProfileManagerService = profileManagerServiceFactory.getProfileManagerService(categoryNodeRef);
				canJoin = !categoryProfileManagerService.getInvitedUsers(categoryNodeRef).contains(currentUser);
			}
		}
		return canJoin;
	}
	 ...
	 * @return
	 */
	public static final List<SortableSelectItem> buildInvitableUserItems(final String domain, final String match, final Log logger)
	{
	 ...
	 * @return
	 */
	public static final List<SortableSelectItem> buildInvitableUserItems(final String domain, final String match, final List<String> alreadyInvitedAuthority, final Log logger)
	{
		return buildInvitedUserItems(getAllAvailableUsers(domain, match, logger), MAX_ELEMENTS_IN_LIST, null, alreadyInvitedAuthority, logger);
	}
	 ...
	 * @return
	 */
	public static final List<SortableSelectItem> buildInvitedUserItems(final Node node, final Log logger, final boolean onlineUsers)
	{
	 ...
	 * @return
	 */
	public static final List<SortableSelectItem> buildCircabcUserItems(final Node node, final String match, final boolean onlineUsers, final List<String> alreadyInvitedAuthority, final Log logger)
	{
		return buildInvitedUserItems(getCircabcUsers(node, logger, onlineUsers, true), MAX_ELEMENTS_IN_LIST, match, alreadyInvitedAuthority, logger);
	}
	 ...
	 * @return
	 */
	public static final List<SortableSelectItem> buildCategoryUserItems(final Node node, final String match, final boolean onlineUsers, final List<String> alreadyInvitedAuthority, final Log logger)
	{
		return buildInvitedUserItems(getCategoryUsers(node, logger, onlineUsers, true), MAX_ELEMENTS_IN_LIST, match, alreadyInvitedAuthority, logger);
	}
	 ...
	 * @return
	 */
	public static final List<SortableSelectItem> buildInvitedUserItems(final Node node, final String match, final boolean onlineUsers, final List<String> alreadyInvitedAuthority, final Log logger)
	{
		return buildInvitedUserItems(getInvitedUsers(node, logger, onlineUsers, false), MAX_ELEMENTS_IN_LIST, match, alreadyInvitedAuthority, logger);
	}

	/**
	 * Build a Jsf select item of all users invited user in the given node and its children according given filters.
	 *
	 * @param node
	 * @param match							filter of the profile name
	 * @param alreadyInvitedAuthority		filter of some profile authority to remove
	 ...
	 * @return
	 */
	public static final List<SortableSelectItem> buildInvitedUserItemsRecursivly(final Node node, final String match, final boolean onlineUsers, final List<String> alreadyInvitedAuthority, final Log logger)
	{
		return buildInvitedUserItems(getInvitedUsers(node, logger, onlineUsers, true), MAX_ELEMENTS_IN_LIST, match, alreadyInvitedAuthority, logger);
	}
	
	public static final String getPermissionLabel(final String permission)
	{
		return Application.getMessage(FacesContext.getCurrentInstance(), VALUE_PERM_PREFIX + permission.toLowerCase());
	}

	public static final String getPermissionTooltip(final String permission)
	{
		return Application.getMessage(FacesContext.getCurrentInstance(), TOOLTIP_PERM_PREFIX + permission.toLowerCase());
	}

    //-- Private helpers
	
	private static List<SortableSelectItem> buildInvitedUserItems(final Collection<SearchResultRecord> records, final int maxUserToReturn, final String match, final List<String> alreadyInvitedAuthority, final Log logger)
	{
		long startTime = 0;
    	...
    	final Map<String, SortableSelectItem> items = new HashMap<String, SortableSelectItem>(records.size());
		...
		for(final SearchResultRecord record: records)
		{
			...
			if(needNameFilter == true && userText.toLowerCase().contains(match.toLowerCase()) == false)
	    	{
				// the search text is not contained in the users name or email, remove it from the list
	    		if(logger.isDebugEnabled())
	    		{
	    			logger.debug("  --  Profile " + userId + " removed from the list because it doesn't matches filter. Search performed on text: " + userText);
	    		}
	    	}
		...
		if(logger.isDebugEnabled())
		{
			logger.debug(items.size() + " Users added in " + (System.currentTimeMillis() - startTime) + "ms");
		}


		final List<SortableSelectItem> listItems = new ArrayList<SortableSelectItem>(items.size());
		listItems.addAll(items.values());
		return listItems;
	}

	private static Collection<SearchResultRecord> getAllAvailableUsers(final String domain, final String criteria, final Log logger)
	{
		long startTime = 0;
    	...
    	if(criteria.indexOf(BLANK_DELIM) == -1) {
    		if(criteria.indexOf('@') == -1) {
    			users = getUserService().getUsersByDomainFirstNameLastNameEmail(domain, criteria);
    		} else {
    			users = getUserService().getUsersByMailDomain(criteria, domain);
    		}
    	} else {
    		users = getUserService().getUsersByDomainFirstNameLastNameEmail(domain, criteria);
    	}
    	...
    	if(logger.isDebugEnabled())
		{
			logger.debug(users.size() + " Users found in " + (System.currentTimeMillis() - startTime) + "ms");
		}

    	return users;
	}
	
	private static Collection<SearchResultRecord> getCircabcUsers(final Node where, final Log logger, final boolean onlineUsers, final boolean recursive)
	{
		long startTime = 0;
		...
		final ProfileManagerService profileManager = getProfileManagerServiceFactory().getCircabcRootProfileManagerService();
		...
		if(logger.isDebugEnabled())
		{
			logger.debug(userRecords.size() + " Users found in " + (System.currentTimeMillis() - startTime) + "ms");
		}
		
		return userRecords;
	}
	
	private static Collection<SearchResultRecord> getCategoryUsers(final Node where, final Log logger, final boolean onlineUsers, final boolean recursive)
	{
		long startTime = 0;
		...
		final ProfileManagerService profileManager = getProfileManagerServiceFactory().getCategoryProfileManagerService();
	...
	private static Collection<SearchResultRecord> getInvitedUsers(final Node where, final Log logger, final boolean onlineUsers, final boolean recursive)
	{
		long startTime = 0;
		...
		if(recursive)
		{
			...
			else if(NavigableNodeType.CATEGORY.equals(type))
			{
				...
				final ProfileManagerService profileManagerService = getProfileManagerServiceFactory().getProfileManagerService(circabcNodeRef);
				final Set<String> persons = profileManagerService.getPersonInProfile(circabcNodeRef, CircabcRootProfileManagerService.Profiles.CIRCABC_ADMIN);
			...
			else if(NavigableNodeType.IG_ROOT.equals(type))
			{
				...
				final ProfileManagerService profileManagerService = getProfileManagerServiceFactory().getProfileManagerService(categoryNodeRef);
				final Set<String> persons = profileManagerService.getPersonInProfile(categoryNodeRef, CategoryProfileManagerService.Profiles.CIRCA_CATEGORY_ADMIN);
				userRecords.addAll(buildUsersList(persons, onlineUsers));
			}
		}
	...
	private static List<SearchResultRecord> buildUsersList(final Set<String> users, final boolean onlineUsers) {
		final List<SearchResultRecord> userRecords = new ArrayList<SearchResultRecord>(users.size());

		NodeRef person;
		Map<QName, Serializable> properties;
		
		Set<String> allOnlineUsers = new HashSet<String>();
		if (onlineUsers == true)
		{
    ...
    private static String permissionsToString(final List<String> permissions)
	{
		...
		if (permissions != null)
	    {
	         ...
	         {
	        	 final String perm = permissions.get(i);
	...
	public static String computeUserLogin(final String username, final PersonService personService, final NodeService nodeService)
	{
	...
	public static String computeUserLogin(final Map<? extends Object, ? extends Object> properties)
	{

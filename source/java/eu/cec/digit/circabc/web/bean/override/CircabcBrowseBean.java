...
/**
 * Extension of the Alfresco Browse bean that manage the specific Circabc
 * navigation features and the wai browse.
 * 
...
public class CircabcBrowseBean extends BrowseBean
{
	...
	public static final String PREFIXED_WAI_BROWSE_OUTCOME = CircabcNavigationHandler.WAI_PREFIX + WAI_BROWSE_OUTCOME;
	public static final  String CLOSE_NAVIGATION_OUTCOME = WAI_BROWSE_OUTCOME + CircabcNavigationHandler.OUTCOME_SEPARATOR + "close";
	public static final  String PREFIXED_WAI_CLOSE_NAVIGATION_OUTCOME = CircabcNavigationHandler.WAI_PREFIX
	...
	/**
	 * Action called when a folder space is clicked in a WAI page. Have an
	 * accessible navigation into the space.
	 */
            ...
            public Object execute() throws Exception
            {
          	String id = null;
        		...
        		} else
        		{
        			...
        			params = new HashMap<String, String>(childs.size());

        			for (final Object child : childs)
        			{
        				if (child instanceof UIParameter)
        				{
        					params.put(((UIParameter) child).getName(), ((UIParameter) child).getValue().toString());
        				}
        			}
		...
		if (id != null && id.length() != 0)
		{
			...
			} catch (final InvalidNodeRefException refErr)
			{
				Utils.addErrorMessage(MessageFormat.format(
						Application.getMessage(FacesContext.getCurrentInstance(), Repository.ERROR_NODEREF), new Object[] { id }));
			}
		}
	}
	...
	public void clickWai(final NodeRef ref)
	{
		...
		boolean canRead =  getPermissionService().hasPermission(ref, PermissionService.READ)  == AccessStatus.ALLOWED;
		boolean isLibNoAccess =  getPermissionService().hasPermission(ref, LibraryPermissions.LIBNOACCESS.toString())  == AccessStatus.ALLOWED;
	...
	private void clickWaiSecure(final NodeRef ref)
	{
		...
		if (!refToSend.equals(ref))
		{
			...
			if (getPermissionService().hasPermission(ref, PermissionService.READ) != AccessStatus.ALLOWED)
			{
				if (logger.isWarnEnabled())
				{
			...
			Utils.addErrorMessage(MessageFormat.format(Application.getMessage(fc, MSG_REDIRECTED_TO), new Object[] { nodeName }));
		}
		...
		fc.getApplication().getNavigationHandler().handleNavigation(fc, refToSend.getId(), PREFIXED_WAI_BROWSE_OUTCOME);
		...
		if (typeDef != null)
		{
			...
			if ((ContentModel.TYPE_FOLDER.equals(modelType) || getDictionaryService().isSubClass(modelType,
					ContentModel.TYPE_FOLDER))
					&& (ApplicationModel.TYPE_FOLDERLINK.equals(modelType) || getDictionaryService().isSubClass(modelType,
	...
	/**
	 * Refresh the actual state of the navigation bean with the parent of the
	 * curent node.
	 */
	public void clickParent()
	{
		...
		if (type != null)
		{
			...
			case EVENT_APPOINTMENT:
				node = (Node) getCircabcNavigator().getCurrentIGRoot().get(InterestGroupNode.EVENT_SERVICE);
				if (logger.isDebugEnabled())
				{
				...
				break;
			default:
				final NodeRef parentRef = getNodeService().getPrimaryParent(getCircabcNavigator().getCurrentNode().getNodeRef())
						.getParentRef();
		...
		if (id != null && id.length() != 0)
		{
			try
			{
				...
				if (getNodeService().hasAspect(ref, ForumModel.ASPECT_DISCUSSABLE) == false)
				{
					...
					Utils.addErrorMessage("Discuss called for an object that does not have a discussion! " + id);
				} else
				{
					// as the node has the discussable aspect there must be a
					// discussions child assoc
					final List<ChildAssociationRef> children = getNodeService().getChildAssocs(ref, ForumModel.ASSOC_DISCUSSION,
					...
					} else
					{
						...
						if (logger.isWarnEnabled())
						{
							logger.warn("Node has the discussable aspect but does not have 1 child, it has " + children.size()
									+ " children!");
						}

						throw new InvalidNodeRefException("Node has the discussable aspect but does not have 1 child, it has "
	...
	/**
	 * @param perform
	 *            the navigation via the ml content details of the given node
	 */
	...
	/**
	 * Click to browse the current space into the native interface. The browsing
	 * is secured, only a subclass of Folder can be navigable inside the native
	 * interface.
	 */
	public void clickNativeSpace(final ActionEvent event)
	{
		...
		{
			if (!(ContentModel.TYPE_FOLDER.equals(type) || getDictionaryService().isSubClass(type, ContentModel.TYPE_FOLDER)))
			{
	...
	/**
	 * Action called from the Simple Search component. Sets up the SearchContext
	 * object with the values from the simple search menu.
	 */
	public void searchWai(final ActionEvent event)
	{
		...
		if (search.getSearchContext() != null)
		{
			...
			fc.getApplication().getNavigationHandler().handleNavigation(fc, null, SearchResultDialog.WAI_DIALOG_CALL);
		} else
		{
			fc.getApplication().getNavigationHandler().handleNavigation(fc, null, AdvancedSearchDialog.WAI_DIALOG_CALL);
		}

		final ViewHandler viewHandler = fc.getApplication().getViewHandler();
		final UIViewRoot viewRoot = viewHandler.createView(fc, CircabcNavigationHandler.WAI_DIALOG_CONTAINER_PAGE);
	...
	/**
	 * This method is called when the navigation parameters are ready to be
	 * activated.
	 */
	public void applyWaiBrowsing(final String nodeId, final boolean informListeners)
	{
		...
		if (nodeId != null)
		{
			...
			getCircabcNavigator().setupDispatchContext(new Node(new NodeRef(Repository.getStoreRef(), nodeId)));
	...
	public int getListElementNumber()
	{
		...
		{
			final String numberStr = CircabcConfiguration.getProperty(CircabcConfiguration.DEFAULT_LIST_ELEMENT_NUMBER_PROPERTIES);
			...
			if (numberStr != null && numberStr.length() > 1)
			{
				...
				catch (final NumberFormatException ex)
				{
					logger.error("The property file " + CircabcConfiguration.DEFAULT_PROPERTY_FILE
							+ " is corrupted. The value of the key " + CircabcConfiguration.DEFAULT_LIST_ELEMENT_NUMBER_PROPERTIES
			...
			{
				logger.error("The property file " + CircabcConfiguration.DEFAULT_PROPERTY_FILE
						+ " not complete. The value of the key " + CircabcConfiguration.DEFAULT_LIST_ELEMENT_NUMBER_PROPERTIES
						+ " must be set and must be a valid integer and not "
						+ "\nPlease correct the problem and restart the server. For instance the value used by default is "
						+ DEFAULT_LIST_ELEMENT_NUMBER);
			}

			cachedListElementNumber = Integer.valueOf(response);
		}

		return cachedListElementNumber.intValue();
	}
	...
	/**
	 * Resolver to get the trunckated string of the name with 100 characters and
	 * "..." if too long (&gt; 100)
	 */
	...
	/**
	 * Resolver to get the trunckated string of the name with 400 characters and
	 * "..." if too long (&gt; 400)
	 */
	...
	/**
	 * Resolver to get the trunckated string of the parent's name with 50
	 * characters and "..." if too long
	 */
	public NodePropertyResolver resolverFromTopic = new NodePropertyResolver()
	{
		...
		public Object get(final Node node)
		{
			...
			final String copedName = UtilsCircabc.cropEncodeLight((title == null || title.length() < 0) ? name : title, 50);
			final FacesContext fc = FacesContext.getCurrentInstance();

			final StringBuffer buf = new StringBuffer("");

			buf.append(MessageFormat.format(Application.getMessage(fc, MSG_FROM_TOPIC), new Object[] { copedName }))
					.append(OPEN_BLOCKQUOTE).append(resolverContentFourHundred.get(node)).append(CLOSE_BLOCKQUOTE);
	...
	public NodePropertyResolver resolverReplies = new NodePropertyResolver()
	{
		...
		public Object get(final Node node)
		{
			...
			final List<NodeRef> replies = serviceRegistry.getSearchService().selectNodes(node.getNodeRef(), repliesXPath,
			...
			return Integer.valueOf(noReplies);
		}
	};
	...
	public NodePropertyResolver resolverCreatorUserName = new NodePropertyResolver()
	{
		...
		public Object get(final Node node)
		{
			...
			final String creator = (String) node.getProperties().get(ContentModel.PROP_CREATOR.toString());
	...
	public NodePropertyResolver resolverLastPost = new NodePropertyResolver()
	{
		...
		public Object get(final Node node)
		{
			...
			{
				final FacesContext fc = FacesContext.getCurrentInstance();
				final String formatedDate = WebClientHelper.formatLocalizedDate(lastDate, fc, true, true);
		...
		public Object get(final Node node)
		{
			...
			if (reports.size() > 0)
			{
				...
				for (final AbuseReport report : reports)
				{
					...
					final String safeMessage = (message == null || message.trim().length() < 1) ? NO_MESSAGE : message.replace(
							"\n", " ");
					final String i18nMessage = MessageFormat.format(Application.getMessage(fc, MSG_ABUSE_REPORT), reporter, date,
							safeMessage);
	...
	public NodePropertyResolver resolverRejectMessages = new NodePropertyResolver()
	{
		...
		public Object get(final Node node)
		{
			...
			{
				final FacesContext fc = FacesContext.getCurrentInstance();
				final String moderator = (String) node.getProperties().get(ModerationModel.PROP_REJECT_BY.toString());
				final Date date = (Date) node.getProperties().get(ModerationModel.PROP_REJECT_ON.toString());
				final String message = (String) node.getProperties().get(ModerationModel.PROP_REJECT_MESSAGE.toString());
				final String safeMessage = (message == null || message.trim().length() < 1) ? NO_MESSAGE : message.replace("\n",
						" ");
				final String i18nMessage = MessageFormat.format(Application.getMessage(fc, MSG_REJECT_DETAILS), moderator, date,
						safeMessage);
	...
	public NodePropertyResolver resolverHasAttachement = new NodePropertyResolver()
	{
		...
		public Object get(final Node node)
		{
			final boolean hasAttachement = getAttachementService().getAttachements(node.getNodeRef()).size() > 0;
			return Boolean.valueOf(hasAttachement);
		}
	};
	...
	public NodePropertyResolver resolverCreatorProfile = new NodePropertyResolver()
	{
		...
		public Object get(final Node node)
		{
			...
			if (creator != null && personService.personExists(creator))
			{
				...
				{
					final CircabcServiceRegistry circabcServiceRegistry = Services.getCircabcServiceRegistry(context);
					...
					String profileName = getProfileName(circabcServiceRegistry.getIGRootProfileManagerService(), igRoot, creator);
					...
					if (profileName == null)
					{
						profileName = getProfileName(circabcServiceRegistry.getCircabcRootProfileManagerService(),
		...
		private String getProfileName(final ProfileManagerService profileService, final NodeRef nodeRef, final String creator)
		{
	...
	public NodePropertyResolver resolverCreatorAvatar = new NodePropertyResolver()
	{
		...
		public Object get(final Node node)
		{
			final PersonService personService = getServiceRegistry().getAlfrescoServiceRegistry().getPersonService();
			final UserDetailsBusinessSrv userDetailsService = getBusinessRegistry().getUserDetailsBusinessSrv();
	...
	public NodePropertyResolver resolverSpaceSize = new NodePropertyResolver()
	{
		...
		public Object get(final Node node)
		{
			...
			final List<FileInfo> childs = serviceRegistry.getFileFolderService().list(ref);

			final int size = childs == null ? 0 : childs.size();
	...
	public NodePropertyResolver resolverDocumentNumber = new NodePropertyResolver()
	{
		...
		public Object get(final Node node)
		{
			...
			final ServiceRegistry serviceRegistry = Services.getAlfrescoServiceRegistry(context);
			final NodeRef ref = node.getNodeRef();
			final List<FileInfo> childs = serviceRegistry.getFileFolderService().listFiles(ref);
	...
	private NodeRef getSecureNodeRef(final NodeRef fromNodeRef)
	{
		if (getNodeService().exists(fromNodeRef)
				&& getPermissionService().hasPermission(fromNodeRef, PermissionService.READ) == AccessStatus.ALLOWED)
		{
			return fromNodeRef;
		} else
		{
			if (logger.isWarnEnabled())
			{
				...
				} else if (getPermissionService().hasPermission(fromNodeRef, PermissionService.READ) != AccessStatus.ALLOWED)
				{
	...
	private boolean isNodeSecure(final NodeRef nodeRef)
	{
		...
		{
			if (getPermissionService().hasPermission(nodeRef, PermissionService.READ) == AccessStatus.ALLOWED)
			{
	...
	/**
	 * @param businessRegistry
	 *            the businessRegistry to set
	 */
	...
	public String getBannerRenderPropertyName(){
		...
		if(categoryNode !=null && categoryNode.getProperties().containsKey(CircabcModel.PROP_NAVIGATION_LIST_RENDER_TYPE.toString()))
		{
			/*
			 * if = shortname or title to be used for banner
			 */
				renderPropertyName = categoryNode.getProperties().get(CircabcModel.PROP_NAVIGATION_LIST_RENDER_TYPE.toString()).toString();

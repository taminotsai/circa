...
public class ManageNodeDetailsDialog extends BaseWaiDialog
{
	...
	public void init(Map<String, String> parameters)
	{
		...
		if(parameters != null)
		{
			...
			else if(ManagedNodes.CONTENT.equals(actionNodeType) || ManagedNodes.TRANSLATION.equals(actionNodeType) ||
					ManagedNodes.EMPTY_TRANSLATION.equals(actionNodeType) || ManagedNodes.ML_CONTAINER.equals(actionNodeType))
			{
				throw new IllegalArgumentException("This kind of node is not managed by this dialog but the wai navigation framework. Node Type : " + actionNodeType);
			}
	...
	{
		return new ActionsListWrapper(getActionNode(), actionNodeType.toString() + "_details_actions_wai");
	}
	...
	{
		return !ManagedNodes.DOSSIER.equals(actionNodeType) && !ManagedNodes.LINK.equals(actionNodeType);
	}
    ...
    public void applyVersionable(final ActionEvent event)
	{
		...
		try
		{
			...
			RetryingTransactionCallback<Object> callback = new RetryingTransactionCallback<Object>()
			{
				...
				{
					getNodeService().addAspect(ref, ContentModel.ASPECT_VERSIONABLE, Collections.<QName, Serializable>singletonMap(ContentModel.PROP_AUTO_VERSION, Boolean.TRUE));
		...
		{
			logRecord.setInfo("took ownership of  " + getNodeService().getProperty(ref, ContentModel.PROP_NAME));
    ...
    */
	...
	{
		return translate("manage_" + actionNodeType.toString() + "_details_dialog_node_icon_tooltip", getActionNode().getName());
	}
   ...
   {
	   return translate("manage_" + actionNodeType.toString() + "_details_dialog_title", getActionNode().getName());
   }

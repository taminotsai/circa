...
public class InformationBean extends LibraryBean
{
	...
	public boolean isRenderNewWindow()
	{
		...
		return isIndexFileFound() && adapt != null && !((Boolean) adapt).booleanValue();
	}
	...
	public String getWebdavUrl(boolean relative)
	{
		...
		{
			String indexPage =  (String) currentNode.getProperties().get(CircabcModel.PROP_INF_INDEX_PAGE.toString());
			if(indexFileFound(currentNode.getNodeRef(), indexPage))
			{
				// redirect to the index page
				return  url + (indexPage.startsWith(FILE_SEPARATOR) == false ? FILE_SEPARATOR : "") + indexPage;
			}
			else
			{
				return url;
	...
	{
		final Node currentNode = getCurrentNode();
		final String indexPage = (String) currentNode.getProperties().get(CircabcModel.PROP_INF_INDEX_PAGE.toString());
	...
	public NavigationPreference getContainerNavigationPreference()
	{
		return getNavigationPreferencesService().getServicePreference(
				getCurrentNode().getNodeRef(), NavigationPreferencesService.INFORMATION_SERVICE, NavigationPreferencesService.CONTAINER_TYPE);
	}

	public NavigationPreference getContentNavigationPreference()
	{
		return getNavigationPreferencesService().getServicePreference(
				getCurrentNode().getNodeRef(), NavigationPreferencesService.INFORMATION_SERVICE, NavigationPreferencesService.CONTENT_TYPE);
	}
	
	public String getRenderPropertyNameNavigationPreference()
	{
		return getNavigationPreferencesService().getServicePreference(
				getCurrentNode().getNodeRef(), NavigationPreferencesService.INFORMATION_SERVICE, NavigationPreferencesService.CONTAINER_TYPE).getRenderPropertyName();
	}
	...
	public List<ActionWrapper> getActions()
	{
				...
				|| type.equals(NavigableNodeType.INFORMATION_SPACE))
		{
			...
			actions.add(
					new ActionWrapper(
							CREATE_CHILDREN,	
							translate(BULK_UPLOAD_ACTION),	
							null,	
							BULK_UPLOAD_BEAN_START,	
							translate(ADD_CONTENT_TOOLTIP),	
							ID,	
							(Serializable) getNavigator().getCurrentNodeId()	
							)	

...
public class MLContentDetailsBean extends ContentDetailsBean implements WaiNavigator
{
	...
	{
		// Initialisation
		editionNodes = new ArrayList<MapNode>();
		...
		Collections.<Version> sort(orderedEditionList, new VersionLabelComparator());
	...
	public void takeOwnershipMLContainer(final ActionEvent event)
	{
		...
		{
			mlContainer = getMultilingualContentService().getTranslationContainer(getCurrentNode().getNodeRef());
		}
		else
		{
		...
		final Collection<NodeRef> translations = getMultilingualContentService().getTranslations(mlContainer).values();
		...
		try
		{
			...
			RetryingTransactionCallback<Object> callback = new RetryingTransactionCallback<Object>()
			{
				...
				public Object execute() throws Throwable
				{
					...
					Utils.addStatusMessage(FacesMessage.SEVERITY_INFO, Application.getBundle(fc).getString(MSG_SUCCESS_OWNERSHIP_ML_CONTAINER));
	...
	protected EditionService getEditionService()
	{
		...
		{
			this.editionService = Services.getAlfrescoServiceRegistry(FacesContext.getCurrentInstance()).getEditionService();
		}

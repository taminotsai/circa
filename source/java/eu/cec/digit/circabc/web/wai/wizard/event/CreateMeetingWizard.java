...
public class CreateMeetingWizard extends BaseWaiWizard {
    ...
    protected String finishImpl(FacesContext context, String outcome) throws Exception
    {
    	...
    	{
    		NodeRef nodeRef = getEventService().createMeeting(getActionNode().getNodeRef(), (Meeting) getAppointment());
    ...
    public SelectItem[] getFilters()
    {
        if  (filters == null)
        {
            filters = PermissionUtils.getDomainFilters(true, true, false);
        }
        return filters;
    }
    ...
    public SelectItem[] pickerCallback(final int filterIndex, final String contains)
    {
        ...
        final RetryingTransactionCallback<List<SortableSelectItem>> callback = new RetryingTransactionCallback<List<SortableSelectItem>>()
        {
			...
			public List<SortableSelectItem> execute() throws Throwable
            {
                ...
                final String filterDescription = getFilters()[filterIndex].getDescription() ;

                // build the set of users or groups who are already invited
                final List<String> alreadyInvitedSet = new ArrayList<String>(internalUsers.size());
                for(InternalUser internalUser : internalUsers)
                {
                    alreadyInvitedSet.add(internalUser.getAuthority());
                }

                // Profiles
                if (PermissionUtils.FILTER_VALUE_PROFILES.equals(filterDescription))
                {
                    items = ProfileUtils.buildMailableProfileItems(getActionNode(), contains, alreadyInvitedSet, logger);
                }
                else
                {
                    if(contains == null || contains.trim().length() < MIN_CHAR_ALLOWED_FOR_QUERY)
                    {
                        Utils.addErrorMessage(translate(AT_LEAST_TREE_CHAR, MIN_CHAR_ALLOWED_FOR_QUERY));

                    }
                    else
                    {
                    	items = PermissionUtils.buildInvitableUserItems(filterDescription, contains, alreadyInvitedSet, logger);
                    }
                }
        ...
        final List<SortableSelectItem> result = txnHelper.doInTransaction(callback, false, true);
    ...
    public void addSelection(final ActionEvent event)
    {
        ...
        else
        {
            ...
            final ProfileManagerService profileManagerService = getProfileManagerServiceFactory().getProfileManagerService(interestGroup);
    ...
    protected void resetFileds()
    {
        ...
        this.internalUsers = new ArrayList<InternalUser>(30);
    ...
    protected void resetAppointment(Appointment appointment)
    {
        ...
        if(!getNavigator().isGuest())
        {
            ...
            appointment.setName((firstName == null ? "" : firstName + " ") + (lastName == null ? "" : lastName));
        ...
        final TimeOfDay nowTime = new TimeOfDay(hour, min);
    ...
    protected void validateDetails(Appointment appointment)
    {
        ...
        if(occurenceMain.equals(MainOccurence.Times))
        {
        	...
        	else if(timesOne == null || timesOne.intValue() > maxOccurence)
            {
            ...
            {
            	// all is ok
            	occurenceRate.setTimes(timesOne.intValue());
            }
        }
        if(occurenceMain.equals(MainOccurence.EveryTimes))
        {
        	...
        	else if(timesTwo == null || timesTwo.intValue() > maxOccurence)
            {
            	Utils.addErrorMessage(translate(MSG_ERROR_BAD_OCCURENCE_TIME, maxOccurence));
            }
        	else
            {
            	occurenceRate.setTimes(timesTwo.intValue());
            }
        	...
        	{
            	occurenceRate.setEvery(every.intValue());
        	}
        }
    }
    ...
    protected void applyUserSelection()
    {
    	if(isAudienceOpen() == false)
    	{
    		...
    		if(externalUser != null && externalUser.length() > 0)
        	{
        		...
        		StringTokenizer tokens = new StringTokenizer(externalUser, AppointmentImpl.separator, false);
        		String email;
    		...
    		if(internalUsers != null && internalUsers.size() > 0)
    		{
    			...
    			for(InternalUser internalUser : internalUsers)
    			{
    				...
    				if(AuthorityType.getAuthorityType(authority).equals(AuthorityType.GROUP))
    				{
    					...
    					usersInProfile = igRootProfileManagerService.getPersonInProfile(igRootRef, profile.getProfileName());
    ...
    private Integer safeConvert(final String intAsString, final boolean canBeNegative)
    {
        ...
        else
        {
            try
            {
                ...
                return (!canBeNegative && value.intValue() < 0) ? null : value;

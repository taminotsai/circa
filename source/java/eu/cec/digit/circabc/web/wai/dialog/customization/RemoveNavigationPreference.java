...
public class RemoveNavigationPreference extends BaseWaiDialog
{
	...
	public void init(final Map<String, String> parameters)
	{
		...
		if(parameters != null)
		{
			...
			preference = getNavigationPreferencesService().getServicePreference(nodeRef, serviceParam, typeParam);
			globalPreference = preference.getService();

			final ChildAssociationRef parent = getNodeService().getPrimaryParent(nodeRef);
			parentPreference = getNavigationPreferencesService().getServicePreference(parent.getParentRef(), serviceParam, typeParam);
		}
	}
	...
	{
		getNavigationPreferencesService().removeServicePreference(getActionNode().getNodeRef(), globalPreference.getName(), globalPreference.getType());
	...
	public String getConfirmation()
	{
		...
		try
		{
			final SimplePath simplePath = new SimplePath(getNodeService(), parentPreference.getCustomizedOn());
	...
	{
		return translate("remove_navigation_dialog_title", globalPreference.getName(), globalPreference.getType(), getActionNode().getName());
	}

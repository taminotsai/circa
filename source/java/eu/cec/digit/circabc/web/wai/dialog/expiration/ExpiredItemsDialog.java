...
/**
 * Backing bean for the "Show Expired Items" and "Delete Expired Items" dialog.
 * The UI is represented by the file showExpiredItemsDialog.jsp and deleteExpiredItemsDialog.jsp
 *
 * @author makz
 */
public class ExpiredItemsDialog extends BaseWaiDialog implements IContextListener 
{
	...
	private static final String PROP_EXPIRATION_DATE = Repository.escapeQName(DocumentModel.PROP_EXPIRATION_DATE);
	
	private static final String QUERY_SEARCH_All = "PARENT:\"%s\"";
	private static final String QUERY_SEARCH_NAME = QUERY_SEARCH_All + " AND @" + PROP_NAME + ":\"%s\"";
	...
	protected String finishImpl(FacesContext context, String outcome) throws Exception
	{
		...
		if(this.getConfirmationItems().size() > 1)
			Utils.addStatusMessage(FacesMessage.SEVERITY_INFO, translate(MSG_DELETED_ITEMS, this.getConfirmationItems().size()));
		else 
	...
	public void setupConfirmation(ActionEvent event)
	{
		...
		if(componentId.equals("action-link-delete-item"))
		{
			...
			if(params.containsKey("deleteNodeId"))
			{
				...
				addNodeToConfirmationList(new NodeRef(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE, deleteNodeId));
			}
		}
		else if(componentId.equals("action-link-delete-multilingual-item"))
		{
			
		}
		else if(componentId.equals("action-link-delete-all-items"))
		{
			for(Node n : getItems())
			{
	...
	private NodePropertyResolver resolverIsMultilingualContainer = new NodePropertyResolver() 
	{
		private static final long serialVersionUID = -9181535522349485509L;
		public Object get(Node node) {
			return getNodeService().hasAspect(node.getNodeRef(), ContentModel.TYPE_MULTILINGUAL_CONTAINER);
		}
	};
	...
	public void refreshListedItems() 
	{
		...
		List<Node> itemNodes = Collections.<Node> emptyList();
		try 
		{
			...
			if (results != null && results.length() != 0) 
			{
				...
				for (final ResultSetRow row : results) 
				{
					...
					if (getDictionaryService().isSubClass(nodeType, ContentModel.TYPE_FOLDER) == true && 
						getDictionaryService().isSubClass(nodeType, ContentModel.TYPE_SYSTEM_FOLDER) == false) 
					{
		...
		catch (final Throwable err) 
		{
			Utils.addErrorMessage(MessageFormat.format(Application
					.getMessage(FacesContext.getCurrentInstance(),
							Repository.ERROR_GENERIC), new Object[] { err
					.getMessage() }), err);
			try 
			{
				if (tx != null) 
	...
	private String buildSearchQuery() 
	{
		...
		if (this.getDateFilter().equals(FILTER_DATE_TODAY)) 
		{
			fromDate = df.format(cal.getTime());
		} 
		else if (this.getDateFilter().equals(FILTER_DATE_WEEK)) 
		{
			cal.add(Calendar.WEEK_OF_YEAR, -1);
			fromDate = df.format(cal.getTime());
		} 
		else if (this.getDateFilter().equals(FILTER_DATE_MONTH)) 
		{
		...
		if (fromDate != null) 
		{
			...
			buf.append(" AND @");
			buf.append(PROP_EXPIRATION_DATE);
			buf.append(":[");
			buf.append(QueryParser.escape(fromDate));
			buf.append(" TO NOW]");
			query += buf.toString();
		}
	...
	public List<MapNode> getConfirmationItems() 
	{
		if(confirmationItems == null)
			confirmationItems = new ArrayList<MapNode>();
	...
	public SearchService getInternalSearchService() {
        ...
        {
			internalSearchService = Services.getCircabcServiceRegistry(FacesContext.getCurrentInstance()).getNonSecuredSearchService();
        }

...
public class ModerationDialog extends BaseWaiDialog
{
	...
	protected void reject(final String message) throws Exception
	{
		...
		final String creator = (String) getActionNode().getProperties().get(ContentModel.PROP_CREATOR.toString());
		...
		if(personService.personExists(creator))
		{
			...
			final Map<String, Object> model = getMailPreferencesService().buildDefaultModel(nodeRef, creatorRef, null);
			...
			final MailWrapper mail = getMailPreferencesService().getDefaultMailTemplate(nodeRef, MailTemplate.REJECT_POST);

			getMailService().send(noReply, creatorEmail, null ,mail.getSubject(model, locale), mail.getBody(model, locale), true);
		...
		else
		{
			...
			{
				logger.warn("Impossible to found the nodRef of the creator of the post  " + nodeRef + ". Impossible to contact him !!!");
			}
		}
	}
	...
	protected void signalAbuse(final String message) throws Exception
	{
		...
		if(logger.isDebugEnabled())
		{
			logger.debug("Node successfully signaled as abuse: " + nodeRef + ". With message: " + message);
		}
		...
		{
			final String moderatorEmail = (String) getNodeService().getProperty(moderator, ContentModel.PROP_EMAIL);
			String noReply = getMailService().getNoReplyEmailAddress();
			final Locale locale = getMailLanguage(moderator);
			final Map<String, Object> model = getMailPreferencesService().buildDefaultModel(nodeRef, moderator, null);
			model.put(MailTemplate.KEY_ABUSE_DATE, report.getReportDate());
			model.put(MailTemplate.KEY_ABUSE_REASON, (report.getMessage() == null) ? "" : report.getMessage());
			final MailWrapper mail = getMailPreferencesService().getDefaultMailTemplate(nodeRef, MailTemplate.SIGNAL_ABUSE);

			getMailService().send(noReply, moderatorEmail, reporterEmail, mail.getSubject(model, locale), mail.getBody(model, locale), true);
	...
	protected Set<NodeRef> getModerators()
	{
		...
		final List<String> moderatorPermissions = new ArrayList<String>(2);
		...
		final List<Map> moderators = PermissionUtils.getInterestGroupAuthorities(topicRef, moderatorPermissions);
		final Set<NodeRef> moderatorsRef = new HashSet<NodeRef>(moderators.size());
		...
		for(Map moderator: moderators)
		{
			...
			else
			{
				...
				if(profile != null)
				{
					for(final String userId: profileService.getPersonInProfile(igRef, profile.getProfileName()))
					{
	...
	{
		Serializable langObject = getUserService().getPreference(person, UserService.PREF_INTERFACE_LANGUAGE);
		Locale locale;
	...
	{
		final Date date = (Date) getActionNode().getProperties().get(ContentModel.PROP_CREATED.toString());

...
public class EditNavigationPreference extends BaseWaiDialog
{
	...
	public void init(final Map<String, String> parameters)
	{
		...
		if(parameters != null)
		{
			...
			{
				throw new IllegalArgumentException(PARAM_PREF_SERVICE + " AND " + PARAM_PREF_TYPE + " are mandatories.");
			}
			initPreferences(getActionNode().getNodeRef(), serviceParam, typeParam);
		}
		this.selectRenderChoices = new ArrayList<SelectItem>();
		this.selectRenderChoices.add(new SelectItem(navigationListShortNameRender,translate("category_customise_dialog_navigation_shortname_render")));
		this.selectRenderChoices.add(new SelectItem(navigationListTitleRender,translate("category_customise_dialog_navigation_title_render")));
	 ...
	 * @param typeParam
	 */
	private void initPreferences(final NodeRef ref, final String serviceParam, final String typeParam)
	{
		...
		preference = getNavigationPreferencesService().getServicePreference(ref, serviceParam, typeParam);
		
		selectedRenderChoice = (preference.getRenderPropertyName()!= null) ? preference.getRenderPropertyName() : navigationListShortNameRender;
	...
	protected String finishImpl(FacesContext context, String outcome) throws Throwable
	{
		...
		final List<ColumnConfig> newColumns = new ArrayList<ColumnConfig>(getSelectedColumns().size() + 2);
		...
		else
		{
			...
			if(primaryColumType.equals(VALUE_PRIMARY_COL_NAME))
			{
				newColumns.add(name);
			}
			else if(primaryColumType.equals(VALUE_PRIMARY_COL_TITLE))
			{
				newColumns.add(title);
			}
			else if(primaryColumType.equals(VALUE_PRIMARY_COL_NAME_TITLE))
			{
				newColumns.add(name);
				newColumns.add(title);
			}
			else if(primaryColumType.equals(VALUE_PRIMARY_COL_TITLE_NAME))
			{
				newColumns.add(title);
				newColumns.add(name);
			}
		...
		//4. Compute actions
		final List<String> newActions = new ArrayList<String>(getSelectedActions().size());
	...
	{
		initPreferences(getActionNode().getNodeRef(), globalPreference.getName(), globalPreference.getType());
	}
	...
	public boolean isNameAndTitleKeys()
	{
		if(nameAndTitleKeys == null)
		{
			...
			if(nameAndTitleKeys)
			{
				...
				if(col1.equals(col2) || (col1.equals(COLUMN_NAME) == false && col2.equals(COLUMN_NAME) == false) || (col1.equals(COLUMN_TITLE) == false && col2.equals(COLUMN_TITLE) == false))

				throw new IllegalStateException("This kind of double keys columns is not managed by the bean yet: " + col1 + " - " + col2);
			}
		}
	...
	public boolean isBestTitleKey()
	{
		if(bestTitleKey == null)
		{
			...
			if(bestTitleKey && keyColumns.get(0).getName().equals(COLUMN_TITLE_OR_NAME) == false)
			{
				throw new IllegalStateException("This kind of single key column is not managed by the bean yet: " + keyColumns.get(0).getName());
			}
		}
		return bestTitleKey;
	}
	...
	private List<SelectItem> getAllActions()
	{
		if(allAction == null)
		{
			...
			allAction = new ArrayList<SelectItem>(actions.size());

			final Config globalConfig = Application.getConfigService(FacesContext.getCurrentInstance()).getGlobalConfig();
	...
	public List<SelectItem> getListSizes()
	{
		if(listSizes == null)
		{
			...
			listSizes = new ArrayList<SelectItem>(until - from);
	...
	public final List<SelectItem> getAvailableColumns()
	{
		...
		{
			NodeRef currentInterestGroup = getManagementService().getCurrentInterestGroup(getActionNode().getNodeRef());
			List<DynamicProperty> dynamicProperties = Collections.emptyList();
			
			if (currentInterestGroup != null)
			{
				dynamicProperties = this.getDynamicPropertyService().getDynamicProperties(currentInterestGroup);
			}
			...
			{
				availableColumns = new ArrayList<SelectItem>(allColumns.size() - maximumNumberOfDynamicProperties + actualPropertiesSize);
			}
			else
			{
				availableColumns = new ArrayList<SelectItem>(allColumns.size());
			}

			for(final ColumnConfig col : allColumns)
			{
				if(definedColumns.contains(col) == false && keyColumns.contains(col) == false)
				{
					if (col.isDynamicProperty())
					{
						...
						if (dynamicPropetyExists(index,dynamicProperties))
						{
							...
							availableColumns.add(new SelectItem(col.getName(), getDynamicPropertylabelByIndex(dynamicProperties, index)));
						}
					}
					else
					{
	...
	public final List<SelectItem> getSelectedColumns()
	{
		if(selectedColumns == null)
		{
			...
			selectedColumns = new ArrayList<SelectItem>(definedColumns.size() - keyColumns.size());

			for(final ColumnConfig col : definedColumns)
			{
				if(keyColumns.contains(col) == false)
				{
					if (col.isDynamicProperty())
					{
						// process dynamic properties label
						...
						if (dynamicPropetyExists(index, dynamicProperties))
						{
							selectedColumns.add(new SelectItem(col.getName(),getDynamicPropertylabelByIndex(dynamicProperties, index)));
						}
					}
					else
					{
	...
	public final List<SelectItem> getAvailableActions()
	{
		if(availableActions == null)
		{
			...
			availableActions = new ArrayList<SelectItem>(allActions.size());
			Object value;
			for(final SelectItem action : allActions)
			{
				value = action.getValue();
				if(definedActions.contains(value) == false && value.equals(mandatory) == false)
				{
	...
	public final List<SelectItem> getSelectedActions()
	{
		if(selectedActions == null)
		{
			...
			selectedActions = new ArrayList<SelectItem>(definedActions.size() - 1);
			Object value;
			for(final SelectItem action : allActions)
			{
				value = action.getValue();
				if(definedActions.contains(value) && value.equals(mandatory) == false)
				{
	...
	private void move(final ActionEvent event, final String fromPickerId, final List<SelectItem> fromList, final List<SelectItem> toList)
	{
		...
		else
		{
			...
			final List<SelectItem> itemsToMove = new ArrayList<SelectItem>(selectedAsList.size());
			for(final SelectItem item: fromList)
			{
				if(selectedAsList.contains(item.getValue()))
				{
	...
	private void sortColumn(final ActionEvent event, final String fromPickerId, final List<SelectItem> listToSort, boolean up)
	{
		final String[] selected = getSelectedItems(event, fromPickerId);
		if(selected == null || selected.length == 0)
		{
		...
		else
		{
			...
			final List<Integer> itemsToMove = new ArrayList<Integer>(selectSize);

			for(int x = 0; x < sortSize; ++x)
			{
				if(selectedAsList.contains(listToSort.get(x).getValue()))
				{
					itemsToMove.add(Integer.valueOf(x));
				}
			}
	...
	private boolean isColumnFromName(final List<ColumnConfig> columns, final int idx, final String name)
	{
	...
	private ColumnConfig getColumnByName(final String name, List<ColumnConfig> columns, boolean failIsNull)
	{
		...
		{
			throw new IllegalStateException("A column with name " + name + " is not found in the configuration.");
		}
	...
	{
		return translate("edit_navigation_dialog_title", globalPreference.getName(), globalPreference.getType(), getActionNode().getName());
	}

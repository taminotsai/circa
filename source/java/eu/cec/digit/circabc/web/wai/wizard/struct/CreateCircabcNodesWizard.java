...
public class CreateCircabcNodesWizard extends InviteCircabcUsersWizard
{
    ...
    public void init(final Map<String, String> parameters)
    {
        ...
        newsGroupTitle = CircabcConfiguration.getProperty(CircabcConfiguration.NEWSGROUP_NODE_TITLE_PROPERTIES);
        newsGroupDescription = CircabcConfiguration.getProperty(CircabcConfiguration.NEWSGROUP_NODE_DESCRIPTION_PROPERTIES);
        newsGroupIcon = ManagementService.DEFAULT_NEWSGROUP_ICON_NAME;

        libraryTitle = CircabcConfiguration.getProperty(CircabcConfiguration.LIBRARY_NODE_TITLE_PROPERTIES);
        libraryDescription = CircabcConfiguration.getProperty(CircabcConfiguration.LIBRARY_NODE_DESCRIPTION_PROPERTIES);
        libraryIcon = ManagementService.DEFAULT_LIBRARY_ICON_NAME;

        surveyTitle = CircabcConfiguration.getProperty(CircabcConfiguration.SURVEY_NODE_TITLE_PROPERTIES);
        surveyDescription = CircabcConfiguration.getProperty(CircabcConfiguration.SURVEY_NODE_DESCRIPTION_PROPERTIES);
        surveyIcon = ManagementService.DEFAULT_SURVEY_ICON_NAME;

        informationTitle = CircabcConfiguration.getProperty(CircabcConfiguration.INFORMATION_NODE_TITLE_PROPERTIES);
        informationDescription = CircabcConfiguration.getProperty(CircabcConfiguration.INFORMATION_NODE_DESCRIPTION_PROPERTIES);
        ...
        eventDescription = CircabcConfiguration.getProperty(CircabcConfiguration.EVENT_NODE_DESCRIPTION_PROPERTIES);
        ...
        else if(getActionNode().getNodeRef().equals(getManagementService().getCompanyHomeNodeRef()))
        {
            ...
            name = CircabcConfiguration.getProperty(CircabcConfiguration.CIRCABC_ROOT_NODE_NAME_PROPERTIES);
            title = CircabcConfiguration.getProperty(CircabcConfiguration.CIRCABC_ROOT_NODE_TITLE_PROPERTIES);
            description = CircabcConfiguration.getProperty(CircabcConfiguration.CIRCABC_ROOT_NODE_DESCRIPTION_PROPERTIES);
        }
        else
        {
        ...
        this.isInterestGroupToCreate = this.toCreateNodeType.equals(NodeType.IGROOT);
    }
            ...
            public String execute() throws Throwable
            {
                try
                {
                    ...
                    catch (final Throwable e)
                    {
                        ...
                        {
                            case CIRCABC:
                                if(logger.isErrorEnabled()) logger.error("Impossible to create a Circabc root folder named " + getName() + " under the space " + getActionNode().getName(),e); break;
                            case CATEGORY:
                                if(logger.isErrorEnabled()) logger.error("Impossible to create a Category named " + getName() + " under the Circabc root node " + getActionNode().getName(),e); break;
                            case IGROOT:
                                if(logger.isErrorEnabled()) logger.error("Impossible to create an Interest group named " + getName() + " under the Category " + getActionNode().getName(),e); break;
                        }
                    }
    ...
    public String next()
    {
        ...
        if(step == 2)
        {
            ...
            else if(getNodeService().getChildByName(getActionNode().getNodeRef(), ContentModel.ASSOC_CONTAINS, this.name) != null)
            {
                FacesContext.getCurrentInstance().addMessage(
            ...
            if(this.lightDescription.length()>LightDescriptionSizeExceedException.MAX_CHARACTER_LIMIT)
            {
            	 FacesContext.getCurrentInstance().addMessage(
    ...
    {
        final NodeRef circabcNodeRef = getManagementService().createCircabc(getActionNode().getNodeRef());
        ...
        uiFacetsProps.put(ContentModel.PROP_DESCRIPTION, this.description == null ? "" : this.description);
        ...
        if(logger.isDebugEnabled())
        {
            logger.debug("Circabc root folder '" + getName() +  "' created under the " + getActionNode().getName()
                    + "\n  Title: " + getTitle()
    ...
    protected void createCategory(final FacesContext context)
    {
        final NodeRef categoryNodeRef = getManagementService().createCategory(
                    ...
                    getName(),
                    new NodeRef(categoryHeaderId)
                );

        //	apply the uifacets aspect - icon, title and description props
        final Map<QName, Serializable> uiFacetsProps = new HashMap<QName, Serializable>(5);
        uiFacetsProps.put(ApplicationModel.PROP_ICON, this.icon);
        uiFacetsProps.put(ContentModel.PROP_TITLE, (this.title == null || this.title.trim().length() < 1) ? getName() : this.title);
        uiFacetsProps.put(ContentModel.PROP_DESCRIPTION, this.description == null ? "" : this.description);
    ...
    {
        // create the ig root
        final NodeRef igRootNodeRef = getManagementService().createIGRoot(
        ...
        if(logger.isDebugEnabled())
        {
            logger.debug("Interest group '" + getName() + "' created under the Category " + getActionNode().getName()
                    + "\n  Title: " + getTitle()
                    + "\n  Description: " + getDescription()
    ...
    protected NodeRef createLibrary(final NodeRef igRootNodeRef)
    {
        ...
        uiFacetsProps.put(ContentModel.PROP_TITLE, this.libraryTitle);
        ...
        if(logger.isDebugEnabled())
        {
            logger.debug("Library created under " + getNodeService().getProperty(igRootNodeRef, ContentModel.PROP_NAME)
    ...
    protected NodeRef createNewsGroup(final NodeRef igRootNodeRef)
    {
        ...
        uiFacetsProps.put(ContentModel.PROP_TITLE, this.newsGroupTitle);
        uiFacetsProps.put(ContentModel.PROP_DESCRIPTION, this.newsGroupDescription);
        getNodeService().addAspect(circabcNewsGroupNodeRef, ApplicationModel.ASPECT_UIFACETS, uiFacetsProps);

        if(logger.isDebugEnabled())
        {
            logger.debug("NewsGroup created under " + getNodeService().getProperty(igRootNodeRef, ContentModel.PROP_NAME)
    ...
    protected NodeRef createSurvey(final NodeRef igRootNodeRef)
    {
        ...
        if(logger.isDebugEnabled())
        {
            logger.debug("Survey created under " + getNodeService().getProperty(igRootNodeRef, ContentModel.PROP_NAME)
    ...
    protected NodeRef createInformation(final NodeRef igRootNodeRef)
    {
        ...
        uiFacetsProps.put(ContentModel.PROP_TITLE, this.informationTitle);
        ...
        if(logger.isDebugEnabled())
        {
            logger.debug("Information created under " + getNodeService().getProperty(igRootNodeRef, ContentModel.PROP_NAME)
    ...
    protected NodeRef createEvent(final NodeRef igRootNodeRef)
    {
        ...
        uiFacetsProps.put(ContentModel.PROP_TITLE, this.eventTitle);
        ...
        if(logger.isDebugEnabled())
        {
            logger.debug("Event created under " + getNodeService().getProperty(igRootNodeRef, ContentModel.PROP_NAME)
    ...
    public String getSelectedUserMessage()
    {
        ...
        {
            case CIRCABC:
                msgId = MESSAGE_ID_CREATE_CIRCABC_SELECTED_USERS; break;
            case CATEGORY:
                msgId = MESSAGE_ID_CREATE_CATEGORY_SELECTED_USERS; break;
            case IGROOT:
                msgId = MESSAGE_ID_CREATE_IG_SELECTED_USERS; break;
        }
    ...
    public String getErrorMessage()
    {
        ...
        {
            case CIRCABC:
                msgId = MESSAGE_ID_CREATE_CIRCABC_ERROR; break;
            case CATEGORY:
                msgId = MESSAGE_ID_CREATE_CATEGORY_ERROR; break;
            case IGROOT:
                msgId = MESSAGE_ID_CREATE_IG_ERROR; break;
        }
    ...
    public DataModel getCategoryListDataModel()
    {
        ...
        final List<CategoryWrapper> categoriesWrapper = new ArrayList<CategoryWrapper>(categories.size());
    ...
    public void addSelectedUsers(final ActionEvent event)
    {
        ...
        if (results != null)
        {
            ...
            {
                case CIRCABC:
                    profile = CIRCABC_ADMIN_PROFILE; break;
                case CATEGORY:
                    profile = CATEGORY_ADMIN_PROFILE; break;
                case IGROOT:
                    profile = IG_LEADER_PROFILE; break;
            }

            if(profile == null)
            {
                logger.error("The application is in a illegal state. Impossible to determine which kind of administrator must be setted for the creation of such a node. The createion of the special space '" + getName()  + "'failed.");

                throw new IllegalStateException("Impossible to determine which kind of administrator must be setted for the creation of such a node");
            }

            // invite all selected users
            for (int i = 0; i < results.length; i++)
            {
                final String authority = results[i];
                String userName = "" ;
                ...
                {
                    final UserGroupProfile wrapper = this.userGroupProfiles.get(n);
                ...
                {
                    FacesContext.getCurrentInstance().addMessage(
                            null, new FacesMessage(getBundle().getString(InviteCircabcUsersWizard.USER_SPECIFIED_TWICE)));
                }
            }
        }
    }
    ...
    public boolean getNextButtonDisabled()
    {
        ...
        if (toCreateNodeType == NodeType.CATEGORY)
        {
            ...
            {
                return (!requiredPropertiesSet) ?  true : ((!categoryHeaderIdSetted) ? false : userGroupProfiles.isEmpty());
            }
        }
        else
        {
     ...
     */
    public void setContact(final String con)
    {
    	if(con!=null && !(con.length()==0))
    	{
	    	...
	    	this.contact = Jsoup.clean(con, basicWhitelist);
    	}
    	else
    	{
    		this.contact = con;
    ...
    public void setDescription(final String desc) 
    {
    	if(desc!=null && !(desc.length() == 0))
    	{
	    	...
	    	basicWhitelist.addTags("p", "span", "strong", "b", "i", "u", "br", "sub", "sup", "a");
	    	basicWhitelist.addAttributes(":all", "style");
	    	this.description = Jsoup.clean(desc, basicWhitelist);
    	}
    	else
    	{
   ...
   public List<UIListItem> getIcons()
   {
      ...
      //       which the user can change during the advanced space wizard
       ...
       final List<String> iconNames = new ArrayList<String>(8);

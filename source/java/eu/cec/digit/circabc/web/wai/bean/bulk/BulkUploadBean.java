...
public class BulkUploadBean extends AddContentBean
{
    ...
    public void init(final Map<String, String> parameters)
    {
        ...
        if(parameters != null)
		{
            ...
            errorFound = false;
            this.indexFileEntries = Collections.<IndexRecord>emptyList();
            this.uploadedEntries = new ArrayList<UploadedEntry>();
            this.indexRecordCountCache = new HashMap<String, Integer>();
    ...
    {
    	final String value = (uploadedFile == null) ? null : uploadedFile.getSubmitedProperty(ID_CHECK_DISABLE_INDEX);
        ...
        try {
            ...
            if (fileBean != null) {
				...
				try {
                    ...
                    //Reset the cache
                    indexRecordCountCache = new HashMap<String, Integer>();

                    if(isSubmitedSkipIndexfile(fileBean))
                    {
                    	indexFileEntries = Collections.<IndexRecord>emptyList();
                    }
                    else
                    {
                    ...
                    uploadedEntries = getBulkService().upload(getNavigator().getCurrentNode().getNodeRef(), fileBean.getFile(), indexFileEntries, messages);
                    ...
                    if(!foundFatal) {
                    	messages.add(new ValidationMessageImpl(-1 /* TODO */, uploadedFile.getFileName(), "Error while uploading the file:" + uploadedFile.getFileName() + " caused by:" + ex.getMessage() + " Please contact the Circabc Support Team", ErrorType.Fatal));
                    }
                    if(logger.isErrorEnabled()) {
    ...
    {
        final HttpServletRequest httpReq = (HttpServletRequest) FacesContext.getCurrentInstance().getExternalContext().getRequest() ;
        if (httpReq.getAttribute(IS_SECURE_TRANSPORT) == null) {
            ...
            httpReq.setAttribute(IS_SECURE_TRANSPORT, Boolean.valueOf(this.isSecureTransport));
        } else {
            this.isSecureTransport = ((Boolean)httpReq.getAttribute(IS_SECURE_TRANSPORT)).booleanValue();
        }
        return this.isSecureTransport ;
    }

    private void upload() throws NotSupportedException, SystemException, SecurityException, IllegalStateException, RollbackException, HeuristicMixedException, HeuristicRollbackException {
        ...
        if (fileBean != null) 
        {
			...
			{
                messages.add(new ValidationMessageImpl(-1 /* TODO */, uploadedFile.getFileName(), "Error while uploading the file:" + uploadedFile.getFileName() + " caused by:" + ex.getMessage(), ErrorType.Fatal));
                if(logger.isErrorEnabled())
                {
    ...
    private int getGenericCountCache(final String headerName) {
        ...
        if(count == -1) {
            ...
            indexRecordCountCache.put(headerName, Integer.valueOf(count));
        }
        return count;
    }
        ...
        for(final ValidationMessage message : messages) {
            if(message.getErrorType().equals(ErrorType.Fatal)) {
                if(logger.isWarnEnabled()) {
                    logger.warn("Found an error in file:" + message.getFileName() + " description:" + message.getErrorDescription());
                }
                disabled = true;
                break;
            }
...
//  ------------------------------------------------------------------------------
    ...
    public TransactionService getTransactionService() {
        if (this.transactionService == null) {
            final FacesContext fc = FacesContext.getCurrentInstance();
            this.transactionService = (TransactionService)Services.getAlfrescoServiceRegistry(fc).getTransactionService();
        }
   ...
   public NotificationService getNotificationService() {
       if (this.notificationService == null) {
           final FacesContext fc = FacesContext.getCurrentInstance();
           this.notificationService = (NotificationService)Services.getCircabcServiceRegistry(fc).getNotificationService();
       }
  ...
  public NotificationSubscriptionService getNotificationSubscriptionService() {
      if (this.notificationSubscriptionService == null) {
          final FacesContext fc = FacesContext.getCurrentInstance();
          this.notificationSubscriptionService = (NotificationSubscriptionService)Services.getCircabcServiceRegistry(fc).getNotificationSubscriptionService();
      }
   ...
   */
  public void setNotificationSubscriptionService(final NotificationSubscriptionService notificationSubscriptionService) {
	 ...
	 */
	public final void setDisableIndexFile(boolean ignore)
	{
	}

...
/**
 * Dialog bean to upload a new translation without content and to add it to an existing MLContainer.
 *
 * @author David Ferraz
 */
public class AddTranslationWithoutContentDialog extends org.alfresco.web.bean.ml.AddTranslationWithoutContentDialog implements WaiDialog
{
	...
	public void init(Map<String, String> parameters)
	{
		...
		if(this.isFinished == false && this.newTranslation != null)
		{
			return;
		}
		...
		if (id != null && id.length() > 0)
		{
			try
			{
				...
				if (ApplicationModel.TYPE_FOLDERLINK.equals(getNodeService().getType(this.newTranslation)))
				{
					this.newTranslation = (NodeRef) getNodeService().getProperty(this.newTranslation, ContentModel.PROP_LINK_DESTINATION);
				}
			}
			catch (InvalidNodeRefException refErr)
			{
				Utils.addErrorMessage(MessageFormat.format(Application.getMessage(FacesContext.getCurrentInstance(), Repository.ERROR_NODEREF), new Object[]
				{ id }));
			}
		} else
		{
		...
		if (getNodeService().hasAspect(this.newTranslation, ContentModel.ASPECT_AUTHOR) == true && getNodeService().getProperty(this.newTranslation, ContentModel.PROP_AUTHOR) != null)
		{
			setAuthor((String) getNodeService().getProperty(this.newTranslation, ContentModel.PROP_AUTHOR));
		} else
		{
			setAuthor("");
		}

		if (getNodeService().hasAspect(this.newTranslation, ContentModel.ASPECT_LOCALIZED) == true && getNodeService().getProperty(this.newTranslation, ContentModel.PROP_LOCALE) != null)
		{
			setLanguage(((Locale) getNodeService().getProperty(this.newTranslation, ContentModel.PROP_LOCALE)).toString());
		} else
		{
			setLanguage(null);
		}
		...
		// We do this to use
		this.navigator.setCurrentNodeId(getNodeService().getPrimaryParent(this.newTranslation).getParentRef().getId());
	...
	protected boolean validate()
	{
		...
		{
			Utils.addErrorMessage(Application.getMessage(FacesContext.getCurrentInstance(), "make_multilingual_error_language"));
			valid = false;
		}
        ...
        {
			Utils.addErrorMessage(Application.getMessage(FacesContext.getCurrentInstance(), "make_multilingual_error_author"));
			valid = false;
        }
...
//	@Override
	protected String doPostCommitProcessing(FacesContext context, String outcome)
	{
		...
		if(isShowOtherProperties())
		{
	        ...
	        FacesContext.getCurrentInstance().getViewRoot().setViewId(CircabcNavigationHandler.WAI_NAVIGATION_CONTAINER_PAGE);
			return CircabcNavigationHandler.CLOSE_WAI_DIALOG_OUTCOME + CircabcNavigationHandler.OUTCOME_SEPARATOR + EditNodePropertiesDialog.DIALOG_CALL;
		}
		else
		{
	...
	{
		return Application.getMessage(FacesContext.getCurrentInstance(), "add_translation_without_content_action_wai");
	}

	public String getPageIconAltText()
	{
		return Application.getMessage(FacesContext.getCurrentInstance(), "add_translation_without_content_icon_tooltip");
	}

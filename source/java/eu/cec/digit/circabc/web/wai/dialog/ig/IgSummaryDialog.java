...
public class IgSummaryDialog extends BaseWaiDialog {
	...
	public void init(Map<String, String> parameters) {
		...
		this.availableStatisticChoices = new ArrayList<SelectItem>();
		...
		this.availableExportOptions = new ArrayList<SelectItem>();
	...
	private void buildIGData() {
		...
		this.igData = new ArrayList<StatData>();

		
		this.igData.add(new StatData(translate("ig_summary_statistic_created_date"), this.statsServ.getIGCreationDate(currentNode)));
		this.igData.add(new StatData(translate("ig_summary_statistic_number_of_users"),this.statsServ.getNumberOfUsers(currentNode)));
		this.igData.add(new StatData(translate("ig_summary_statistic_library_folder_count"),this.statsServ.getNumberOfLibrarySpaces(currentNode)));
		this.igData.add(new StatData(translate("ig_summary_statistic_library_document_count"),this.statsServ.getNumberOfLibraryDocuments(currentNode)));
		Long libSize= this.statsServ.getContentSizeOfLibrary(currentNode);
		this.igData.add(new StatData(translate("ig_summary_statistic_library_size"), libSize+ " bytes"));
		this.igData.add(new StatData(translate("ig_summary_statistic_information_folder_count"),this.statsServ.getNumberOfInformationSpaces(currentNode)));
		this.igData.add(new StatData(translate("ig_summary_statistic_information_document_count"),this.statsServ.getNumberOfInformationDocuments(currentNode)));
		Long infSize=this.statsServ.getContentSizeOfInformation(currentNode);
		this.igData.add(new StatData(translate("ig_summary_statistic_information_size"), infSize+ " bytes"));
		this.igData.add(new StatData(translate("ig_summary_statistic_total_size"), (libSize + infSize) + " bytes"));		
		this.igData.add(new StatData(translate("ig_summary_statistic_event_count"), this.statsServ.getNumbetOfEvents(currentNode)));
		this.igData.add(new StatData(translate("ig_summary_statistic_meeting_count"), this.statsServ.getNumbetOfMeetings(currentNode)));
		this.igData.add(new StatData(translate("ig_summary_statistic_forum_count"), this.statsServ.getNumberOfForums(currentNode)));
		this.igData.add(new StatData(translate("ig_summary_statistic_topic_count"), this.statsServ.getNumberOfTopics(currentNode)));
		this.igData.add(new StatData(translate("ig_summary_statistic_post_count"), this.statsServ.getNumberOfPosts(currentNode)));
	...
	private void builIGStructure() {
		ServiceTreeRepresentation informationRepresent = this.statsServ.getInformationStructure(currentNode);
		ServiceTreeRepresentation libraryRepresent = this.statsServ.getLibraryStructure(currentNode);
		ServiceTreeRepresentation newgroupsRepresent = this.statsServ.getNewsgroupsStructure(currentNode);
	...
	private String buildUlLiTreeFolder(Child child, Boolean firstChild)
	{
		...
		if(firstChild)
		{
			html.append("<span class=\"recordSetHeader\">"+child.getName()+"</span>");
			html.append("<br/>");
		}
		else{
	...
	public String export(ActionEvent event) {
		...
		try {
			...
			if (getSelectedStatisticExport().equals(EXPORT_CSV) && getSelectedStatisticChoices().equals(GLOBAL_STATISTICS)) {
				response.setHeader("Content-Disposition","attachment;filename=Statistics.csv");
				response.setContentType("text/csv;charset=UTF-8");
				writeCSVData(getIgData(), outStream);
			}

			else if (getSelectedStatisticExport().equals(EXPORT_XML) && getSelectedStatisticChoices().equals(GLOBAL_STATISTICS)) {
				response.setHeader("Content-Disposition","attachment;filename=Statistics.xml");
				response.setContentType("text/xml;charset=UTF-8");
				writeXMLData(getIgData(), outStream);
			}

			else if (getSelectedStatisticExport().equals(EXPORT_XLS) && getSelectedStatisticChoices().equals(GLOBAL_STATISTICS)) {
				response.setHeader("Content-Disposition","attachment;filename=Statistics.xls");
				response.setContentType("application/vnd.ms-excel;charset=UTF-8");
				writeXLS(getIgData(), outStream);
			}
			else if (getSelectedStatisticExport().equals(EXPORT_CSV) && getSelectedStatisticChoices().equals(TIMELINE_STATISTICS)) {
				writeCSVTimeline(getIgTimeLineActivity(), outStream);
			}

			else if (getSelectedStatisticExport().equals(EXPORT_XML) && getSelectedStatisticChoices().equals(TIMELINE_STATISTICS)) {
			...
			else if (getSelectedStatisticExport().equals(EXPORT_XLS) && getSelectedStatisticChoices().equals(TIMELINE_STATISTICS)) {
	...
	private void writeXLS(List<StatData> statDatas, ServletOutputStream outStream) throws IOException {
	...
	private void writeXLSTimeline(List<ActivityCountDAO> igTimeLineActivities, ServletOutputStream outStream) throws IOException {
    ...
    private void writeXMLTimeline(List<ActivityCountDAO> igTimeLineActivity2, ServletOutputStream outStream) throws XMLStreamException {
        ...
        try
        {
        	...
        	for (ActivityCountDAO dim : igTimeLineActivity2) {
    			...
    			xtw.writeStartElement(dim.getService()+" "+dim.getActivity());
	...
	private void writeCSVTimeline(List<ActivityCountDAO> igTimeLineActivity2, ServletOutputStream outStream) {
	...
	private void writeXMLData(List<StatData> IGdata, ServletOutputStream outStream) throws XMLStreamException {
	...
	private void writeCSVData(List<StatData> IGdata, ServletOutputStream outStream) throws IOException
    {

...
public class MigrateProperties extends AddContentBean
{
    ...
    private static SimpleDateFormat _DATE_FORMAT = new SimpleDateFormat("dd/MM/yyyy");
    ...
    protected String finishImpl(final FacesContext context, String outcome) throws Exception
    {
    	...
    	{
    		throw new IllegalArgumentException("Invalid number of column. Expected: " +  EXPECTED_COL_TITLES.length);
    	}
    	for(int x = 0; x < EXPECTED_COL_TITLES.length; ++x)
    	{
    		if(!EXPECTED_COL_TITLES[x].equals(nextLine[x]))
    		{
    			throw new IllegalArgumentException("Invalid column name at position " + (x+1) + "Expected: " +  EXPECTED_COL_TITLES[x]);
    		}
    	}
    ...
    public static Date convert(String dateAsString)
    {
    	...
    	try
		{
			return  _DATE_FORMAT.parse(dateAsString);
		}

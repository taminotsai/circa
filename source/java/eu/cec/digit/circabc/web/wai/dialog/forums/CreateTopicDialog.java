...
public class CreateTopicDialog extends CreateContentBaseDialog
{
   ...
   public List<UIListItem> getIcons()
   {
      ...
      final Config config = Application.getConfigService(FacesContext.getCurrentInstance()).getConfig(typePrefixForm + " icons");
   ...
   /**
	 ...
	 */
	/*package*/ NodeRef createTopicNode(final NodeRef forumNodeRef) throws FileExistsException, InvalidNodeRefException, InvalidAspectException
	{
		NodeRef topicNodeRef;
		final String validName = WebClientHelper.toValidFileName(this.name);
		final FileInfo topicFile = getFileFolderService().create(forumNodeRef, validName, ForumModel.TYPE_TOPIC);
		...
		final Map<QName, Serializable> uiFacetsPropTopics = new HashMap<QName, Serializable>(5);
  	...
  	/*package*/ NodeRef createPost(final FacesContext context, final NodeRef topicNodeRef, final String _message) throws FileExistsException, InvalidNodeRefException, InvalidAspectException, InvalidTypeException, ContentIOException
	{
		NodeRef postNodeRef;
		...
		final FileInfo postFile  = getFileFolderService().create(topicNodeRef, fileName, ForumModel.TYPE_POST);
		...
		final Map<QName, Serializable> editProps = new HashMap<QName, Serializable>(1, 1.0f);
		...
		final ContentWriter writer = getContentService().getWriter(postNodeRef, ContentModel.PROP_CONTENT, true);
		// set the mimetype and encoding
		writer.setMimetype(Repository.getMimeTypeForFileName(context, fileName));
		writer.setEncoding("UTF-8");
		writer.putContent(_message);
	...
	public final List<SelectItem> getSecurityRankings()
	{
		...
		final List<SelectItem> items = new ArrayList<SelectItem>(rankings.size());

...
public class SelfRegistrationDialog extends CreateCircabcUserWizard
{
    ...
    /**
     * Method called at the end of the registration dialog to create an inactive user
     * after testing the validity of the fields.
     *
    		...
    		final RetryingTransactionCallback<String> callback = new RetryingTransactionCallback<String>()
    		{
    			public String execute() throws Throwable
    			{
                    ...
                    catch (Exception e)
                    {
                        ...
                        {
                    		logger.error("The user " + getUserName() + " can't register itself due to an error: " + e.getMessage(),e);
                        }
                    ...
                    {
                        logger.debug("The user " + getUserName() + " has just registred itself. Its activation is " + activationURL);
                    }
    ...
    {
    	return FacesContext.getCurrentInstance().getExternalContext().getRequestContextPath() + "/captchaSound.wav?bean=" + BEAN_NAME;
    }

    @Override
    protected boolean validateFields(boolean validateUsername, boolean validatePassword, boolean validateMadatotyProperties, boolean addFacesErrorMessages)
    {
    	super.validateFields(validateUsername, validatePassword, validateMadatotyProperties, addFacesErrorMessages);
    ...
    /**
     * @return the translated error launched by the captcha validation or null if validation successfull
     */
    ...
    /**
     * Return the jsp name of the registration page and init the form. Sould be used for each externale call !!!
     *
     * @return
     */
     ...
     */
    public void validateCaptcha(FacesContext context, UIComponent component, Object value) throws ValidatorException
    {
        ...
        if(isResponseCorrect.booleanValue() == false)
        {
     ...
     *
     * @param URL
     *            The activation link to send to the user
     */
	...
	protected boolean notifyUser(String URL) throws Exception
    {
        ...
        if (to != null && to.length() != 0)
        {
    		...
    		final MailWrapper mail = getMailPreferencesService().getDefaultMailTemplate(circabcRoot, MailTemplate.SELF_REGISTRATION);

    		I18NUtil.setLocale(currentLocale);

    		return getMailService().send(getMailService().getNoReplyEmailAddress(),to ,null, mail.getSubject(model), mail.getBody(model), true);
        }
        else
        {
        	return false;
     ...
     */
    protected String generateActivationURL(String userName, String activationKey) throws UnsupportedEncodingException
    {

        final StringBuffer path = new StringBuffer();

        final String urlRoot = CircabcConfiguration.getProperty(CircabcConfiguration.WEB_ROOT_URL_PROPERTIES);

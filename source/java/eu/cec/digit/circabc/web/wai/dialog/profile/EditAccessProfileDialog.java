...
public class EditAccessProfileDialog extends BaseWaiDialog
{
    ...
    public void init(final Map<String, String> params)
    {
        ...
        if(params != null)
        {
            ...
            if (oldProfileName != null)
            {
	            ...
	            else if (oldProfileName.equals(CircabcRootProfileManagerService.ALL_CIRCA_USERS_PROFILE_NAME))
	            {
            ...
            {
            	// edition mode
            	final ProfileManagerService profService = getProfileManagerServiceFactory().getIGRootProfileManagerService();
            	...
            	selectedProfile = new AccessProfileWrapper(profService.getProfile(igNodeRef, profileName), igNodeRef, currentUserName);
    ...
    protected String finishImpl(FacesContext context, String outcome) throws Throwable
    {
        ...
        if(getTranslationDataModel().getRowCount() < 1)
        {
            ...
            {
                logger.debug("Impossible to add a title to the Profile " + selectedProfile.getName() + " because the list of translation is empty.");
            }
        ...
        if(logger.isDebugEnabled())
        {
            logger.debug("Trying to add update / create  the Profile " + selectedProfile.getName() + "...");
        }
        ...
        if(selectedProfile.getAutority() == null)
        {
         ...
         if (oldProfileName == null)
         {
        	 info = MessageFormat.format("Profile {0} permissions : {1}", new Object[]{webProf.getProfileName(), servicesPermissions.toString() });
         }
         else
         {
        	 info = MessageFormat.format("Profile {0} permissions : {1}", new Object[]{oldProfileName, servicesPermissions.toString() });
         }
		 logRecord.setInfo(info ) ;

        // update the profile at the IG Root level and at ig services level
		profService.updateProfile(igNodeRef, webProf.getProfileName(), webProf.getServicesPermissions(), false);
        profService.updateProfileMap(igNodeRef, webProf);


        if(logger.isDebugEnabled())
        {
            logger.debug("Profile  successfully updated for the Interest Group " + getActionNode().getName()
                    + "\n	ig: " + igNodeRef
                   +  "\n	profile: " + selectedProfile.getName());

            logger.debug("Time to create/update profile: " + (System.currentTimeMillis() - startTime) + "ms");
        }
	 ...
	 */
	private void createProfile(final ProfileManagerService profService, final Profile webProf, final NodeRef igNodeRef) throws ProfileException
	{
		...
		if(logger.isDebugEnabled())
		{
		    logger.debug("Profile successfully created for the Interest Group " + selectedProfile.getName()
		            + "\n	ig: " + igNodeRef
		            + "\n	profile: " + name);
		}
	}
	...
	{
		final List<ServicePermissionWrapper> wrappers = (List<ServicePermissionWrapper>) this.permissionDataModel.getWrappedData();
    ...
    public DataModel getTranslationDataModel()
    {
        ...
        final List<Entry<Locale, String>> entryList = new ArrayList<Entry<Locale,String>>(entrySet.size());
    ...
    public DataModel getPermissionDataModel()
    {
        if (this.permissionDataModel == null)
        {
            ...
            final List<ServicePermissionWrapper> wrappers = new ArrayList<ServicePermissionWrapper>(5);

            final String profileName = selectedProfile.getName();
			boolean isSpecial = profileName != null &&
						(profileName.equals(CircabcRootProfileManagerService.ALL_CIRCA_USERS_PROFILE_NAME) || profileName.equals(CircabcConstant.GUEST_AUTHORITY));

            final InformationPermissions[] infPerm = (isSpecial) ? InformationPermissions.minimalValues() : InformationPermissions.values();
            final LibraryPermissions[] libPerm = (isSpecial) ? LibraryPermissions.minimalValues() : LibraryPermissions.values();
            final DirectoryPermissions[] dirPerm = (isSpecial) ? DirectoryPermissions.minimalValues() : DirectoryPermissions.values();
            final EventPermissions[] evePerm = (isSpecial) ? EventPermissions.minimalValues() : EventPermissions.values();
            final NewsGroupPermissions[] newPerm = (isSpecial) ? NewsGroupPermissions.minimalValues() : NewsGroupPermissions.values();

            wrappers.add(new ServicePermissionWrapper(CircabcServices.INFORMATION, this.selectedProfile.getInfPermissionValue(), infPerm));
			wrappers.add(new ServicePermissionWrapper(CircabcServices.LIBRARY,     this.selectedProfile.getLibPermissionValue(), libPerm));
			wrappers.add(new ServicePermissionWrapper(CircabcServices.DIRECTORY,   this.selectedProfile.getDirPermissionValue(), dirPerm, editPorfileOfCurrentUser));
			wrappers.add(new ServicePermissionWrapper(CircabcServices.EVENT,       this.selectedProfile.getEvePermissionValue(), evePerm));
			wrappers.add(new ServicePermissionWrapper(CircabcServices.NEWSGROUP,   this.selectedProfile.getNewPermissionValue(), newPerm));
    ...
    {
    	final Map.Entry<Locale, String> wrapper = (Map.Entry<Locale, String>) this.translationDataModel.getRowData();
        if (wrapper != null)
        {
	 ...
	 */
	public final void setContentFilterLanguagesService(final ContentFilterLanguagesService contentFilterLanguagesService)
	{

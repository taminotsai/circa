...
public class RemoveLogoPreference extends BaseWaiDialog
{
	...
	protected String finishImpl(FacesContext context, String outcome) throws Throwable
	{
		try
		{
			...
			final List<LogoDefinition> logos = getLogoPreferencesService().getAllLogos(currentLocationRef);
			...
			if(logoByName == null)
			{
		        ...
		        {
		        	logger.debug("No logo with the name " + this.logoName + " found on " + getActionNode().getName());
		        }
			...
			{
				Utils.addErrorMessage(translate(MSG_ERROR_LOCATION_DELETE, getPath(logoByName.getDefinedOn())));

				if(logger.isDebugEnabled())
				{
					logger.debug("Delete a logo defined on a parent location is not allowed. Parent: " + logoByName.getDefinedOn() + " instand of " + currentLocationRef);
				}
			...
			else
			{
				...
				Utils.addStatusMessage(FacesMessage.SEVERITY_INFO, translate(MSG_LOGO_DELETED, this.logoName));
		...
		catch(final Throwable t)
		{
	         ...
	         if(logger.isDebugEnabled())
	         {
	        	 logger.debug("Unexpected error during removing logo on " + getPath(getActionNode().getNodeRef()), t);
	         }

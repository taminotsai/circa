...
public abstract class SelectKeywordsBaseDialog extends ManageKeywordsDialog
{
    ...
    public void addSelection(ActionEvent event)
    {
        ...
        if(picker == null)
        {
        	throw new NullPointerException("No select many component found with the ID set-keywords-list");
        }
	...
	{
		final List<SelectItem> itemAsList = new ArrayList<SelectItem>();
		final Locale filteredLanguage = (getSelectedLanguage() == null || getSelectedLanguage().equals(NULL_VALUE)) ? null : new Locale(getSelectedLanguage());
			...
			for(final Map.Entry<NodeRef, Keyword> entry : interestGroupKeywords.entrySet())
			{
				if(entry.getValue().isKeywordTranslated())
				{
					...
					if(!settedKeywords.containsKey(entry.getKey()))
					{
						...
						{
							itemAsList.add(new SelectItem(entry.getKey().toString(), values.get(filteredLanguage)));
						}
					}
				}
				else
	...
	{
		final List<KeywordWrapper> keys = new ArrayList<KeywordWrapper>(settedKeywords.size());
	...
	protected void initKeywords()
	{
		...
		interestGroupKeywords = new HashMap<NodeRef, Keyword>(keywords.size());
		...
		if(getActionNode() != null)
		{
			...
			{
			   logger.debug("Trying to retreive the keyword list for the current document : " + getActionNode().getName() +  "(" + getActionNode().getNodeRef() + ")");
			}

			// get the keywords assigned to the document and fill the map
			final List<Keyword> docKeywordsList = getKeywordsService().getKeywordsForNode(getActionNode().getNodeRef());
			...
			if(logger.isDebugEnabled())
			{
			   logger.debug("Keyword successfully retreived for the document : " + getActionNode().getName() +  "(" + getActionNode().getNodeRef() + ")"

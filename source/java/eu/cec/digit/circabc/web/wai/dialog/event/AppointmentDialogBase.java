...
public abstract class AppointmentDialogBase extends CreateEventWizard {
	...
	protected void setupAppointement()
	{
		...
		if(AudienceStatus.Closed.equals(getAppointment().getAudienceStatus()))
		{
	        ...
	        internalUsers = new ArrayList<InternalUser>(users.size());
	        externalUser = "";
		...
		{
			internalUsers = new ArrayList<InternalUser>(5);
			externalUser = "";
		}
	...
	{
		return isMeeting() ? translate(MSG_DIALOG_MEETING_INSTRUCTION)  : translate(MSG_DIALOG_EVENT_INSTRUCTION);
	}
	...
	public final void setOccurenceChoice(String occurenceChoice)
	{
		...
		else if(!(occurenceChoice.equals(CHOICE_THIS) || occurenceChoice.equals(CHOICE_SERIES)|| occurenceChoice.equals(CHOICE_FUTURS)))
		{
			throw new IllegalArgumentException("Only following choices are possible: " + CHOICE_THIS + " Or " + CHOICE_SERIES + " Or " + CHOICE_FUTURS);
		}

		...
		try {
			...
			if (results != null && results.length() != 0) {
				...
				for (final ResultSetRow row : results) {
					...
					childRef = (ChildAssociationRef) getInternalNodeService().getProperty(nodeRef, ContentModel.PROP_ARCHIVED_ORIGINAL_PARENT_ASSOC);
					if (getInternalNodeService().exists(childRef.getParentRef()))
		               {
						originalNodePath = getInternalNodeService().getPath(childRef.getParentRef()).toString() + SLASH;
	...
	private NodePropertyResolver resolverLocationPath = new NodePropertyResolver() {
		...
		public Object get(Node node) {
			final ChildAssociationRef childRef =
	            (ChildAssociationRef)node.getProperties().get(ContentModel.PROP_ARCHIVED_ORIGINAL_PARENT_ASSOC.toString());
	...
	private String buildSearchQuery() {
		...
		if (FILTER_DATE_ALL.equals(property.getDateFilter()) == false) {
			...
			if (fromDate != null) {
				...
				buf.append(AT).append(DATE_ATTR).append(SEMICOLON).append(
						BRAKET).append(strFromDate).append(TO)
						.append(strToDate).append(AND);
					...
					if (archivedPath) {
						ChildAssociationRef childRef = (ChildAssociationRef) node
								...
								.get(
										ContentModel.PROP_ARCHIVED_ORIGINAL_PARENT_ASSOC.toString());
						if (getNodeService().exists(childRef.getParentRef())) {
							buf.append(getDisplayPath(getNodeService().getPath(childRef.getParentRef()), true, 5));
						}
					} else {

...
public class ExternalAccessServlet extends org.alfresco.web.app.servlet.ExternalAccessServlet
{
   ...
   protected void service(final HttpServletRequest req, final HttpServletResponse res) throws ServletException, IOException
   {
      ...
      final PermissionService permissionService = Services.getAlfrescoServiceRegistry(getServletContext()).getPermissionService();
      ...
      if (OUTCOME_DOCDETAILS.equals(outcome) || OUTCOME_BROWSE.equals(outcome) || OUTCOME_USER_DETAILS.equals(outcome))
      {
			...
			if(isDialog)
			{
				...
				final Map<String, String> parameters = Collections.singletonMap("id", nodeRef == null ? "" : nodeRef.getId());
			...
			else if(!isGuest() || permissionService.hasPermission(nodeRef, PermissionService.READ) == AccessStatus.ALLOWED)
			{
				// set the external container session flag so that a plain container gets used
	            fc.getExternalContext().getSessionMap().put(
	                     AlfrescoNavigationHandler.EXTERNAL_CONTAINER_SESSION, Boolean.TRUE);
			...
			final Iterator messages = fc.getMessages();
			if(messages.hasNext())
			{
      ...
      {
   	      ErrorsRenderer.addForcedMessage(new FacesMessage(FacesMessage.SEVERITY_ERROR, Application.getMessage(fc, MSG_BAD_URL), Application.getMessage(fc, MSG_BAD_URL)));
    	  res.sendRedirect(req.getContextPath() + BaseServlet.FACES_SERVLET + CircabcNavigationHandler.WAI_NAVIGATION_CONTAINER_PAGE);
      }
   }
	         ...
	         boolean redirectToLoginPage) throws IOException
   {
	      ...
	      if (status == AuthenticationStatus.Failure && redirectToLoginPage)
	      {
	    	  ...
	    	  if(wasNotGuest)
	    	  {
	    		  final String authAsGuest = Application.getBundle(req.getSession()).getString(AuthenticationFilter.MSG_AUTH_AS_GUEST);
   ...
   {
	   return CircabcConstant.GUEST_AUTHORITY.equals(Beans.getWaiNavigator().getCurrentUser().getUserName());
   }
   ...
   public static String generateRelativeWaiExternalURL(final String outcome, final String args)
   {
      if (args == null)
      {
         return MessageFormat.format(WAI_EXTERNAL_URL, new Object[] {outcome} );
      }
      else
      {
         return MessageFormat.format(WAI_EXTERNAL_URL_ARGS, new Object[] {outcome, args} );
      }
   }

   /**
    * Generate a URL to the External Access Servlet and perefix that with the server address.
    * Allows access to JSF views (via an "outcome" ID) from external URLs.
    *
    * @param outcome
    * @param args

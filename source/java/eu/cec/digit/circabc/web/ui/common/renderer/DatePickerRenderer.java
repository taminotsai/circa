...
public class DatePickerRenderer extends BaseRenderer
{
	 ...
	 *
	 * The decode method takes the parameters from the external requests, finds the ones revelant to this component and decodes the results into an object known as the "submitted value".
	 */
	public void decode(FacesContext context, UIComponent component)
	{
		try
		{
			...
			if (!datePicker.getDisabled()) {
				...
				if (((String) params.get(clientId + FIELD_CMD_TODAY)) != null && ((String) params.get(clientId + FIELD_CMD_TODAY)).length() > 0)
				{
					// Click on today
					datePicker.setValue(new Date());
				} else if (((String) params.get(clientId + FIELD_CMD_NONE)) != null && ((String) params.get(clientId + FIELD_CMD_NONE)).length() > 0) {
					// Click on none - Open version
					datePicker.setValue(null);
				} else if (((String) params.get(clientId + FIELD_CMD_SET)) != null && ((String) params.get(clientId + FIELD_CMD_SET)).length() > 0) {
	 ...
	 *
	 * All rendering logic for this component is implemented here. A renderer for an input component must render the submitted value if it's set, and use the local value only if there is no submitted value.
	 */
	public void encodeBegin(FacesContext context, UIComponent component) throws IOException
	{
		...
		if (component.isRendered() )
		{
			...
			if (date != null)
			{
				...
				if(showDate)
				{
					renderMenu(out, datePicker, getDays(), calendar.get(Calendar.DAY_OF_MONTH), clientId + FIELD_DAY);
					renderMenu(out, datePicker, getMonths(), calendar.get(Calendar.MONTH), clientId + FIELD_MONTH);
					renderMenu(out, datePicker, getYears(datePicker.getStartYear(), datePicker.getYearCount()), calendar.get(Calendar.YEAR), clientId + FIELD_YEAR);
					out.write("&nbsp;");
				}
				else
				{
					renderTimeElement(out, datePicker, calendar.get(Calendar.DAY_OF_MONTH), clientId + FIELD_DAY, false);
					renderTimeElement(out, datePicker, calendar.get(Calendar.MONTH), clientId + FIELD_MONTH, false);
					renderTimeElement(out, datePicker, calendar.get(Calendar.YEAR), clientId + FIELD_YEAR, false);
				}
	 ...
	 * @throws IOException
	 */
	private void renderMenu(ResponseWriter out, UIInputDatePicker component, List<SelectItem> items, int selected, String clientId) throws IOException
	{
	 ...
	 */
	private void renderTimeElement(ResponseWriter out, UIInputDatePicker component, int currentValue, String clientId, boolean showTime) throws IOException
	{
		out.write("<input");
		outputAttribute(out, clientId, HTML.NAME_ATTR);
	...
	{
		List<SelectItem> hours = new ArrayList<SelectItem>(24);
		for (int i = 0; i < 24; i++)
		{
			Integer hour = Integer.valueOf(i);
	...
	private List<SelectItem> getMinutes(int step )
	{
		...
		List<SelectItem> minutes = new ArrayList<SelectItem>( size);
		for (int i = 0; i < size; i++)
		{
			final int currentMinute = i*step;
			minutes.add(new SelectItem(Integer.valueOf(currentMinute),  String.valueOf(currentMinute)));
		}
		return minutes;
	}
}

...
public class UICalendar extends UIComponentBase
{
	...
	private static  final List<String> VALID_VIEW_MODES = Collections.unmodifiableList(Arrays.asList(VIEW_MODE_MONTH, VIEW_MODE_WEEK, VIEW_MODE_WORKWEEK, VIEW_MODE_DAY, VIEW_MODE_AGENDA, VIEW_MODE_TRIMESTER));
	...
	public static final String WEEK_START_TUESDAY = EventModel.WEEK_START_DAY_CONSTRAINT_VALUES.get(2);
    public static final String WEEK_START_WEDNESDAY = EventModel.WEEK_START_DAY_CONSTRAINT_VALUES.get(3);
    public static final String WEEK_START_THURSDAY = EventModel.WEEK_START_DAY_CONSTRAINT_VALUES.get(4);
    public static final String WEEK_START_FRIDAY = EventModel.WEEK_START_DAY_CONSTRAINT_VALUES.get(5);
    public static final String WEEK_START_SATURDAY = EventModel.WEEK_START_DAY_CONSTRAINT_VALUES.get(6);
	...
	private void writeFiveDaysColumns(final FacesContext context, final int dateDiff) throws IOException
    {
    ...
    private void writeOneDay(final FacesContext context, final int dateDiff) throws IOException
    {
        ...
        if (getShowPreviousNextLinks().booleanValue())
        {
            ...
            writer.write(decoratorObject.getWeekdayTitle(startCalendar.get(Calendar.DAY_OF_WEEK)) + HTML.NBSP_ENTITY);
        ...
        else
        {
        	...
        	writer.write("    <td colspan=\"" +  columnNumber + "\" class=\"" + cssPrefix + TITLE_STYLE + "\" >");
        	writer.write(decoratorObject.getCalendarTitle());
        	writer.write("</td>\r\n");
        }
       ...
       TimeOfDay workDayStart = new TimeOfDay(8, 0, 0);
       // the latest time of the calendar
       TimeOfDay endDayStart = new TimeOfDay(19, 0, 0);
       ...
       {
    	   final TimeOfDay firstHour = appointments.get(0).getStart();
    	   // if an appointment is found before 8:00, change the first hour to display
    	   if(firstHour.isBefore(workDayStart))
    	   {
    		   workDayStart = new TimeOfDay(firstHour.getHourOfDay(), 0, 0);
    	   }

    	   TimeOfDay lastHour = null;
    	   TimeOfDay lastHourTemp = null;
       ...
       TimeOfDay currentTime = workDayStart.plusMinutes(0);

       final TimeOfDay upperLimit = new TimeOfDay(0,0);
       while(currentTime.isBefore(endDayStart))
       {
    	   writer.write("  <tr>\r\n");

    	   writer.write("    <th colspan=\"1\" rowspan=\"4\" class=\"" + cssPrefix + decoratorObject.getDayStyleClass(true, true) +  "\" >");
           writer.write(padInt(currentTime.getHourOfDay()) + ":" + padInt(currentTime.getMinuteOfHour()) + HTML.NBSP_ENTITY + HTML.NBSP_ENTITY);
           writer.write("</th>\r\n");
    ...
    private void writeASingleDayQuarter(final FacesContext context, TimeOfDay currentTime, final List<AppointmentUnit> appointments, final int truedayWidth) throws IOException
    {
    	final ResponseWriter writer = context.getResponseWriter();

    	writer.write("    <td colspan=\"6\" class=\"" + cssPrefix + decoratorObject.getDayStyleClass(false, false) +  "\" width=\"" + truedayWidth + "\">");
        ...
        for(AppointmentUnit unit : appointments)
        {
     	   if(unit.isInQuarter(currentTime))
     	   {
     		   ...
     		   String tooltip = MessageFormat.format(
     				   ...
     				   new Object[]{unit.getTitle(), unit.getStartAsString(), unit.getEndAsString()});
    ...
    private void writeSevenDaysColumns(final FacesContext context) throws IOException
    {
        ...
        for (int i = 0; i < columnNumber; i++)
        {
            ...
            writer.write("    <td class=\"" + cssPrefix + WEEKDAY_STYLE + "\">" + decoratorObject.getWeekdayTitle(day) + "</td>\r\n");
        }
        writer.write("  </tr>\r\n");
        ...
        if(getBeyond())
        {
        	...
        	int endEmptyDay = (columnNumber - 1) - ((endCalendar.get(Calendar.DAY_OF_WEEK) - calendar.getFirstDayOfWeek()) % columnNumber);
        ...
        {
        	writer.write("    <td class=\"" + cssPrefix + EMPTY_DAY_STYLE + "\" height=\"" + dayHeight + "\" width=\"" + truedayHeight + "\">");
        	if(getBeyond())
        	{
        		...
        		currentDayAction = buildActionLinkForDayEvents(context, convert(emptyDay.getTime()), decoratorObject.getEmptyDay(), eventRootId);
        ...
        while (!iteratingDate.after(endCalendar))
        {
            ...
            writer.write("    <td "+  " class=\"" + cssPrefix + decoratorObject.getDayStyleClass(isOddMonth, CalendarTagUtil.isSameDay(calendar, iteratingDate))
                        + "\" height=\"" + truedayHeight + "\" width=\"" + truedayWidth + "\">");

            currentDayAction = buildActionLinkForDayEvents(context, convert(iteratingDate.getTime()), decoratorObject.getDay(null), eventRootId);
            Utils.encodeRecursive(context, currentDayAction);
        ...
        {
        	writer.write("    <td class=\"" + getCssPrefix() + EMPTY_DAY_STYLE + "\" height=\"" + truedayHeight + "\" width=\"" + truedayWidth + "\">");
    ...
    protected void checkDate()
    {
    	...
    	if(startCalendar == null)
    	{
    		...
    		{
    			startCalendar.set(Calendar.MONTH, getStartMonthOfTrimester(startCalendar.get(Calendar.MONTH)));
    			startCalendar.set(Calendar.DAY_OF_MONTH, 1);
    		}
    	}
    	if(endCalendar == null)
    ...
    private void writeAppoitments(final FacesContext context, final List<AppointmentUnit> appointments, final ResponseWriter writer) throws IOException
    {
    ...
    private UIActionLink buildActionLinkForDayEvents(final FacesContext context, final String date, final String value, final String valueIdParam)
    {
    	final String tooltip = MessageFormat.format(Application.getMessage(context, MSG_SELECTED_DATE_TOOLTIP), date);
    	final UIActionLink actionLink = buildActionLink(context, getAction(), getActionListener(), value, tooltip, valueIdParam);
    ...
    private UIActionLink buildActionLinkForSingleAppointement(final FacesContext context, final String title, final String tooltip, final AppointmentUnit appointment)
    {
    	final UIActionLink actionLink = buildActionLink(context, getAction(), getActionListener(), title, tooltip, String.valueOf(appointment.getId()));

    	return actionLink;
    }

    private UIActionLink buildActionLink(final FacesContext context, final String action, final String actionListener, final String value, final String tooltip, final String valueIdParam)
    {
    	...
    	{
    		actionLink.setActionListener(context.getApplication().createMethodBinding(actionListener, ACTION_CLASS_ARGS));
    	}
    ...
    @SuppressWarnings("unchecked")
	private void addParam(final FacesContext context, UIActionLink actionLink, final String paramName, final String paramValue)
    {
    	final javax.faces.application.Application facesApp = context.getApplication();

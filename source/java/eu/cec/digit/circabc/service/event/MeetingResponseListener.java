...
public class MeetingResponseListener implements Job
{
	...
	private void initialize(final JobExecutionContext context)
	{
		if (!isInitialized)
		{
				...
				emailServerPort = Integer.valueOf(CircabcConfiguration.getProperty(CircabcConfiguration.EMAIL_SERVER_PORT));
				isActive = Boolean.valueOf(CircabcConfiguration.getProperty(CircabcConfiguration.IS_EMAIL_LISTENER_ACTIVE));
	...
	private void checkEmail()
	{
		...
		try
		{
			final URLName url = new URLName(emailProtocol, emailServer, emailServerPort, emailBox, emailUsername, emailPassword);
			...
			if(store.isConnected()) {
				...
				for (final Message message : folder.getMessages())
				{
					...
					if (ct.getPrimaryType().equals("multipart") ||  ( ct.getPrimaryType().equals("text") && ct.getSubType().equals("calendar") ) )
					{
	...
	private void processMessage(final POP3Message message) throws Exception
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			isResponse = body.contains(BEGIN_VCALENDAR) && body.contains(BEGIN_VEVENT) && body.contains(METHOD_REPLY);

			if (isResponse)
			{
				meetingRequestStatus = getMeetingRequestStatus(
						meetingRequestStatus, body);
				...
				if (startPositionRecurrence > -1)
				{
					...
					recurrenceLine = getRecurrenceLine(body,
							startPositionRecurrence);
					meetingNodeRef = getMeetingNodeRefByRecurrenceLine(meetingNodeRef,
							transactionService, recurrenceLine);
				}
		...
		if (isResponse)
		{
			updateMeetingRequestStatus(meetingNodeRef, meetingRequestStatus,
					mode, userName, transactionService);
	...
	private NodeRef getMeetingNodeRefByRecurrenceLine(NodeRef meetingNodeRef,
			final TransactionService transactionService, String recurrenceLine)
	...
	private String getRecurrenceLine(String body,
	...
	private void updateMeetingRequestStatus(NodeRef meetingNodeRef,
			MeetingRequestStatus meetingRequestStatus, UpdateMode mode,
			String userName, final TransactionService transactionService)
			throws SystemException {
		UserTransaction tx = null;
		try
		{
	...
	private String getBodyFromString(final POP3Message message,
			final Object messageContent) throws IOException,
			MessagingException, NoSuchFieldException, IllegalAccessException {
		String body;

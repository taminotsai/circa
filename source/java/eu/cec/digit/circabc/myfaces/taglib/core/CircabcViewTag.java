...
/**
 * Circabc ViewTag implementation for use when views are defined via tags in JSP pages.
 * We need other action when a state is not found in session
 * Clone from orignal but with the attend behavior (why original is not good ????)
 *
 * @author Guillaume
 */
	...
	public int doAfterBody() throws JspException
	{
		...
		try
		{
			...
			if (bodyContent != null)
			{
				...
				final StateManager.SerializedView serializedView = stateManager.saveSerializedView(facesContext);
				...
				final String viewState = StateUtils.construct(savedState, facesContext.getExternalContext());
		...
		if (_locale != null)
		{
			...
			if (UIComponentTag.isValueReference(_locale))
			{
				...
				} else
				{
					if (localeValue != null)
					{
						throw new IllegalArgumentException("Locale or String class expected. Expression: " + _locale + ". Return class: " + localeValue.getClass().getName());
					} else
					{
						throw new IllegalArgumentException("Locale or String class expected. Expression: " + _locale + ". Return value null");
					}
				}
			} else
			{
			...
			Config.set((ServletRequest) getFacesContext().getExternalContext().getRequest(), Config.FMT_LOCALE, locale);
		}
	}
}

...
public class LinksBusinessImpl implements LinksBusinessSrv
{
    ...
    public NodeRef createLink(final NodeRef parent, final NodeRef target)
    {
        ...
        final String title = LINK_TO_PREFIX + getNodeService().getProperty(target, ContentModel.PROP_TITLE);
        final ChildAssociationRef assocRef = getNodeService().getPrimaryParent(target);

        final Map<QName, Serializable> nodeProps = new HashMap<QName, Serializable>(2);
     ...
     */
    public NodeRef createSharedSpaceLink(final NodeRef parent, final NodeRef targetSharedSpace, final String title, final String description)
    {
     ...
     */
    public void applySharing(final NodeRef sharedSpace, final NodeRef interestGroup, final LibraryPermissions libraryPermission)
    {
    	AssertUtils.notNull(libraryPermission);

    	assertInvitation(sharedSpace, interestGroup);

    	getShareSpaceService().inviteInterestGroup(sharedSpace, interestGroup, libraryPermission.toString());
    }
    ...
    public List<ShareSpaceItem> getAvailableSharedSpaces(final NodeRef nodeRef)
    {
        ...
        if(allSharedSpaces == null || allSharedSpaces.size() < 1)
        {
            ...
            return Collections.<ShareSpaceItem>emptyList();
        }
        else
        {
    ...
    */
    public List<ShareSpaceItem> findSharedSpaces(final NodeRef nodeRef)
    {
        ...
        final List<NodeRef> allSharedSpaces = getShareSpaceService().getAllSharedSpaceInInterestGroup(nodeRef);
    ...
    public List<InterestGroupItem> getInterestGroupForSharing(final NodeRef nodeRef)
    {
        ...
        if(interestGroups == null || interestGroups.size() < 1)
        {
             ...
             return Collections.<InterestGroupItem>emptyList();
         }
         else
         {
        	 final List<InterestGroupItem> items = new ArrayList<InterestGroupItem>(interestGroups.size());
    ...
    public List<InterestGroupItem> getInvitationsForSharing(final NodeRef nodeRef)
    {
         ...
         final List<Pair<NodeRef, String>> interestGroups = getShareSpaceService().getInvitedInterestGroups(nodeRef);
    ...
    */
	...
	{
		final List<ShareSpaceItem> wrappers = new ArrayList<ShareSpaceItem>(allSharedSpaces.size());
    ...
    private boolean checkIfOneInvitedInterestGroupExists(NodeRef shareSpace) {
		...
		{
			List<ChildAssociationRef> childAssocs = getNodeService().getChildAssocs(shareSpace, SharedSpaceModel.ASSOC_SHARE_SPACE_CONTAINER, RegexQNamePattern.MATCH_ALL);
			final ChildAssociationRef assocRef;
			if (childAssocs.size() > 0)
			{
				assocRef = childAssocs.get(0);
				NodeRef container = assocRef.getChildRef();
				List<ChildAssociationRef> igChildAssocs = getNodeService().getChildAssocs(container, SharedSpaceModel.ASSOC_ITEREST_GROUP, RegexQNamePattern.MATCH_ALL);
				for (ChildAssociationRef ref : igChildAssocs)
				{
					NodeRef childRef = ref.getChildRef();
					NodeRef igNodeRef = (NodeRef) getNodeService().getProperty(childRef, SharedSpaceModel.PROP_INTEREST_GROUP_NODE_REF);
	...
	private void applyUIFacet(final NodeRef ref, final String title, final String desc, final String icon)
    {
    ...
    private void assertLinkCreation(final NodeRef parent, final NodeRef target, final String title, final boolean isSharedSpace)
    {
    ...
    private InterestGroupItem buildInterestGroupItem(final NodeRef interestGroup, final String permission)
	{
    ...
    private String getLinkName(final NodeRef parent, final NodeRef target)
    {
        ...
        return getMetadataManager().getValidUniqueName(parent, LINK_TO_PREFIX + targetName + LINK_NODE_EXTENSION);
    }

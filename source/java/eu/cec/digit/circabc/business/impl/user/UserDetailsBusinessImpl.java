...
public class UserDetailsBusinessImpl implements UserDetailsBusinessSrv
{
    ...
    public void updateAvatar(final NodeRef person, final NodeRef imageRef)
    {
        ...
        updateAvatar(person, (String) nodeService.getProperty(imageRef, ContentModel.PROP_NAME), null, imageRef);
    }
     ...
     */
    public NodeRef updateAvatar(final NodeRef person, final String avatarFileName, final File avatarFile)
    {
        final BusinessStackError stack = new BusinessStackError();
        validationManager.validateNodeRef(person, stack);
        validationManager.validatePerson(person, stack);
    ...
    public NodeRef getDefaultAvatar()
    {
        if(defaultAvatar == null)
        {
            ...
            try
            {
                defaultAvatar = nodePreferencesService.getDefaultConfigurationFile(rootRef, AVATAR_CONFIG_ROOT, AVATAR_CONFIG_TYPE, AVATAR_CONFIG_ELEMENT);
            }
            ...
            {
                logger.warn("Impossible to found the default avatar. Check the presence of a file in the location: $circabc dictionary folder$/" + AVATAR_CONFIG_ROOT + "/" + AVATAR_CONFIG_TYPE + "/" + AVATAR_CONFIG_ELEMENT);
            }
        }
    ...
    public void updateUserDetails(final NodeRef personRef, final UserDetails userDetails)
    {
        ...
        {
            throw new IllegalArgumentException("User details node reference (" + userDetails.getNodeRef() + ") must be equals to the person reference (" + personRef + ")");
        }
        ...
        if(propertiesToUpdate != null && propertiesToUpdate.size() > 0)
        {
            ...
            {
                logger.debug(propertiesToUpdate.size() + " properties setted for user: " + userDetails.getUserName());
            }
        }
        ...
        if(preferencesToUpdate != null && preferencesToUpdate.size() > 0)
        {
            ...
            if(logger.isDebugEnabled())
            {
                logger.debug(preferencesToUpdate.size() + " preferences setted for user: " + userDetails.getUserName());
            }
        }
        else if(logger.isDebugEnabled())
        {
    ...
    /**
     * Return the preferenences of a person in a separate RO transaction (used for UserDEtails lazy loading)
     *
     * @param person
     * @return
     */
    ...
    /**
     * Return the properties of a person in a separate RO transaction (used for UserDEtails lazy loading)
     *
     * @param person
     * @return
     */
    ...
    private NodeRef updateAvatar(final NodeRef person, final String avatarFileName, final File avatarFile, final NodeRef imageRef)
    {
        ...
        if(imageRef == null)
        {
            imageToLink = contentManager.createContent(person, avatarFileName,  ContentModel.ASSOC_PREFERENCE_IMAGE,  ContentModel.TYPE_CONTENT, avatarFile, false);
        }
        else
        {
        ...
        permissionService.setPermission(imageToLink, CircabcConstant.GUEST_AUTHORITY, PermissionService.READ, true);
    ...
    {
        final List<AssociationRef> targetAssoc = nodeService.getTargetAssocs(person, ContentModel.ASSOC_AVATAR);

        if(logger.isWarnEnabled() && targetAssoc != null && targetAssoc.size() > 1)
        {
            logger.warn("Business model inconsistency: To many avatars found (" + targetAssoc.size() + ") for user " + nodeService.getProperty(person, ContentModel.PROP_USERNAME));
        }
    ...
    {
        final List<ChildAssociationRef> childAssocs = nodeService.getChildAssocs(person, RegexQNamePattern.MATCH_ALL, ContentModel.ASSOC_PREFERENCE_IMAGE);

        if(logger.isWarnEnabled() && childAssocs != null && childAssocs.size() > 1)
        {
            logger.warn("Business model inconsistency: To many prefered images found (" + childAssocs.size() + ") for user " + nodeService.getProperty(person, ContentModel.PROP_USERNAME));
        }
        return childAssocs;
    }

    private void removeChildAssociations(final NodeRef person, final List<ChildAssociationRef> assocs)
    {
        if (assocs != null)
        {

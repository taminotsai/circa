...
public class CociContentBusinessImpl implements CociContentBusinessSrv
{
	...
	public NodeRef checkOut(final NodeRef nodeRef)
	{
        ...
        {
        	logger.debug("Node successfully locked and checked out. Created working copy: " + workingCopyRef);
        }
	...
	public NodeRef checkOutForWorkflow(final NodeRef nodeRef, final String workflowTaskId)
	{
        ...
        if (task != null)
        {
            ...
            {
                final String wokingCopyName = (String) getNodeService().getProperty(workingCopyRef, ContentModel.PROP_NAME);
				final QName validQname = QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, QName.createValidLocalName(wokingCopyName));

				getNodeService().addChild(workflowPackage, workingCopyRef, ContentModel.ASSOC_CONTAINS, validQname);
            ...
            {
            	logger.error("No workflow package found for the task " + workflowTaskId + ". Impossible to link the working copy to the workflow.");
            }
        }
        else
        {
	...
	public NodeRef checkIn(final NodeRef workingCopy, final boolean minor, final String versionNote, boolean keepCheckOut)
	{
        ...
        final Map<String, Serializable> props = new HashMap<String, Serializable>(1, 1.0f);
        ...
        final ContentData contentData = (ContentData) getNodeService().getProperty(workingCopy, ContentModel.PROP_CONTENT);
        final String contentUrl = (contentData == null ? null : contentData.getContentUrl());

        // perform the checkin
        final NodeRef original = getCheckOutCheckInService().checkin(workingCopy, props, contentUrl, keepCheckOut);
	...
	public NodeRef checkIn(final NodeRef workingCopy, final boolean minor, final String versionNote, boolean keepCheckOut, final File file, boolean disableNotification,String uploadedfilename)
	{
    	...
    	try
    	{
	    	...
	    	{
	    		wasEnable = !policyBehaviourFilter.disableBehaviour(ContentNotifyAspect.ASPECT_CONTENT_NOTIFY);
	    	}
	 ...
	 */
	public void update(final NodeRef document, final File file, final boolean disableNotification,final String uploadedfilename)
	{
		boolean wasEnable = false;
    	try
    	{
	...
	public void update(final NodeRef document, final File file,final String uploadedfilename)
	{
        ...
        final ContentWriter writer = getContentService().getWriter(document, ContentModel.PROP_CONTENT, true);
	...
	public NodeRef getWorkingCopyOf(final NodeRef workingCopyRef)
	{
		...
		final NodeRef lockedRef = (NodeRef) getNodeService().getProperty(workingCopyRef, ContentModel.PROP_COPY_REFERENCE);

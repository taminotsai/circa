...
/**
 * Concrete implementation of the Notification Subscription Service. The status is stored as a permission.
 *
...
public class NotificationSubscriptionServiceImpl implements NotificationSubscriptionService
{
	...
	/** The notification status defined by default for any authority in any node under an Interest Group */
	...
	public Set<AuthorityNotification> getNotifications(final NodeRef nodeRef)
	{
		...
		final Set<AuthorityNotification> notifications = new HashSet<AuthorityNotification>(permissions.size());
		...
		if(logger.isDebugEnabled())
		{
			logger.debug(notifications.size() + " Notification(s) status successfully retrived for the node " + nodeRef
					+ "\n\tName:                      " + nodeService.getProperty(nodeRef, ContentModel.PROP_NAME)
					+ "\n\tNotification status found: " + notifications) ;
		}
	...
	public void setNotificationStatus(final NodeRef nodeRef, final String authority, final NotificationStatus status)
	{
		...
		{
			throw new IllegalStateException("The notification can only be setted to a Profile or to a user. Authority " + authority + " from type " + type + " is not allowed.");
		}

		if(status.equals(NotificationStatus.INHERITED) && nodeService.hasAspect(nodeRef, CircabcModel.ASPECT_IGROOT))
		{
			throw new IllegalStateException("The INHERITED notification status is not allowed for an Interest Group. Please select either SUSCRIBE or UNSUSCRIBE ");
		}
	...
	public AuthorityNotification getAuthorityNotificationStatus(final NodeRef nodeRef, final String authority) throws InvalidNodeRefException
	{
		...
		if(logger.isDebugEnabled())
		{
			logger.debug("Trying to retive the notification status of the authority " + authority + " for the node " + nodeRef + " ...") ;
		}


		final NodeRef interestGroup = getManagementService().getCurrentInterestGroup(nodeRef);

		final NotificationStatus notificationStatus = getAuthorityNotificationStatusImpl(nodeRef, interestGroup, authority);

		final AuthorityNotification authorityNotification = new AuthorityNotificationImpl(
				notificationStatus,
				authority);

		if(logger.isDebugEnabled())
		{
			logger.debug("Notification status successfully retrived for the authority " + authority
					+ "\n\tOn node: " + nodeRef
					+ "\n\tResult :	" + authorityNotification);
		}
	...
	public void removeNotification(NodeRef nodeRef, String authority)
	{
		...
		if(logger.isDebugEnabled())
		{
			logger.debug("Trying to remove the notification settings of the authority " + authority + " on the node " + nodeRef  + "...") ;
		}
		...
		if(logger.isDebugEnabled())
		{
			logger.debug("Notification settings successfully remove for the authority " + authority + " on the node " + nodeRef  + "...") ;
		}
	}
	...
	public UserNotificationReport getUserNotificationReport(final NodeRef nodeRef, final String userAuthority)
	{
		...
		if(!AuthorityType.getAuthorityType(userAuthority).equals(AuthorityType.USER) || !personService.personExists(userAuthority))
		{
			if(logger.isDebugEnabled())
			{
				logger.debug("No User Notification Report found for " + userAuthority + " on " + nodeRef + ". Reason: the user is be an right user (can't be ADMIN or GUEST)");
			}
		...
		if(igNoderef == null)
		{
			if(logger.isDebugEnabled())
			{
				logger.debug("No User Notification Report found for " + userAuthority + " on " + nodeRef + ". Reason: nodeRef not a Interest Group child");
			}
		...
		final UserNotificationReport report = getReportUserProfileImpl(nodeRef, igNoderef, userAuthority, profile) ;
	...
	public Set<NotifiableUser> getNotifiableUsers(final NodeRef nodeRef) throws InvalidNodeRefException
	{
		...
		if(logger.isDebugEnabled())
		{
			logger.debug("Trying to retive the notifiable users settest in the interest group " + igNoderef + " of the node " + nodeRef  + "...") ;
		}


		final Map<String, LeveledNotificationStatus> userNotif = new HashMap<String, LeveledNotificationStatus>(10);
		final Map<String, LeveledNotificationStatus> profileNotif = new HashMap<String, LeveledNotificationStatus>(80);
		...
		// with the profile define notifications, enhance the use map. At the same level, the user entry is
		...
		for(final Map.Entry<String, LeveledNotificationStatus> entry : profileNotif.entrySet())
		{
			...
			usersInProfile = getIGProfileManagerService().getPersonInProfile(igNoderef, profile.getProfileName());
			for (final String user : usersInProfile)
			{
				userLeveledNotificationStatus = userNotif.get(user);
				if(userLeveledNotificationStatus == null || (userLeveledNotificationStatus.level > entry.getValue().level))
				{
		...
		// the last step is to return the users that are SUSCRIBE and where the Global Notification properties is set as true
		final Set<NotifiableUser> usersToNotify = new HashSet<NotifiableUser>(userNotif.size());
		...
		boolean isMultilingualDocument = nodeService.hasAspect(nodeRef,ContentModel.ASPECT_MULTILINGUAL_DOCUMENT);
		...
		for(final Map.Entry<String, LeveledNotificationStatus> entry : userNotif.entrySet())
		{
			...
			if(NotificationStatus.SUBSCRIBED.equals(userLeveledNotificationStatus.status))
			{
				...
				if(globalNotif != null && globalNotif.booleanValue() == true)
				{
					final Boolean hasReadPermision = (Boolean) AuthenticationUtil.runAs(new AuthenticationUtil.RunAsWork<Object>()
							{
								public Object doWork()
								{
									return  getPermissionService().hasPermission(nodeRef, PermissionService.READ_PROPERTIES).equals(AccessStatus.ALLOWED)  ;
								}
							}, authority);
					
					if (hasReadPermision)
					{
						// for instance, the interface language can be setted either under a String or Locale
						langObject = getUserService().getPreference(person, UserService.PREF_INTERFACE_LANGUAGE);
						locale = getLocale(langObject);
						if (isMultilingualDocument)
						{
							
							contentLanguageFilter = getUserService().getPreference(person, UserService.PREF_CONTENT_FILTER_LANGUAGE);
							...
							boolean isDocumentAndContentLanguageSame = documentLanguage.equalsIgnoreCase(contentLanguage);
							boolean isDocumentAndPivitLanguageSame = documentLanguage.equalsIgnoreCase(pivotLanguage);
							// notify user if content filter language is all or if it is content filter language or pivot language
							if (contentLocale == null ||  isDocumentAndContentLanguageSame  || isDocumentAndPivitLanguageSame )
							{
	...
	private void fillNotifiableAuthoritiesImpl(final NodeRef from, final NodeRef to, int level, final Map<String, LeveledNotificationStatus> userNotif, final Map<String, LeveledNotificationStatus> profileNotif)
	{
		if(from == null)
		{
			return;
		...
		final Set<AccessPermission> permissions = getPermissionService().getAllSetPermissions(from);
		for (final AccessPermission permission : permissions)
		{
			if(isPermissionNotificationStatus(permission.getPermission()))
			{
				authority = permission.getAuthority();
				type = AuthorityType.getAuthorityType(authority);
				status = computeAccessPermission(permission.getAccessStatus());

				if(AuthorityType.GROUP.equals(type))
				{
					if(!profileNotif.containsKey(authority))
					{
				...
				{
					if(!userNotif.containsKey(authority))
					{
	...
	private NotificationStatus getAuthorityNotificationStatusImpl(final NodeRef fromRef, final NodeRef toRef, final String authority) throws InvalidNodeRefException
	{
	...
	private UserNotificationReport getReportUserProfileImpl(final NodeRef nodeRef, final NodeRef igRef, final String user, final String profile)
	{
		...
		final Boolean globalNotification = (Boolean) getNodeService().getProperty(person, UserModel.PROP_GLOBAL_NOTIFICATION);

		Boolean willUserReceive = null;

		if(globalNotification != null && globalNotification.booleanValue() == true)
		{
		...
		return new UserNotificationReportImpl(
								nodeRef, user, profile,
								globalNotification,
								statuses.userStatus,
								statuses.profileStatus,
								willUserReceive.booleanValue()
						);
	}
	...
	private static class UserProfileStatuses
	{
		...
		public UserProfileStatuses(final NotificationStatus userStatus, final NotificationStatus profileStatus)
		{
			super();
	...
	private UserProfileStatuses fillStatuses(final NodeRef nodeRef, boolean topLevelNodeRef , final String user, final String profile)
	{
	...
	private NotificationStatus getDefinedAuthorityStatus(final NodeRef nodeRef, final String authority)
	{
		NotificationStatus status = null;
		...
		for (AccessPermission permission : permissions)
		{
			if(permission.getAuthority().equals(authority) && permission.getPermission().equals(NOTIFICATION_AS_PERMISSION))
			{
				status = computeAccessPermission(permission.getAccessStatus());
				break;
			}
	...
	private NodeRef checkAndReturnIG(final NodeRef nodeRef) throws InvalidNodeRefException
	{
		...
		{
			throw new InvalidNodeRefException("The node ref is a mandatory parameter and must be existing.", nodeRef);
		}
		...
		if(igNoderef == null )
		{
			throw new InvalidNodeRefException("The node received as parameter is not an Interest Group child.", nodeRef);
		}

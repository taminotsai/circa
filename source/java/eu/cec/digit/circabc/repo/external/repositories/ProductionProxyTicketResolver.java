...
/**
 * A more intelligent implementation of the proxy ticket resolver that returns
 * the ECAS proxy that has to be validated for the production environment.
 * 
 * @author schwerr
 */
public class ProductionProxyTicketResolver implements ProxyTicketResolver, ServletContextAware {
	...
	public String getProxyTicket() {
		...
		if (attributes == null) {
			logger.error("Request context attributes " +
		...
		return getProxyTicketIntern(servletContext, ((ServletRequestAttributes) 
	...
	private String getProxyTicketIntern(ServletContext context, HttpServletRequest request) {
		...
		try {
			ProxyTicketHelperIntf proxyTicketHelper = 
									ProxyTicketHelper.getInstance(context);

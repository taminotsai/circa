...
/**
 * Implementation of the Main circabc management service. It serves to manage the particular structure of primary Circabc Nodes
 *
...
public class ManagementServiceImpl implements ManagementService
{
	...
	private static final MLText CIRCABC_ADMIN_TITLE = new MLText(Locale.ENGLISH, "Circabc Administrator");
    ...
    public SimplePath getNodePath(final NodeRef nodeRef) throws PathNotFoundException
    {
    	...
    	else if(EventModel.TYPE_EVENT.equals(type))
    	{
    		...
    		final ChildAssociationRef calendarService = nodeService.getPrimaryParent(parentparent.getParentRef());
    ...
    public List<SimplePath> getChildsPath(final NodeRef nodeRef) throws PathNotFoundException {
    	final List<SimplePath> list = new ArrayList<SimplePath>();
	...
	public SimplePath getNodePath(final String pathString) throws PathNotFoundException
	{
		...
		{
			throw new PathNotFoundException("The given path is not managed by circabc. The path must start with " + getRootSimplePath().toString());
		}

		final StringTokenizer tokens = new StringTokenizer(pathString, String.valueOf(SimplePath.PATH_SEPARATOR), false);
		...
		while(tokens.hasMoreTokens())
		{
			...
			if(iterNodeRef == null)
			{
				throw new PathNotFoundException("The path is invalid. Impossible to found the path element '" + iterName + "' found at the position: "  + iterationConter);
			}
		}
	...
	public List<NodeRef> getAllParents(final NodeRef initialNodeRef, final boolean includeContets, QName untilAspect, final boolean includeUntilAspect)
	{
		...
		final List<NodeRef> result = new LinkedList<NodeRef>();
		...
		for(;;)
		{
			...
			if(!includeContets)
			{
				...
				if (typeDef != null)
				{
					...
					if (ContentModel.TYPE_CONTENT.equals(type) || this.getDictionaryService().isSubClass(type, ContentModel.TYPE_CONTENT))
					{
			...
			{
				if(includeUntilAspect && getPermissionService().hasPermission(nodeRef, PermissionService.READ).equals(AccessStatus.ALLOWED))
				{
			...
			{
				if(getPermissionService().hasPermission(nodeRef, PermissionService.READ).equals(AccessStatus.ALLOWED))
				{
					((LinkedList<NodeRef>)result).addFirst(nodeRef);
				}
	...
	public NodeRef createCircabc(final NodeRef parentNodeRef) {
		...
		// Create circaBCNode
		final NodeRef circabcNodeRef = createFolder(parentNodeRef,
				getCircaBCFolderName());

		// add CircaBC aspect
		final Map<QName, Serializable> circaBCProps = new HashMap<QName, Serializable>();

		nodeService.addAspect(circabcNodeRef, CircabcModel.ASPECT_CIRCABC_ROOT,
				circaBCProps);

		if (logger.isDebugEnabled()) {
			logger.debug("Add:" + CircabcModel.ASPECT_CIRCABC_ROOT
					+ " to the node:" + circabcNodeRef);
		}
	...
	public NodeRef createCategory(final NodeRef parentNodeRef,
		...
		final NodeRef circabcCategoryNodeRef = createFolder(parentNodeRef,
				folderName);

		// add CircaBC aspect
		final Map<QName, Serializable> circabcCategoryProps = new HashMap<QName, Serializable>();

		nodeService
				.addAspect(circabcCategoryNodeRef,
						CircabcModel.ASPECT_CATEGORY,
						circabcCategoryProps);

		if (logger.isDebugEnabled()) {
			logger.debug("Add:" + CircabcModel.ASPECT_CATEGORY
					+ " to the node:" + circabcCategoryNodeRef);
		}
		...
		final ArrayList<NodeRef> categories = new ArrayList<NodeRef>();
			...
			final String folderName, final String circabcIGRootContact, final boolean createOptionalProfiles)
	{
		...
		// add CircaBC aspect
		final Map<QName, Serializable> circabcIGRootProps = new HashMap<QName, Serializable>();

		/** Circabc contact for interest group leader */
		circabcIGRootProps.put(getIGRootProfileManagerService()
				.getContactParamName(), circabcIGRootContact);
		nodeService.addAspect(circabcIGRootNodeRef,
				CircabcModel.ASPECT_IGROOT, circabcIGRootProps);

		if (logger.isDebugEnabled()) {
			logger.debug("Add:" + CircabcModel.ASPECT_IGROOT
					+ " to the node:" + circabcIGRootNodeRef);
		}
	...
	public NodeRef createLibrary(final NodeRef parentNodeRef) {
		...
		final String circabcLibraryDescription = CircabcConfiguration.getProperty(CircabcConfiguration.LIBRARY_NODE_DESCRIPTION_PROPERTIES);
		final NodeRef circabcLibraryNodeRef = createFolder(parentNodeRef,
				CircabcConfiguration.getProperty(CircabcConfiguration.LIBRARY_NODE_NAME_PROPERTIES));
		// add CircaBC aspect
		final Map<QName, Serializable> circabcLibraryRootProps = new HashMap<QName, Serializable>();

		circabcLibraryRootProps.put(ContentModel.PROP_DESCRIPTION, circabcLibraryDescription);
		nodeService.addAspect(circabcLibraryNodeRef,
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Add:"
					+ CircabcModel.ASPECT_LIBRARY_ROOT
					+ " to the node:" + circabcLibraryNodeRef);
		}
	...
	public NodeRef createNewsGroup(final NodeRef parentNodeRef) {
		final String circabcNewsGroupDescription = CircabcConfiguration.getProperty(CircabcConfiguration.NEWSGROUP_NODE_DESCRIPTION_PROPERTIES);
		final NodeRef circabcNewsGroupNodeRef = createNewsGroup(parentNodeRef,
				CircabcConfiguration.getProperty(CircabcConfiguration.NEWSGROUP_NODE_NAME_PROPERTIES));
		// add CircaBC aspect
		final Map<QName, Serializable> circabcNewsGroupProps = new HashMap<QName, Serializable>();

		circabcNewsGroupProps.put(ContentModel.PROP_DESCRIPTION, circabcNewsGroupDescription);
		nodeService.addAspect(circabcNewsGroupNodeRef,
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Add:"
					+ CircabcModel.ASPECT_NEWSGROUP_ROOT
					+ " to the node:" + circabcNewsGroupNodeRef);
		}
		setDefaultNewsGroupRules(circabcNewsGroupNodeRef);
		setCircabcNewsGroupDefaultSettings(parentNodeRef,
	...
	public NodeRef createSurvey(final NodeRef parentNodeRef) {
		final String circabcSurveyDescription = CircabcConfiguration.getProperty(CircabcConfiguration.SURVEY_NODE_DESCRIPTION_PROPERTIES);
		final NodeRef circabcSurveyNodeRef = createSurvey(parentNodeRef,
				CircabcConfiguration.getProperty(CircabcConfiguration.SURVEY_NODE_NAME_PROPERTIES));
		// add CircaBC aspect
		final Map<QName, Serializable> circabcSurveyProps = new HashMap<QName, Serializable>();

		circabcSurveyProps.put(ContentModel.PROP_DESCRIPTION, circabcSurveyDescription);

		nodeService.addAspect(circabcSurveyNodeRef,
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Add:"
					+ CircabcModel.ASPECT_SURVEY_ROOT
					+ " to the node:" + circabcSurveyNodeRef);
		}
	...
	public NodeRef createDirectory(final NodeRef parentNodeRef)
	{
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Add:"
					+ CircabcModel.TYPE_DIRECTORY_SERVICE
					+ " to the node:" + dirContainerNodeRef + " with name " + nodeService.getProperty(parentNodeRef, ContentModel.PROP_NAME));
		}
	...
	{
		final String circabcEventDescription = CircabcConfiguration.getProperty(CircabcConfiguration.EVENT_NODE_DESCRIPTION_PROPERTIES);
		final NodeRef circabcEventNodeRef = createFolder(parentNodeRef,
				CircabcConfiguration.getProperty(CircabcConfiguration.EVENT_NODE_NAME_PROPERTIES));
		// add Event aspect
		final Map<QName, Serializable> circabcEventProps = new HashMap<QName, Serializable>();

		circabcEventProps.put(ContentModel.PROP_DESCRIPTION, circabcEventDescription);
		nodeService.addAspect(circabcEventNodeRef,
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Add:"
					+ CircabcModel.ASPECT_EVENT_ROOT
					+ " to the node:" + circabcEventNodeRef);
		}
	...
	{
		final String circabcInformationDescription = CircabcConfiguration.getProperty(CircabcConfiguration.INFORMATION_NODE_DESCRIPTION_PROPERTIES);
		final NodeRef circabcInformationNodeRef = createFolder(parentNodeRef,
				CircabcConfiguration.getProperty(CircabcConfiguration.INFORMATION_NODE_NAME_PROPERTIES));
		// add CircaBC aspect
		final Map<QName, Serializable> circabcInformationProps = new HashMap<QName, Serializable>();

		circabcInformationProps.put(ContentModel.PROP_DESCRIPTION, circabcInformationDescription);
		nodeService.addAspect(circabcInformationNodeRef,
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Add:"
					+ CircabcModel.ASPECT_INFORMATION_ROOT
					+ " to the node:" + circabcInformationNodeRef);
		}
	 ...
	 */
	private NodeRef createFolder(final NodeRef parentNodeRef,
			final String folderName) {
		NodeRef newNodeRef = null;
		if (logger.isDebugEnabled()) {
			logger.debug("create new Folder:" + folderName + " on parentNode: "
					+ parentNodeRef);
		}
		try {
			final FileInfo fileInfo = fileFolderService.create(parentNodeRef,
					folderName, ContentModel.TYPE_FOLDER);
			newNodeRef = fileInfo.getNodeRef();
		} catch (final RuntimeException e)
		{
			if(logger.isErrorEnabled())
				logger.error("Impossible to create the folder '" + folderName + "' under the node " + parentNodeRef, e);
	 ...
	 * @return the FileInfo of the new Folder
	 */
	private NodeRef createNewsGroup(final NodeRef parentNodeRef,
			final String newsGroupName) {
		NodeRef newNodeRef = null;
		if (logger.isDebugEnabled()) {
			logger.debug("create new Folder:" + newsGroupName
					+ " on parentNode: " + parentNodeRef);
		}
		try {
			final FileInfo fileInfo = fileFolderService.create(parentNodeRef,
					newsGroupName, ForumModel.TYPE_FORUMS);
			newNodeRef = fileInfo.getNodeRef();
		}
		catch (final RuntimeException e)
		{
	 ...
	 * @return the FileInfo of the new Folder
	 */
	private NodeRef createSurvey(final NodeRef parentNodeRef,
			final String surveyName) {
		NodeRef newNodeRef = null;
		if (logger.isDebugEnabled()) {
			logger.debug("create new Folder:" + surveyName + " on parentNode: "
					+ parentNodeRef);
		}
		try {
			final FileInfo fileInfo = fileFolderService.create(parentNodeRef,
					surveyName, SurveyModel.TYPE_SURVEY_SPACE);
			newNodeRef = fileInfo.getNodeRef();
		} catch (final RuntimeException e) {
			if(logger.isErrorEnabled())
	...
	private void setCircabcDefaultSettings(final NodeRef nodeRef) {
		...
		/*final String supportGroup = */getCircabcRootProfileManagerService().createCircaSupportGroup();
		...
		servicesPermissions.put(CircabcServices.CIRCABC.toString(),
				permissionsSet);
		getCircabcRootProfileManagerService().addProfile(nodeRef,
				CircabcRootProfileManagerService.Profiles.CIRCABC_ADMIN,
				servicesPermissions);
		getCircabcRootProfileManagerService().addProfileTitles(nodeRef,
				CircabcRootProfileManagerService.Profiles.CIRCABC_ADMIN, CIRCABC_ADMIN_TITLE);
		...
		servicesPermissions.put(CircabcServices.CIRCABC.toString(),
				permissionsSet);
		getCircabcRootProfileManagerService().addProfile(nodeRef, profileName,
				servicesPermissions);
		getCircabcRootProfileManagerService().addProfileTitles(nodeRef,
				profileName, REGISTRED_TITLE);
		...
		nodeService.addAspect(nodeRef,
				CircabcModel.ASPECT_CIRCABC_MANAGEMENT, null);
		...
		getCircabcRootProfileManagerService().setAlfrescoAdminPermissions(nodeRef,
				permissions, true);
	}
	...
	private void setCircabcDefaultRules(final NodeRef circabcNodeRef) {
		// Add CircaManagement
		final CompositeAction compositeActionManagement = actionService
				.createCompositeAction();

		// Create Action
		final Action actionManagement = actionService
		...
		compositeActionManagement
				.setDescription("Add CircaManagement Aspect Description");

		// Create Condition
		final ActionCondition actionConditionLibrary = actionService
				.createActionCondition(NoConditionEvaluator.NAME);
	...
	private void setCircabcCategoryDefaultSettings(final NodeRef nodeRef) {
		...
		servicesPermissions = new HashMap<String, Set<String>>();
		permissionsSet = new HashSet<String>();
		permissionsSet.add(CategoryPermissions.CIRCACATEGORYADMIN
				.toString());
		servicesPermissions.put(CircabcServices.CATEGORY.toString(),
				permissionsSet);

		getCategoryProfileManagerService().addProfile(nodeRef,
						CategoryProfileManagerService.Profiles.CIRCA_CATEGORY_ADMIN,
						servicesPermissions);

		getCategoryProfileManagerService().addProfileTitles(nodeRef,
				CategoryProfileManagerService.Profiles.CIRCA_CATEGORY_ADMIN, CATEGORY_ADMIN);
		...
		permissionsSet.add(CategoryPermissions.CIRCACATEGORYACCESS
				.toString());
		servicesPermissions.put(CircabcServices.CATEGORY.toString(),
				permissionsSet);
		getCategoryProfileManagerService().addProfile(nodeRef, profileName,
				servicesPermissions);
		getCategoryProfileManagerService().addProfileTitles(nodeRef,
				profileName, REGISTRED_TITLE);
		...
		// CircaBCAdmin
		final NodeRef circabcNodeRef = nodeService.getPrimaryParent(nodeRef)
				.getParentRef();
		final String circabcAdminGroup = getCircabcRootProfileManagerService()
				.getProfile(circabcNodeRef,
						CircabcRootProfileManagerService.Profiles.CIRCABC_ADMIN)
		...
		permissionService.setPermission(nodeRef, circabcAdminGroup,
				CategoryPermissions.CIRCACATEGORYADMIN.toString(), true);
	}
			...
			final NodeRef circabcCategoryNodeRef, final NodeRef nodeRef, boolean createOptionalProfiles)
	{
		...
		if(createOptionalProfiles)
		{
			...
			servicesPermissions = new HashMap<String, Set<String>>();
			servicesPermissions.put(CircabcServices.DIRECTORY.toString(),
					permissionsSet);
			permissionsSet = new HashSet<String>();
			permissionsSet.add(VisibilityPermissions.VISIBILITY.toString());
			servicesPermissions.put(CircabcServices.VISIBILITY.toString(),
					permissionsSet);
			getIGRootProfileManagerService().addProfile(nodeRef,
					IGRootProfileManagerService.Profiles.ACCESS,
					servicesPermissions);
			getIGRootProfileManagerService().addProfileTitles(nodeRef,
					IGRootProfileManagerService.Profiles.ACCESS, ACCESS_TITLE);
			...
			getIGRootProfileManagerService().addProfile(nodeRef,
					...
					servicesPermissions);
			getIGRootProfileManagerService().addProfileTitles(nodeRef,
					IGRootProfileManagerService.Profiles.AUTHOR, AUTHOR_TITLE);
			...
			getIGRootProfileManagerService().addProfile(nodeRef,
					...
					servicesPermissions);
			getIGRootProfileManagerService().addProfileTitles(nodeRef,
					IGRootProfileManagerService.Profiles.CONTRIBUTOR, CONTRIB_TITLE);
			...
			getIGRootProfileManagerService().addProfile(nodeRef,
					...
					servicesPermissions);
			getIGRootProfileManagerService().addProfileTitles(nodeRef,
					IGRootProfileManagerService.Profiles.IGLEADER, LEADER_TITLE);
			...
			getIGRootProfileManagerService().addProfile(nodeRef,
					...
					servicesPermissions);
			getIGRootProfileManagerService().addProfileTitles(nodeRef,
					IGRootProfileManagerService.Profiles.REVIEWER, REVIEWER_TITLE);
			...
			getIGRootProfileManagerService().addProfile(nodeRef,
					...
					servicesPermissions);
			getIGRootProfileManagerService().addProfileTitles(nodeRef,
					IGRootProfileManagerService.Profiles.SECRETARY, SECRETARY_TITLE);
		}

		final String registredProfile = getCircabcRootProfileManagerService().getAllCircaUsersGroupName();
		...
		getIGRootProfileManagerService().addProfileTitles(nodeRef,
				registredProfile, REGISTRED_TITLE);

		getIGRootProfileManagerService().addProfileTitles(nodeRef,
				guestProfile, ANONYMOUS_TITLE);

		// CategoryAdmin
		final String circabcCategoryAdminGroup = getCategoryProfileManagerService()
				.getProfile(circabcCategoryNodeRef, CategoryProfileManagerService.Profiles.CIRCA_CATEGORY_ADMIN)
				.getPrefixedAlfrescoGroupName();

		// Add defaults Profiles permissions
		permissionService.setPermission(nodeRef, circabcCategoryAdminGroup, DirectoryPermissions.DIRADMIN.toString(), true);
		permissionService.setPermission(nodeRef, circabcCategoryAdminGroup, IgPermissions.IGDELETE.toString(), true);
		permissionService.setPermission(nodeRef, circabcCategoryAdminGroup, VisibilityPermissions.VISIBILITY.toString(), true);
	...
	private void setCircabcLibraryDefaultSettings(
		...
		final Map<String, Profile> profiles = getIGRootProfileManagerService().getProfileMap(circabcIGRootNodeRef);
		...
		if(profiles.containsKey(IGRootProfileManagerService.Profiles.ACCESS))
		{
			...
			servicesPermissions = new HashMap<String, Set<String>>();
			servicesPermissions.put(CircabcServices.LIBRARY.toString(),
					permissionsSet);
			getLibraryProfileManagerService().updateProfile(nodeRef,
					IGRootProfileManagerService.Profiles.ACCESS,
					servicesPermissions, true);
		}
		...
		servicesPermissions.put(CircabcServices.LIBRARY.toString(),
				permissionsSet);
		getLibraryProfileManagerService().updateProfile(nodeRef, profileName,
				servicesPermissions, true);
		...
		final NodeRef circabcCategoryNodeRef = nodeService.getPrimaryParent(
				circabcIGRootNodeRef).getParentRef();
		...
		// Add defaults Profiles permissions
		permissionService.setPermission(nodeRef, circabcCategoryAdminGroup,
				LibraryPermissions.LIBADMIN.toString(), true);
	...
	private void setDefaultLibraryRules(final NodeRef libraryNodeRef) {
		{
			// Add CircaDocument
			final CompositeAction compositeAction = actionService
					.createCompositeAction();
			...
			compositeAction
					.setDescription("Add CircaDocument Aspect Description");

			// Create Condition
			final ActionCondition actionConditionCircaDocument = actionService
		...
		{
			// Add CircaLibrary
			final CompositeAction compositeActionLibrary = actionService
					.createCompositeAction();

			// Create Action
			final Action actionLibrary = actionService
					.createAction(AddFeaturesActionExecuter.NAME);
			...
			compositeActionLibrary
					.setDescription("Add CircaLibrary Aspect Description");
	...
	private void setCircabcNewsGroupDefaultSettings(
		...
		if(profiles.containsKey(IGRootProfileManagerService.Profiles.ACCESS))
		{
			...
			servicesPermissions = new HashMap<String, Set<String>>();
			servicesPermissions.put(CircabcServices.NEWSGROUP.toString(),
					permissionsSet);
			getNewsGroupProfileManagerService().updateProfile(nodeRef,
					IGRootProfileManagerService.Profiles.ACCESS,
					servicesPermissions, true);
		...
		servicesPermissions.put(CircabcServices.NEWSGROUP.toString(),
				permissionsSet);
		getNewsGroupProfileManagerService().updateProfile(nodeRef, profileName,
				servicesPermissions, true);
		...
		// Add defaults Profiles permissions
		permissionService.setPermission(nodeRef, circabcCategoryAdminGroup,
				NewsGroupPermissions.NWSADMIN.toString(), true);
	...
	private void setDefaultNewsGroupRules(final NodeRef newsGroupNodeRef) {
		{
			// Add CircaNewsGroup
			final CompositeAction compositeActionNewsGroup = actionService
					.createCompositeAction();

			// Create Action
			final Action actionNewsGroup = actionService
					.createAction(AddFeaturesActionExecuter.NAME);
			...
			compositeActionNewsGroup
					.setDescription("Add CircaNewsGroup Aspect Description");

			// Create Condition
			final ActionCondition actionConditionNewsGroup = actionService
					.createActionCondition(IsSubTypeEvaluator.NAME);
			actionConditionNewsGroup.setParameterValue(
					IsSubTypeEvaluator.PARAM_TYPE, ContentModel.TYPE_CMOBJECT);

			compositeActionNewsGroup
					.addActionCondition(actionConditionNewsGroup);
	...
	private void setCircabcSurveyDefaultSettings(
		...
		if(profiles.containsKey(IGRootProfileManagerService.Profiles.ACCESS))
		{
			...
			servicesPermissions = new HashMap<String, Set<String>>();
			servicesPermissions
					.put(CircabcServices.SURVEY.toString(), permissionsSet);
			getSurveyProfileManagerService().updateProfile(nodeRef,
					IGRootProfileManagerService.Profiles.ACCESS,
					servicesPermissions, true);
		...
		servicesPermissions
				.put(CircabcServices.SURVEY.toString(), permissionsSet);
		getSurveyProfileManagerService().updateProfile(nodeRef, profileName,
				servicesPermissions, true);
		...
		// Add defaults Profiles permissions
		permissionService.setPermission(nodeRef, circabcCategoryAdminGroup,
				SurveyPermissions.SURADMIN.toString(), true);
	...
	private void setDefaultSurveyRules(final NodeRef surveyNodeRef) {
		{
			// Add CircaNewsGroup
			final CompositeAction compositeActionSurvey = actionService
					.createCompositeAction();

			// Create Action
			final Action actionSurvey = actionService
					.createAction(AddFeaturesActionExecuter.NAME);
			...
			compositeActionSurvey
					.setDescription("Add CircaSurvey Aspect Description");

			// Create Condition
			final ActionCondition actionConditionSurvey = actionService
					.createActionCondition(IsSubTypeEvaluator.NAME);
	...
	private void setEventDefaultSettings(
		...
		if(profiles.containsKey(IGRootProfileManagerService.Profiles.ACCESS))
		{
			...
			servicesPermissions = new HashMap<String, Set<String>>();
			servicesPermissions.put(CircabcServices.EVENT.toString(),
					permissionsSet);
			getEventProfileManagerService().updateProfile(nodeRef,
					IGRootProfileManagerService.Profiles.ACCESS,
					servicesPermissions, true);
		...
		servicesPermissions.put(CircabcServices.EVENT.toString(),
				permissionsSet);
		getEventProfileManagerService().updateProfile(nodeRef, profileName,
				servicesPermissions, true);
		...
		// Add defaults Profiles permissions
		permissionService.setPermission(nodeRef, circabcCategoryAdminGroup,
				EventPermissions.EVEADMIN.toString(), true);
	...
	{
		// Add CircaNewsGroup
		final CompositeAction compositeActionEvent = actionService
				.createCompositeAction();

		// Create Action
		final Action actionEvent = actionService
				.createAction(AddFeaturesActionExecuter.NAME);
		...
		compositeActionEvent
				.setDescription("Add Event Aspect Description");

		// Create Condition
		final ActionCondition actionConditionEvent = actionService
				.createActionCondition(IsSubTypeEvaluator.NAME);
	...
	private void setInformationDefaultSettings(final NodeRef circabcIGRootNodeRef, final NodeRef nodeRef)
	{
		final Map<String, Profile> profiles = getIGRootProfileManagerService().getProfileMap(circabcIGRootNodeRef);
		...
		if(profiles.containsKey(IGRootProfileManagerService.Profiles.ACCESS))
		{
			...
			servicesPermissions = new HashMap<String, Set<String>>();
			servicesPermissions.put(CircabcServices.INFORMATION.toString(),
					permissionsSet);
			getInformationProfileManagerService().updateProfile(nodeRef,
					IGRootProfileManagerService.Profiles.ACCESS,
					servicesPermissions, true);
		...
		servicesPermissions.put(CircabcServices.INFORMATION.toString(),
				permissionsSet);
		getInformationProfileManagerService().updateProfile(nodeRef, profileName,
				servicesPermissions, true);
		...
		// Add defaults Profiles permissions
		permissionService.setPermission(nodeRef, circabcCategoryAdminGroup,
				InformationPermissions.INFADMIN.toString(), true);
	...
	{
		// Add Information
		final CompositeAction compositeActionInformation = actionService
				.createCompositeAction();

		// Create Action
		final Action actionInformation = actionService
				.createAction(AddFeaturesActionExecuter.NAME);
		...
		compositeActionInformation
				.setDescription("Add Information Aspect Description");

		// Create Condition
		final ActionCondition actionConditionInformation = actionService
				.createActionCondition(IsSubTypeEvaluator.NAME);
		actionConditionInformation.setParameterValue(
				IsSubTypeEvaluator.PARAM_TYPE, ContentModel.TYPE_CMOBJECT);

		compositeActionInformation
				.addActionCondition(actionConditionInformation);
	...
	private NodeRef getNodeRefFromPath(String path) {
		...
		try
		{
			resultSet = searchService.query(storeRef,
				SearchService.LANGUAGE_XPATH,path );
	...
	public List<NodeRef> getCategoryHeaders(final NodeRef nodeRef) {
		if (!nodeService.hasAspect(nodeRef,
			...
			if (nodeService.hasAspect(nodeRef,
				...
				logger
						.error("A category is found without being linked to a Category Header.");
		...
		final List<NodeRef> cats = (List<NodeRef>) nodeService.getProperty(
				nodeRef, ContentModel.PROP_CATEGORIES);
	...
	public Map<NodeRef, List<NodeRef>> getCategoryHeadersMap() {
		...
		if (categories == null || categories.isEmpty()) {
			return Collections.<NodeRef, List<NodeRef>> emptyMap();
		}

		// construct the map to return
		final Map<NodeRef, List<NodeRef>> categoryHeaderMap = new HashMap<NodeRef, List<NodeRef>>(
				categories.size());
	...
	public Map<NodeRef, List<NodeRef>> getCategoryMap() {
		...
		// construct the map to return
		final Map<NodeRef, List<NodeRef>> categoryMap = new HashMap<NodeRef, List<NodeRef>>(
				categories.size());
		List<NodeRef> categoryHeaders;
	...
	public List<NodeRef> getExistingCategoryHeaders() {
		...
		if (circaBCHeadersNodeRef == null) {
			if (logger.isWarnEnabled()) {
				logger
						.warn("No circabc Category Headers found under the known Category Root Location : "
		...
		final Collection<ChildAssociationRef> childRefs = categoryService.getChildren(circaBCHeadersNodeRef, Mode.SUB_CATEGORIES, Depth.IMMEDIATE);

		headerNodes = new ArrayList<NodeRef>(childRefs.size());
		...
		if (logger.isDebugEnabled()) {
			logger.debug(headerNodes.size() + " Category Headers found: " + headerNodes + " in " + duration + " milliseconds.");			
		}
		...
		if (rootCategoryHeader == null)
		{
			...
			if (rootCategoryHeader != null) {
				if (logger.isDebugEnabled()) {
					...
					logger.debug("CircaBCHeader found at:"
							+ nodeService.getPath(rootCategoryHeader));
				}
	...
	public List<NodeRef> getCategories() {
		final List<FileInfo> childsAssoc = fileFolderService
				.listFolders(getCircabcNodeRef());

		final List<NodeRef> toReturn = new ArrayList<NodeRef>(childsAssoc
				.size());
	...
	{
		final List<ChildAssociationRef> assocs = getServiceRegistry().getNodeService().getChildAssocs(category);

		List<NodeRef> interestGroupsNodes = new ArrayList<NodeRef>(assocs.size());
	...
	public NodeRef getCategory(String categoryName) {
		return nodeService.getChildByName(getCircabcNodeRef(),
				ContentModel.ASSOC_CONTAINS, categoryName);
	}
		...
		if (circabcNodeRef == null) {
			...
			if (circabcNodeRef != null) {
				if (logger.isDebugEnabled()) {
					...
					logger.debug("CircaBC found at:"
							+ nodeService.getPath(circabcNodeRef));
				}

				if (nodeService.hasAspect(circabcNodeRef,
	...
	public NodeRef getAlfrescoDictionaryNodeRef() {
		...
		if (alfrescoDictionary != null) {
			if (logger.isDebugEnabled()) {
				...
				logger.debug("alfrescoDictionary found at:"
	...
	public NodeRef getCircabcDictionaryNodeRef() {
		if (circabcDictionary == null)
		{
			...
			circabcDictionary = nodeService.getChildByName(dictionaryNodeRef, ContentModel.ASSOC_CONTAINS, CIRCABC_DD_FOLDER);

			if(circabcDictionary == null)
			{
				circabcDictionary = fileFolderService.create(dictionaryNodeRef, CIRCABC_DD_FOLDER, ContentModel.TYPE_FOLDER).getNodeRef();
	...
	public String getCircabcLucenePath() {
		if (circabcLucenePath == null) {
			circabcLucenePath = "PATH:\""
					+ APP_COMPANY_HOME_PATH
					+ "/cm:"
					+ CircabcConfiguration.getProperty(CircabcConfiguration.CIRCABC_ROOT_NODE_NAME_PROPERTIES)
					+ "\"";
		}
	...
	public String getAlfrescoDictionaryLucenePath() {
		if (alfrescoDictionaryPath == null) {
			alfrescoDictionaryPath = "PATH:\""
					+ APP_COMPANY_HOME_PATH
					+ APP_ALFRESCO_DICTIONARY_LUCENE_PATH
					+ "\"";
		}
	...
	private String getCategoryHeaderRootLucenePath()
	{
		...
		{
			categoryHeaderLucenePath = "PATH:\""
					+ "/cm:generalclassifiable/cm:"
	...
	public boolean isCircabcUser(final String userName) {
		...
		final String prefixedUserGroupName = authorityService.getName(
				AuthorityType.GROUP, userGroupName);

		final Set<String> containingAuthorithies = authorityService
				.getContainingAuthorities(AuthorityType.GROUP, userName, false);
	...
	public boolean hasGuestVisibility(final NodeRef igRootNodeRef) {
		final Profile profile = getIGRootProfileManagerService().getProfile(
				igRootNodeRef, CircabcConstant.GUEST_AUTHORITY);
		final Set<String> permissions = profile
	...
	public boolean hasAllCircabcUsersVisibility(final NodeRef igRootNodeRef) {
		...
		final Profile profile = getIGRootProfileManagerService().getProfile(
				igRootNodeRef, allUserGroupName);
		final Set<String> permissions = profile
				.getServicePermissions(CircabcServices.VISIBILITY.toString());
		if (permissions.contains(VisibilityPermissions.VISIBILITY.toString())) {
			return true;
	...
	public NodeRef getCurrentNewsGroup(final NodeRef currentNodeRef)
	{
		...
		else
		{
			...
			while (tempNodeRef != null && !getNodeService().hasAspect(tempNodeRef, CircabcModel.ASPECT_NEWSGROUP))
			{
				tempNodeRef = getNodeService().getPrimaryParent(tempNodeRef).getParentRef();
			}
		}
	...
	public NodeRef getCurrentSurvey(final NodeRef currentNodeRef)
	{
		...
		{
			// go up to the root of the IG
			while (tempNodeRef != null && !getNodeService().hasAspect(tempNodeRef, CircabcModel.ASPECT_SURVEY))
			{
				tempNodeRef = getNodeService().getPrimaryParent(tempNodeRef).getParentRef();
			}
		}
	...
	public NodeRef getCurrentCategory(final NodeRef currentNodeRef)
	{
		...
		while (tempNodeRef != null && !getNodeService().hasAspect(tempNodeRef, CircabcModel.ASPECT_CATEGORY))
		{
			tempNodeRef = getNodeService().getPrimaryParent(tempNodeRef).getParentRef();
		}
	...
	public NodeRef getCurrentForum(final NodeRef currentNodeRef)
	{
		...
		while (tempNodeRef != null && typeDef != null &&
			   !(ForumModel.TYPE_FORUM.equals(type) || getDictionaryService().isSubClass(type, ForumModel.TYPE_FORUM)))
		{
			tempNodeRef = getNodeService().getPrimaryParent(tempNodeRef).getParentRef();
	...
	public NodeRef getCurrentTopic(final NodeRef currentNodeRef)
	{
		...
		if (typeDef != null)
		{
			...
			{
				// Nothing to do
			} else if (ForumModel.TYPE_POST.equals(type) || getDictionaryService().isSubClass(type, ForumModel.TYPE_POST))
			{
	...
	public List<NodeRef> luceneSearch(final NodeRef contextNodeRef, final boolean folders, final boolean files)
	{
        ...
        try
        {
            ...
            for (final ResultSetRow row : rs)
            {
            	...
            	if (permissionService.hasPermission(nodeRef, PermissionService.READ).equals(AccessStatus.ALLOWED)) {
	 ...
	 */
	public final void setProfileManagerServiceFactory(ProfileManagerServiceFactory profileManagerServiceFactory)
	{

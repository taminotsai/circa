...
public class NodePreferencesServiceImpl implements NodePreferencesService
{
    ...
    private static final QName TYPE_ML_ROOT = QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, "mlRoot");

    /** The configuration service root container type */
    public static final QName TYPE_CUSTOMIZATION_CONTAINER = QName.createQName(CircabcModel.CIRCABC_CONTENT_MODEL_1_0_URI, "customizationContainer");
    /** The configuration service root container type */
    public static final QName TYPE_CUSTOMIZATION_FOLDER = QName.createQName(CircabcModel.CIRCABC_CONTENT_MODEL_1_0_URI, "customizationFolder");
    /** The configuration service root container type */
    ...
    public static final QName ASSOC_CUSTOMIZE = QName.createQName(CircabcModel.CIRCABC_CONTENT_MODEL_1_0_URI, "customize");
    ...
    public NodeRef makeConfigurable(final NodeRef ref) throws CustomizationException
    {
        ...
        final NodeRef configContainer = nodeService.createNode(ref,
                    ASSOC_CUSTOMIZE,
                    TYPE_CUSTOMIZATION_CONTAINER,
                    TYPE_CUSTOMIZATION_CONTAINER).getChildRef();
        ...
        if(logger.isDebugEnabled())
        {
            logger.debug("The node " + ref + " is successfully setted being configurable. " +
                    "\n\tThe configuration container is " +  configContainer);
        }

        return configContainer;
    }

    public List<NodeRef> getConfigurationFiles(final NodeRef nodeRef, final String configTypeRoot, final String configSubType, final String configElement) throws CustomizationException
    {
        ...
        do
        {
            if(recusionRef == null)
            {
            	...
            	else if(recursNodeType.equals(CircabcModel.TYPE_CATEGORY_HEADER) || recursNodeType.equals(TYPE_ML_ROOT))
            	{
        ...
        {
            final String message = "No customization found for data: " + configTypeRoot + "/" + configSubType + "/" + configElement;
        ...
        else
        {
            if(logger.isDebugEnabled())
            {
                logger.debug(configItems.size() + " customization files for " + nodeRef + " and data: " + configTypeRoot + "/" + configSubType + "/" + configElement);
            }
    ...
    public NodeRef getDefaultConfigurationFile(final NodeRef nodeRef, final String configTypeRoot, final String configSubType, final String configElement)  throws CustomizationException
    {
        final List<NodeRef> files = getConfigurationFiles(nodeRef, configTypeRoot, configSubType, configElement);
        ...
        if(files.size() == 1)
        {
            if(logger.isDebugEnabled())
            {
                logger.debug(files.get(0) + " is the default customization file for data: " + configTypeRoot + "/" + configSubType + "/" + configElement + ". Because it is the only one.");
            }
        ...
        else
        {
            ...
            if(logger.isDebugEnabled())
            {
                if(foundWithName)
                {
                    logger.debug(defaultFileName + " is the default customization file for data: " + configTypeRoot + "/" + configSubType + "/" + configElement + ". Because it is prefixed '" + DEFAULT + "'");
                }
                else
                {
                    logger.debug(defaultFileName + " is the default customization file for data: " + configTypeRoot + "/" + configSubType + "/" + configElement + ". Because it is the oldest '" + defaultFileCreated + "'");
                }
            }
            return defaultFile;
        }
    ...
    public NodeRef addCustomizationFile(final NodeRef nodeRef, final String configTypeRoot, final String configSubType, final String configElement, final String fileName, final InputStream content)  throws CustomizationException
    {
    	return addCustomizationFileImpl(nodeRef, configTypeRoot, configSubType, configElement, fileName, content);
    }

    public NodeRef addCustomizationFile(final NodeRef nodeRef, final String configTypeRoot, final String configSubType, final String configElement, final String fileName, final File content)  throws CustomizationException
    {
    	return addCustomizationFileImpl(nodeRef, configTypeRoot, configSubType, configElement, fileName, content);
    }

    public NodeRef addCustomizationFile(final NodeRef nodeRef, final String configTypeRoot, final String configSubType, final String configElement, final String fileName, final String content)  throws CustomizationException
    {
    	return addCustomizationFileImpl(nodeRef, configTypeRoot, configSubType, configElement, fileName, content);
    }

    public NodeRef addCustomizationFile(final NodeRef nodeRef, final String configTypeRoot, final String configSubType, final String configElement, final String fileName, final Properties content)  throws CustomizationException
    {
    	return addCustomizationFileImpl(nodeRef, configTypeRoot, configSubType, configElement, fileName, content);
    }

    private NodeRef addCustomizationFileImpl(final NodeRef nodeRef, final String configTypeRoot, final String configSubType, final String configElement, final String fileName, final Object contentAsObject)  throws CustomizationException
    {
        checkParamters(nodeRef, configTypeRoot, configSubType, configElement);
        ...
        {
            throw new CustomizationException("The node " + nodeRef + " must be setted as configurable. Please use the NodePreferencesServiceImpl.makeConfigurable method.");
        }
        ...
        if(rootCustomContainer == null)
        {
            ...
            if(logger.isDebugEnabled())
            {
                logger.debug("Root customization container successfully created under the node "  + " with ref " + rootCustomContainer);
            }
        }
        ...
        if(subCustomContainer == null)
        {
            ...
            if(logger.isDebugEnabled())
            {
                logger.debug("Sub customization container successfully created under the node " + nodeRef + " with ref " + subCustomContainer);
            }
        }

        // check the configuration element
        NodeRef elementCustomContainer = getChildByName(subCustomContainer, ASSOC_CHILDREN, configElement);
        if(elementCustomContainer == null)
        {
            ...
            if(logger.isDebugEnabled())
            {
                logger.debug("Element customization container successfully created under the node " + nodeRef + " with ref " + elementCustomContainer);
            }
        }
        ...
        if(contentRef == null)
        {
            ...
            final Map<QName, Serializable> versionProps = Collections.<QName, Serializable>singletonMap(PROP_AUTO_VERSION, Boolean.TRUE);
            nodeService.addAspect(contentRef, ASPECT_VERSIONABLE, versionProps);

            if(logger.isDebugEnabled())
            {
                logger.debug("Customization file '" + fileName + "' successfully created under the node " + nodeRef + " with ref " + contentRef);
            }
        }

        //	set the body (in the content)
        final ContentWriter writter = contentService.getWriter(contentRef, CircabcModel.PROP_CONTENT, true);
        ...
        else if(contentAsObject instanceof Properties)
        {
			...
			{
            	throw new CustomizationException("IO error occurs when update customization content: " + e.getMessage(), e);
			}
        }
        else
        {
        ...
        if(logger.isDebugEnabled())
        {
            logger.debug("Customization file '" + fileName + "' successfully updated with content " + contentAsObject);
        }

        return contentRef;
    }

    public boolean customizationFileExists(final NodeRef nodeRef, final String configTypeRoot, final String configSubType, final String configElement, final String fileName)
    {
        final NodeRef file = getCustomizationOrNull(nodeRef, configTypeRoot, configSubType, configElement, fileName);
    ...
    public NodeRef getCustomization(final NodeRef nodeRef, final String configTypeRoot, final String configSubType, final String configElement, final String fileName) throws CustomizationException
    {
        final NodeRef file = getCustomizationOrNull(nodeRef, configTypeRoot, configSubType, configElement, fileName);
    ...
    public void removeCustomization(final NodeRef nodeRef, final String configTypeRoot, final String configSubType, final String configElement, final String fileName) throws CustomizationException
    {
        checkParamters(nodeRef, configTypeRoot, configSubType, configElement);
        ParameterCheck.mandatoryString("The configuration file name", fileName);
    ...
    public NodeRef getCustomizationFromNode(final NodeRef customizationNode)
    {
        ...
        {
            return getCustomizationFromNode(nodeService.getPrimaryParent(customizationNode).getParentRef());
        }
    }

    private NodeRef getCustomizationOrNull(final NodeRef nodeRef, final String configTypeRoot, final String configSubType, final String configElement, final String fileName)
    {
        checkParamters(nodeRef, configTypeRoot, configSubType, configElement);
        ParameterCheck.mandatoryString("The configuration file name", fileName);
     ...
     * @param configElement
     */
    private void checkParamters(final NodeRef nodeRef, final String configTypeRoot, final String configSubType, final String configElement)
    {
     ...
     * @return
     */
    private List<NodeRef> getConfigFilesImpl(final String configTypeRoot, final String configSubType, final String configElement, final NodeRef container)
    {
        List<NodeRef> configItems = null;
        ...
        if(folders == null)
        {
            folders = new HashMap<ConfigKey, NodeRef>();
        ...
        else
        {
            ...
            {
                final NodeRef configSubTypeRef = getChildByName(configTypeRootRef, childAssoc, configSubType);
        ...
        if(configElementRef != null)
        {
            ...
            configItems = new ArrayList<NodeRef>(childs.size());
    ...
    private NodeRef getChildByName(final NodeRef parent, final QName childAssoc, final String name)
    {
    	...
    	{
    		final List<ChildAssociationRef> assocs = nodeService.getChildAssocs(parent, childAssoc, createNameQName(name));
    		...
    		else
    		{
    			...
    			{
    				logger.warn(size + " children found with qname " + createNameQName(name) + " for customization container/file " + parent);
    			}
     ...
     * @return
     */
    private NodeRef getConfigurationContainer(final NodeRef originalRef, final NodeRef recursionRef, final boolean recursive)
    {
    	...
    	{
        	return getConfigurationContainer(originalRef, multilingualContentService.getPivotTranslation(recursionRef), recursive);
    	}
        ...
        else
        {
            ...
            final List<ChildAssociationRef> associations = nodeService.getChildAssocs(recursionRef,
                        ASSOC_CUSTOMIZE,
                        TYPE_CUSTOMIZATION_CONTAINER);
            ...
            if(associations == null || associations.size() < 1)
            {
                if(recursive)
                {
                    return getConfigurationContainer(originalRef, nodeService.getPrimaryParent(recursionRef).getParentRef(), recursive);
                }
                else
                {
                    return null;
    ...
    {
        final Map<QName, Serializable> properties = Collections.<QName, Serializable>singletonMap(PROP_NAME, name);
        final ChildAssociationRef assoc = nodeService.createNode(parent,
                ASSOC_CHILDREN,
    ...
    static class ConfigKey implements Serializable
    {
         ...
         * @param configElement
         */
        private ConfigKey(final String configTypeRoot, final String configSubType, final String configElement)
        {
            super();
     ...
     */
    public final void setCustomizationFolderCache(SimpleCache<NodeRef, Map<ConfigKey, NodeRef>> customizationFolderCache)
    {

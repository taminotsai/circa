...
public class LogoPreferencesServiceImpl implements LogoPreferencesService
{
	...
	public LogoDefinition addLogo(final NodeRef ref, final String name, final File file) throws CustomizationException, IOException
	{
	...
	public LogoDefinition addLogo(final NodeRef ref, final String name,final InputStream inputStream) throws CustomizationException, IOException
	{
	...
	public List<LogoDefinition> getAllLogos(final NodeRef ref) throws CustomizationException
	{
		...
		{
			final Set<NodeRef> configurationFiles = new HashSet<NodeRef>();
			...
			do
			{
				configurationFiles.addAll(
						nodePreferencesService.getConfigurationFiles(recursRef,
			...
			while(recursRef != null && nodeService.hasAspect(recursRef, CircabcModel.ASPECT_CIRCABC_ROOT) == false);

			final List<LogoDefinition> definitions = new ArrayList<LogoDefinition>(configurationFiles.size());
	...
	public DefaultLogoConfiguration getDefault(final NodeRef ref) throws CustomizationException
	{
		...
		{
			final NodeRef configFile = nodePreferencesService.getDefaultConfigurationFile(ref,
	    			CUSTOMIZATION_ROOT_FOLDER,
	    			CUSTOMIZATION_ICON_FOLDER,
	...
	public void setMainPageLogoConfig(final NodeRef ref, boolean display, int height, int width, boolean sizeForced, boolean logoAtLeft) throws CustomizationException
	{
		ParameterCheck.mandatory("The configurable node", ref);
	...
	public void setOtherPagesLogoConfig(final NodeRef ref, boolean display, int height, int width, boolean sizeForced) throws CustomizationException
	{
		ParameterCheck.mandatory("The configurable node", ref);
	...
	public void removeLogo(final NodeRef ref, final String logoName) throws CustomizationException
	{
		...
		final NodeRef config = nodePreferencesService.getCustomization(ref,
				CUSTOMIZATION_ROOT_FOLDER,
    			CUSTOMIZATION_ICON_FOLDER,
    			CUSTOMIZATION_ICON_DATA_FOLDER,
    			logoName);
		...
		{
			nodePreferencesService.removeCustomization(ref,
					CUSTOMIZATION_ROOT_FOLDER,
	    			CUSTOMIZATION_ICON_FOLDER,
	    			CUSTOMIZATION_ICON_DATA_FOLDER,
	    			logoName);
	...
	private void storeConfig(final NodeRef ref, final DefaultLogoConfigurationImpl config)
	{
		try
		{
			...
			nodePreferencesService.addCustomizationFile(ref,
					CUSTOMIZATION_ROOT_FOLDER,
	    			CUSTOMIZATION_ICON_FOLDER,
	    			CUSTOMIZATION_ICON_CONFIG_FOLDER,
	    			CONFIG_FILE_NAME, props);
	...
	private LogoDefinition addLogoImpl(final NodeRef ref, final String name, final Object content) throws CustomizationException, IOException
	{
		logoCache.clear();
		...
		if(nodePreferencesService.customizationFileExists(ref, CUSTOMIZATION_ROOT_FOLDER,
    			CUSTOMIZATION_ICON_FOLDER, CUSTOMIZATION_ICON_DATA_FOLDER, name))
		{
		...
		{
			customizationRef = nodePreferencesService.addCustomizationFile(ref,
        			CUSTOMIZATION_ROOT_FOLDER,
        			CUSTOMIZATION_ICON_FOLDER,
        			CUSTOMIZATION_ICON_DATA_FOLDER,
					name,
	...
	private DefaultLogoConfigurationImpl getOrCreateConfiguraton(final NodeRef ref, final boolean createIfMissing) throws CustomizationException
	{
		try
		{
			final NodeRef config = nodePreferencesService.getCustomization(ref,
		...
		catch(CustomizationException ex)
		{
			if(createIfMissing)
			{
				...
				try
				{
					...
					final DefaultLogoConfigurationImpl parentConfig = (DefaultLogoConfigurationImpl) getDefault(ref);
					...
					final NodeRef config = nodePreferencesService.addCustomizationFile(ref,
							CUSTOMIZATION_ROOT_FOLDER,
			    			CUSTOMIZATION_ICON_FOLDER,
			    			CUSTOMIZATION_ICON_CONFIG_FOLDER,
			    			CONFIG_FILE_NAME, "");
	...
	private void fillDefinition(final LogoDefinitionImpl definition) throws InvalidNodeRefException
	{
		...
		definition.setTitle((String) props.get(ContentModel.PROP_TITLE));
		definition.setDescription((String) props.get(ContentModel.PROP_DESCRIPTION));
	}

	private DefaultLogoConfigurationImpl toConfiguration(final NodeRef ref, final NodeRef configurationFile) throws CustomizationException
	{
		final Properties props = new Properties();
        ...
        finally 
        {
        	try {
				...
				logger.error("Impossible to close stream for default icon configuration for ref" + ref.toString() + " configurationFile " +  configurationFile.toString() , e);
			}
        }

        final DefaultLogoConfigurationImpl config = new DefaultLogoConfigurationImpl(
        ...
        for(final Map.Entry<Object, Object> entry: props.entrySet())
        {
        	...
        	try
			{
        		...
        		{
        			// logo can be either the logo name for root config or nodeRef.toString();
        		...
        		{
        			final Method setter = DefaultLogoConfigurationImpl.class.getDeclaredMethod(keyStr.replaceAll(REGEX_GET_OR_IS, SET_PREFIX), String.class);
        			setter.invoke(config, value);
        		}
			}
        	catch (final Exception e)
			{
        		logger.warn("Impossible to set value for " + keyStr + " setter doens't exist in DefaultLogoConfigurationImpl?");
			}
        }
	...
	private LogoDefinition toLogoDefinition(final NodeRef customizationRef, final NodeRef fromNode, final String name)
	{
		final LogoDefinitionImpl definition = new LogoDefinitionImpl();
		definition.setReference(customizationRef);
	...
	{
		// Migration 3.1 -> 3.4.6 - 12/01/2012 - Added the right property type retrieval. If the customization does not exist, 
		// it gets it the first time from an Alfresco content object (cm:content), then it is copied to a ci:content

...
public class CreateDirectoryRootPatch extends AbstractPatch
{
	...
	protected String applyInternal() throws Exception
	{
		...
		return "Directory containers successfully created under Interest Goups"
					+ "\n\tInterest Group found:   " + totalIgCount
					+ "\n\tInterest Group updated: " + updatedCount;
	}
	...
	private void addDirectoryNode(NodeRef ref)
	{
		...
		if(aspects.contains(CircabcModel.ASPECT_CATEGORY) || aspects.contains(CircabcModel.ASPECT_CIRCABC_ROOT))
		{
		...
		else if(aspects.contains(CircabcModel.ASPECT_IGROOT))
		{
			...
			final List<ChildAssociationRef> associations = nodeService.getChildAssocs(ref, CircabcModel.ASSOC_IG_DIRECTORY_CONTAINER, CircabcModel.TYPE_DIRECTORY_SERVICE);
			...
			if(associations.size() < 1)
			{
				...
				{
					logger.debug("The Interest Group " + nodeService.getProperty(ref, ContentModel.PROP_NAME) + " has not a directory container setted! Let's to create it....");
				}
			...
			else
			{
				...
				if(logger.isDebugEnabled())
				{
					 logger.debug("The Interest Group " + nodeService.getProperty(ref, ContentModel.PROP_NAME) + " has already a directory container setted.");
				}
			}

			if(directory == null)
			{
				logger.warn("Something goes wrong. Directory home not created under " + nodeService.getProperty(ref, ContentModel.PROP_NAME) + "(" + ref + ")");
			}
			// sometimes the "add circabc management aspect" rule doesn't work successfully ... force it if needed.
			else if(!nodeService.hasAspect(directory, CircabcModel.ASPECT_CIRCABC_MANAGEMENT))
			{
				if(logger.isDebugEnabled())
				{
					 logger.debug("The directory root of the Interest Group " + nodeService.getProperty(ref, ContentModel.PROP_NAME) + " has no the 'CircabcManagement' aspect setted. Set it .....");
				}
			...
			else
			{
				if(logger.isDebugEnabled())
				{
					 logger.debug("The directory root of the Interest Group " + nodeService.getProperty(ref, ContentModel.PROP_NAME) + " has successfully the 'CircabcManagement' aspect setted.");
				}
			}
		}

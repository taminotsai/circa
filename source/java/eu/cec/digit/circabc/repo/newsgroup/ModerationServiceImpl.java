...
public class ModerationServiceImpl implements ModerationService
{
	...
	public void applyModeration(final NodeRef container, final boolean makeContentWaiting)
	{
		...
		if(isContainer(container) == false)
		{
	...
	{
		if(isContainerModerated(container) == false)
		{
			nodeService.addAspect(container,
	...
	public void accept(final NodeRef content)
	{
		...
		{
			throw new IllegalArgumentException("Impossible to accept a container. The moderation is possible only on a content or a post");
		}
		else if(isWaitingForApproval(content) == false)
		{
			throw new IllegalArgumentException("Impossible to accept a post not defined being waiting for approval!");
		}
		...
		if(logger.isDebugEnabled())
		{
			logger.debug("Node successfully accepted by "
					+ moderationProperties.get(ModerationModel.PROP_APPROVED_BY)
					+ " on " + moderationProperties.get(ModerationModel.PROP_APPROVED_ON) + ": " + content);
		}
	}
	...
	public void reject(final NodeRef content, final String message)
	{
		...
		if(isContainer(content))
		{
			throw new IllegalArgumentException("Impossible to reject a container. The moderation is possible only on a content or a post");
		}
		else if(isWaitingForApproval(content) == false)
		{
			throw new IllegalArgumentException("Impossible to reject a post not defined being waiting for approval!");
		}
		...
		try
		{
			...
			wasEnable = !policyBehaviourFilter.disableBehaviour(content, ContentNotifyAspect.ASPECT_CONTENT_NOTIFY);
		...
		if(logger.isDebugEnabled())
		{
			logger.debug("Node successfully rejected by "
					+ moderationProperties.get(ModerationModel.PROP_REJECT_BY)
					+ " on " + moderationProperties.get(ModerationModel.PROP_REJECT_ON) + ": " + content);
	...
	public void waitForApproval(final NodeRef content)
	{
		...
		if(isContainer(content))
		{
			throw new IllegalArgumentException("Impossible to make a container waiting for approval. The moderation is possible only on a content or a post");
		}
	...
	public AbuseReport signalAbuse(NodeRef content, String message)
	{
		...
		if(isContainer(content))
		{
			throw new IllegalArgumentException("Impossible to make a container signaled for abuse. The moderation is possible only on a content or a post");
		}
		...
		{
			nodeService.addAspect(content, ModerationModel.ASPECT_ABUSE_SIGNALED, Collections.singletonMap(ModerationModel.PROP_ABUSE_MESSAGES, (Serializable) abuses));
		}

		if(logger.isDebugEnabled())
		{
			logger.debug("Abuse successfully signaled on the node : " + content + ". With messages: " + abuses);
		}
	...
	public void signalNotAbuse(NodeRef content)
	{
		...
		else if(nodeService.hasAspect(content, ModerationModel.ASPECT_ABUSE_SIGNALED) == false)
		{
	...
	public List<AbuseReport> getAbuses(NodeRef content)
	{
		...
		if(nodeService.hasAspect(content, ModerationModel.ASPECT_ABUSE_SIGNALED))
		{
			reports = (List<AbuseReport>) nodeService.getProperty(content, ModerationModel.PROP_ABUSE_MESSAGES);
		}
		else
		{
			reports = null;
	...
	public boolean isContainerModerated(NodeRef nodeRef)
	{
		...
		if(hasAspect.booleanValue())
		{
			final Boolean isModerated = (Boolean) nodeService.getProperty(container, ModerationModel.PROP_IS_MODERATED);

			return isModerated != null && isModerated.booleanValue();
		}
		else
		{
			return false;
	...
	private boolean isContainer(final NodeRef nodeRef)
	{
		...
		return FORUM_CONTAINERS.contains(type) || dictionaryService.isSubClass(type, ContentModel.TYPE_FOLDER);
	}

	private Map<QName, Serializable> getCommonModerationProperties(boolean reject)
	{
		final Map<QName, Serializable> moderationProps = new HashMap<QName, Serializable>(3);

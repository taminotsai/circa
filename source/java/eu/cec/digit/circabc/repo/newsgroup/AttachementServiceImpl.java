...
public class AttachementServiceImpl implements AttachementService
{
	...
	public NodeRef attach(final NodeRef referer, final NodeRef refered) throws DuplicateChildNodeNameException
	{
		...
		{
			throw new DuplicateChildNodeNameException(referer, DocumentModel.ASSOC_EXTERNAL_REFERENCES, (String) nodeService.getProperty(referer, ContentModel.PROP_NAME), null) ;
		}
	...
	public NodeRef attach(final NodeRef referer, final File referedFile, final String encoding, final String mimetype)
	{
	...
	public NodeRef attach(final NodeRef referer, final InputStream referedIs, final String name, final String encoding, final String mimetype)
	{
	...
	public List<NodeRef> getAttachements(NodeRef referer)
	{
		...
		final List<NodeRef> childs = new ArrayList<NodeRef>();

		if(nodeService.hasAspect(referer, DocumentModel.ASPECT_ATTACHABLE))
		{
			final List<AssociationRef> attachementsAssoc = nodeService.getTargetAssocs(referer, DocumentModel.ASSOC_EXTERNAL_REFERENCES);
			for (final AssociationRef assoc : attachementsAssoc)
			{
				childs.add(assoc.getTargetRef());
			}

			final List<ChildAssociationRef> attachementChilds = nodeService.getChildAssocs(referer, DocumentModel.ASSOC_HIDDEN_REFERENCES, RegexQNamePattern.MATCH_ALL);
    ...
    private NodeRef attachImpl(final NodeRef referer, final File referedFile, final InputStream referedIs, final String candidateName, final String encoding, final String mimetype)
	{
		ParameterCheck.mandatory("The referer", referer);
	...
	private String computeUniqueName(final NodeRef referer, String candidateName)
	{
		...
		{
			if(nodeService.getChildByName(referer, DocumentModel.ASSOC_HIDDEN_REFERENCES, candidateName) != null)
			{
    			...
    			{
    				uniqueName = candidateName.substring(0, idx) + '(' + tries + ')' + candidateName.substring(idx);
    			}
			}
		...
		if(logger.isDebugEnabled() && uniqueName.endsWith(candidateName) == false)
		{
			logger.debug("Another file found with name " + candidateName + ". The file will be created with name " + uniqueName);
		}
		return uniqueName;
	}

...
public class EagerCircabcFileFolderServiceImpl implements CircabcFileFolderService 
{
	...
	public List<FileInfo> list(NodeRef contextNodeRef)
	{
		...
		{
			throw new CircabcRuntimeException(contextNodeRef.toString() + " does not have library aspect");
		}
	...
	private Map<NodeRef, Set<NodeRef>> buildTree(NodeRef nodeRef)
	{
		...
		tree = new HashMap<NodeRef, Set<NodeRef>>();
		...
		try 
		{
			...
			for (ResultSetRow resultSetRow : result)
			{
				...
				{
					final HashSet<NodeRef> value = new HashSet<NodeRef>();
				...
				while (parentNodeRef != null && !getNodeService().hasAspect(parentNodeRef, CircabcModel.ASPECT_IGROOT))
				{
	...
	public void setCircabcFileFolderServiceCache(SimpleCache<String, Map<NodeRef, Set<NodeRef>>> circabcFileFolderServiceCache)
	{

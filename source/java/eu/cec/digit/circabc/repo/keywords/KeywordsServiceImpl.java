...
import static eu.cec.digit.circabc.model.KeywordModel.ASSOC_KEYWORDS;
import static eu.cec.digit.circabc.model.KeywordModel.PROP_TRANSLATED;
import static eu.cec.digit.circabc.model.KeywordModel.TYPE_KEYWORD;
...
public class KeywordsServiceImpl implements KeywordsService
{
    ...
    final static Log logger = LogFactory.getLog(KeywordsServiceImpl.class);

    /** Circabc Keywords association beetween the keyword elements and another keyword elements */
    private static final QName ASSOC_IG_KEYWORDCONTAINER = QName.createQName(CircabcModel.CIRCABC_CONTENT_MODEL_1_0_URI, "igKwContainer");
    ...
    public Keyword createKeyword(final NodeRef igNoderef, final Keyword keyword)
    {
        if(keyword == null)
        {
               throw new NullPointerException(
                        "Keyword is a mandatory parameter");
        }
    ...
    {
        final ArrayList<NodeRef> keywords = new ArrayList<NodeRef>(5);
        if(document != null)
        {
    ...
    {
        final List<NodeRef> keywordsAsNoderef = (List<NodeRef>) DefaultTypeConverter.INSTANCE.getCollection(
                NodeRef.class,
        ...
        {
            keywords = Collections.<Keyword>emptyList();
        }
        else
        {
            keywords = new ArrayList<Keyword>(keywordsAsNoderef.size());
    ...
    {
        final List<NodeRef> keywordAsList = new ArrayList<NodeRef>(1);
        ...
        if(ig == null)
        {
            ...
            throw new IllegalArgumentException("Impossible to get the interest group of the keyword " + keyword);
        }
        ...
        if(logger.isDebugEnabled())
        {
            logger.debug("Keyword successfully dropped from the referenced documents: " +
                    "\n  Keyword: " + keyword +
    ...
    public void setKeywordsToNode(final NodeRef document, final List<Keyword> keywords)
    {
        ...
        final ArrayList<NodeRef> keywordsAsNoderef = new ArrayList<NodeRef>(keywords.size());
    ...
    public List<NodeRef> getNodesForKeywords(final NodeRef parent, final List<Keyword> keywords)
    {
        ...
        final List<NodeRef> keywordNoderef = new ArrayList<NodeRef> (keywords.size());
    ...
    public List<Keyword> getKeywords(final NodeRef ig)
    {
        ...
        final List<Keyword> displayKeywords = new ArrayList<Keyword>(keywordsAsNode.size());
    ...
    protected List<NodeRef> getNodesForKeywordsNode(final NodeRef parent, final List<NodeRef> keywords)
    {
        long startTime = 0;
        if (logger.isDebugEnabled())
    ...
    protected void setKeywordsToNodeImpl(final NodeRef document, final ArrayList<NodeRef> newKeywords, final boolean replace)
    {
        if(document == null)
        {
        ...
        {
            throw new IllegalArgumentException("The " + DocumentModel.ASPECT_CPROPERTIES + " aspect is required to add a keyword to a node");
        }

        Collection<NodeRef> existingKeywords = DefaultTypeConverter.INSTANCE.getCollection(
                NodeRef.class,
                nodeService.getProperty(document, DocumentModel.PROP_KEYWORD));

        if(existingKeywords == null)
        {
            existingKeywords = new ArrayList<NodeRef>();
        }
    ...
    protected void setKeywordTranslationsNoderef(final NodeRef keyword, final MLText allValues)
    {
        if(keyword == null)
        {
            throw new NullPointerException("Keyword is a mandatory parameter. Please to create it before with the createKeyword method." );
        }
    ...
    protected NodeRef getOrCreateKeywordContainer(final NodeRef ig, final boolean create)
    {
        ...
        else if (childAssocRefs.size() == 1)
        {
            ...
            if(logger.isDebugEnabled())
            {
                logger.debug("Keyword container successfully found under the node " + ig + " with the ref " + keywordContainer);
            }
    ...
    protected boolean isKeywordNodeMultilingual(final NodeRef keyword)
    {
        final Boolean isTranslatedObject = (Boolean) nodeService.getProperty(keyword, PROP_TRANSLATED);;

        return isTranslatedObject != null && isTranslatedObject.booleanValue();
    }
    ...
    protected List<NodeRef> getKeywordsNode(final NodeRef ig)
    {
        ...
        final List<NodeRef> keywords = new ArrayList<NodeRef>(keywordsAssoc.size());
        ...
        for (final ChildAssociationRef ref : keywordsAssoc)
        {
            ...
            else
            {
                 if (logger.isWarnEnabled())
                 {
                        logger.warn("A non keyword element founds unedr a keyword container: \n" +
                                "   Interest group :  " + ig + "\n" +
                                "   Container      :  " + container + "\n" +

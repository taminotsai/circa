 ...
 * fatal errors. If a mail is sended more than a <b>FloodingLimit</b>, the mail is not send.
 *
 * <p>
 ...
 * @since 1.0
 */
public class AntiFloodingSMTPAppender extends AppenderSkeleton implements UnrecognizedElementHandler {
	 ...
	 *
	 * <p>
	 * It checks whether the current event is not flooding the system.
	 * If these checks fail, then the boolean value <code>false</code>
	 * is returned.
	 */
	protected boolean checkAntiFloodingConditions(final LoggingEvent event) {
		...
		Element element = floodingCache.get(event.getRenderedMessage());
		if (element == null) {
			element = new Element(event.getRenderedMessage(), (Serializable) Integer.valueOf(1));
			floodingCache.put(element);
		} else {
			Integer count = (Integer)element.getValue();
			count = Integer.valueOf(count.intValue() + 1);
			element = new Element(event.getRenderedMessage(), (Serializable) count);
			floodingCache.put(element);
			if(count.intValue() > this.floodingLimit) {
				return false;
			}
		}
		return true;
	 ...
	 */
	public void setFloodingCacheName(final String floodingCacheName) {
		final CacheManager cacheManager = InternalEhCacheManagerFactoryBean.getInstance();
		this.floodingCache = cacheManager.getCache(floodingCacheName);
	}
	...
	/** {@inheritDoc} */
	public boolean parseUnrecognizedElement(final org.w3c.dom.Element element, final Properties props) throws Exception {

...
<script language="javascript" type="text/javascript">
	...
	function pageLoaded()
    {
		...
		for(i=0;i<aList.length;i++)
		{
			...
			if (ie == 1)
			{
				//var script = new String(aList[i].getAttribute("onclick"));
				//var ieScript = getIeFriendlyScript(script);
		...
		for(i=0;i<inputList.length;i++)
		{
			if(inputList[i].type=='button')
			{
				if (ie == 1)
				{
					//var script = new String(inputList[i].getAttribute("onclick"));
					//var ieScript = getIeFriendlyScript(script);
    ...
    function getIeFriendlyScript(betterScript)
    {
    	betterScript = betterScript.replace("function" ,"");
		betterScript = betterScript.replace("onclick" ,"");
		betterScript = betterScript.replace("{" ,"");
		betterScript = betterScript.replace("()" ,"");
		betterScript = betterScript.replace("}" ,"");
		return betterScript;
	}
	...
	tinyMCE.init(
	{
		...
		//TODO file_browser_callback: "alfresco_TinyMCE_file_browser_callback",
	...
	function saveContent(id, content)
	{
		...
		if(typeof(content) != "undefined" && mode.value == "Html")
		{
			...
			// else don't save the post, user is adding a ne file.
		}
	}
</script>
<script language="javascript"
...
<circabc:panel id="contentMainCreateContentHtml" styleClass="contentMainForm">
	...
	<f:verbatim><div id="edit-as-html-panel" style="display:none;"></f:verbatim>
		<circabc:panel id="create-online-content-action-totext" styleClass="wai_dialog_more_action" >
			<circabc:actionLink href="#" id="create-html-html-doc" image="/images/icons/edit_online.gif" tooltip="#{cmsg.edit_inline_html_warning}" value="#{cmsg.edit_inline_html_simplest_action}" padding="2" showLink="true" onclick="showPanel('Text');return true;" />
	...
	<f:verbatim><div id="edit-as-text-panel" style="display:block;"></f:verbatim>
		...
		<circabc:panel id="create-online-content-action-tohtml" styleClass="wai_dialog_more_action" >
			<circabc:actionLink href="#" id="create-text-simple-text" image="/images/icons/edit_online.gif" tooltip="#{cmsg.edit_inline_text_warning}" value="#{cmsg.edit_inline_text_simplest_action}" padding="2" showLink="true"  onclick="showPanel('Html');return true;" />
		</circabc:panel>
	...
	<circabc:displayer rendered="#{WaiDialogManager.bean.attachementAllowed}">
		...
		<circabc:panel id="attchement-add--section-file" styleClass="signup_rub_title">
			<h:outputText id="sect-attac-1" value="#{cmsg.create_content_dialog_attachement_add_file}" escape="false"  />
		</circabc:panel>
		...
		<circabc:panel id="attchement-add-section-repo" styleClass="signup_rub_title">
			<h:outputText id="sect-attac-2" value="#{cmsg.create_content_dialog_attachement_add_link}" escape="false"  />
		</circabc:panel>
		...
		<circabc:panel id="attachement-all-att-section" styleClass="signup_rub_title">
			<h:outputText id="sect-attac-3" value="#{cmsg.create_content_dialog_attachement_all}" escape="false" />
		</circabc:panel>

		<f:verbatim><br /></f:verbatim>

		<h:outputText id="no-files" value="#{cmsg.create_content_dialog_attachement_nothing}" escape="false" rendered="#{DialogManager.bean.attachementFound == false}" styleClass="noItem" />
		...
		<h:dataTable id="allattachsTable" value="#{DialogManager.bean.attachmentDataModel}" var="attach"
			...
			<h:column id="attach-name-col">
				<f:facet name="header">
					<h:outputText id="attach-display-title" value="#{cmsg.create_content_dialog_attachement_title}" escape="false" styleClass="attachInList"/>
				</f:facet>
			...
			<h:column id="attach-type-col">
				<f:facet name="header">
					<h:outputText id="attach-display-type" value="#{cmsg.create_content_dialog_attachement_type}" escape="false" styleClass="attachInList"/>
				</f:facet>

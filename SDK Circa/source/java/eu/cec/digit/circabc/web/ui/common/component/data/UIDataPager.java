...
public class UIDataPager extends UICommand
{
	...
	public void encodeBegin(FacesContext context) throws IOException
	{
		...
		if (currentPage != 0)
		{
			...
			buf.append("\">");
			buf.append(UtilsCircabc.buildImageTag(context, WebResources.IMAGE_FIRSTPAGE, bundle.getString(FIRST_PAGE)));
			buf.append("</a>");
		} else
		{
			buf.append(UtilsCircabc.buildImageTag(context, WebResources.IMAGE_FIRSTPAGE_NONE, bundle.getString(ACTION_DESACTIVATED)));
		}
		...
		if (currentPage != 0)
		{
			...
			buf.append("\">");
			buf.append(UtilsCircabc.buildImageTag(context, WebResources.IMAGE_PREVIOUSPAGE, bundle.getString(PREVIOUS_PAGE)));
			buf.append("</a>");
		} else
		{
			buf.append(UtilsCircabc.buildImageTag(context, WebResources.IMAGE_PREVIOUSPAGE_NONE, bundle.getString(ACTION_DESACTIVATED)));
		}
		...
		for (int i = 0; i < totalIndex; i++)
		{
			if (i != currentPage)
			{
				...
				buf.append("\" title=\"");
				// output Go to Page X
				buf.append(MessageFormat.format(bundle.getString(MSG_PAGEGOTO), new Object[] { Integer.toString(i+1) } ));
				buf.append("\">");
				buf.append(i + 1);
				buf.append("</a>&nbsp;");
			} else
		...
		if ((pageCount > 10) && (pageCount % 10 != 0))
		{
			if (pageCount - 1 != currentPage)
			{
				...
				buf.append("\" title=\"");
				// output Go to Page X
				buf.append(MessageFormat.format(bundle.getString(MSG_PAGEGOTO), new Object[] { Integer.toString(pageCount) } ));
				buf.append("\">");
		...
		if ((currentPage < dataContainer.getPageCount() - 1) == true)
		{
			...
			buf.append("\">");
			buf.append(UtilsCircabc.buildImageTag(context, WebResources.IMAGE_NEXTPAGE, bundle.getString(NEXT_PAGE)));
			buf.append("</a>");
		} else
		{
			buf.append(UtilsCircabc.buildImageTag(context, WebResources.IMAGE_NEXTPAGE_NONE, bundle.getString(ACTION_DESACTIVATED)));
		}
		...
		if ((currentPage < dataContainer.getPageCount() - 1) == true)
		{
			...
			buf.append("\">");
			buf.append(UtilsCircabc.buildImageTag(context, WebResources.IMAGE_LASTPAGE, bundle.getString(LAST_PAGE)));
			buf.append("</a>");
		} else
		{
			buf.append(UtilsCircabc.buildImageTag(context, WebResources.IMAGE_LASTPAGE_NONE, bundle.getString(ACTION_DESACTIVATED)));
		}
	...
	public void decode(FacesContext context)
	{
		...
		String reqValue = (String) context.getExternalContext().getRequestParameterMap().get(HtmlRendererUtils.getHiddenCommandLinkFieldName(formInfo));
		...
		if (reqValue != null && reqValue.equals(fieldId))
		{
			...
			PageEvent actionEvent = new PageEvent(this, Integer.valueOf(paramValue).intValue());
	...
	/**
	 * We use a hidden field name based on the parent data container component Id and the string "pager" to give a field name that can be shared by all pager links within a single data container component.
	 *

 ...
 */
public class UIClipboardShelfItem extends org.alfresco.web.ui.repo.component.shelf.UIClipboardShelfItem
{
	// ------------------------------------------------------------------------------
	// Component Impl
	...
	public void decode(final FacesContext context)
	{
		...
		if(logger.isInfoEnabled()) {
			logger.info("decodeUIClipboardShelfItem " + HtmlRendererUtils.getHiddenCommandLinkFieldName(formInfo));
		}
	...
	public void encodeBegin(final FacesContext context) throws IOException
	{
		...
		if (this.collapsed)
		{
			...
			out.write(">");
			out.write(UtilsCircabc.buildImageTag(context, WebResourcesCircabc.IMAGE_CLIPBOARD_COLLAPSED, bundle.getString(MSG_CLIPBOARD_TITLE_COLLAPSED), tempClass));
			out.write(A);
		...
		} else
		{
			...
			outputAttribute(out, tempClass, CLASS);
			out.write(">");
			out.write(UtilsCircabc.buildImageTag(context, WebResourcesCircabc.IMAGE_CLIPBOARD_EXPANDED, bundle.getString(MSG_CLIPBOARD_TITLE_EXPANDED), bundle.getString(MSG_CLIPBOARD_TITLE_EXPANDED), tempClass));
			out.write(A);
			out.write(bundle.getString(MSG_CLIPBOARD_TITLE));
			out.write(END_DIV);
...
//					 check that the item has not been deleted since added to the clipboard
					...
					{
						out.write(UtilsCircabc.buildImageTag(context, WebResources.IMAGE_COPY, bundle.getString(MSG_CLIPBOARD_COPY_TOOLTIP)));
					} else
					{
						out.write(UtilsCircabc.buildImageTag(context, WebResources.IMAGE_CUT, bundle.getString(MSG_CLIPBOARD_CUT_TOOLTIP)));
					}
					out.write(NBSP);

					// then the icon item
					type = item.getType();
					isFolder = (ContentModel.TYPE_FOLDER.equals(type) || dd.isSubClass(type, ContentModel.TYPE_FOLDER));
					if (isFolder)
					{
						...
						out.write(UtilsCircabc.buildImageTag(context, icon, bundle.getString(MSG_CLIPBOARD_ITEM_ICON_FOLDER_TOOLTIP)));
					} else
					{
						image = FileTypeImageUtils.getFileTypeImage(item.getName(), true);
						out.write(Utils.buildImageTag(context, image, bundle.getString(MSG_CLIPBOARD_ITEM_ICON_FILE_TOOLTIP)));
					}
					out.write(NBSP);
					...
					// output actions
					buildActionLinkImage(out, ACTION_REMOVE_ITEM, i, bundle.getString(MSG_CLIPBOARD_REMOVE_ITEM_TOOLTIP), WebResources.IMAGE_REMOVE);
					out.write(NBSP);
					buildActionLinkImage(out, ACTION_PASTE_ITEM, i, bundle.getString(MSG_CLIPBOARD_PASTE_ITEM_TOOLTIP), WebResources.IMAGE_PASTE);
					if (item.supportsLink() && item.getMode() == ClipboardStatus.COPY && (ContentModel.TYPE_LINK.equals(item.getType()) || dd.isSubClass(item.getType(), ContentModel.TYPE_LINK)) == false)
					{
						out.write(NBSP);
						buildActionLinkImage(out, ACTION_PASTE_LINK, i, bundle.getString(MSG_CLIPBOARD_PASTE_LINK_TOOLTIP), WebResources.IMAGE_PASTE_LINK);
					}
				...
				if(items.size() > 0)
				{
					...
					out.write(" >");
					buildActionLink(out, ACTION_PASTE_ALL, -1, bundle.getString(MSG_CLIPBOARD_PASTE_ALL), bundle.getString(MSG_CLIPBOARD_PASTE_ALL_TOOLTIP));
					out.write(NBSP_NBSP);
					buildActionLink(out, ACTION_REMOVE_ALL, -1, bundle.getString(MSG_CLIPBOARD_REMOVE_ALL), bundle.getString(MSG_CLIPBOARD_REMOVE_ALL_TOOLTIP));
					out.write(NBSP_NBSP);
					buildActionLink(out, ACTION_DOWNLOAD_ALL, -1, bundle.getString(MSG_CLIPBOARD_DOWNLOAD_ALL), bundle.getString(MSG_CLIPBOARD_DOWNLOAD_ALL_TOOLTIP));
					out.write(END_DIV);
				}
	 ...
	 *
	 */
	private void buildActionLinkImage(final ResponseWriter out, final int action, final int index, final String text, final String image) throws IOException
	{
	 ...
	 *
	 */
	private void buildActionLink(final ResponseWriter out, final int action, final int index, final String text, final String tooltip) throws IOException
	{
		final FacesContext context = getFacesContext();
		final Map<String, String> attributes = new HashMap<String, String>();

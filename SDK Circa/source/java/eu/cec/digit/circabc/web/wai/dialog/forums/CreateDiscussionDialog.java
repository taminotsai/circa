...
public class CreateDiscussionDialog extends CreateTopicDialog
{
	 ...
	 */
	private NodeRef createForumNode() throws InvalidNodeRefException, InvalidAspectException, AlfrescoRuntimeException, InvalidTypeException, InvalidQNameException
	{
		...
		{
			logger.error("createDiscussion called for an object that already has a discussion!" + discussingNodeRef);
		...
		final Map<QName, Serializable> uiFacetsPropForums = new HashMap<QName, Serializable>(5);
		...
		if (logger.isDebugEnabled())
		{
			logger.debug("created forum " + forumNodeRef.toString() + " for content: " + discussingNodeRef.toString());
		}
		return forumNodeRef;
	}

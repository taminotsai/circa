...
public class DefineOthersNotification extends BaseWaiDialog
{
	 ...
	 protected String finishImpl(FacesContext context, String outcome) throws Exception
	{
		 ...
		 {
			 logger.debug("Start to update notification status on the node " + getActionNode().getNodeRef());
		 }

		 if(logger.isDebugEnabled())
		 {
			 logger.debug("-- 1.  Start to remove authorities that the notification status is removed ...");
		 }

		 final Set<AuthorityNotification> oldNotifications = getNotificationSubscriptionService().getNotifications(getActionNode().getNodeRef());
		 
		 Set<AuthorityNotification> newNotifications = Collections.emptySet();
		 
		 if(userNotifications != null)
		 {
			newNotifications = new HashSet<AuthorityNotification>(userNotifications.size());
			for(NotificationWrapper wrapper : userNotifications)
	 		{
	 			newNotifications.add(new AuthorityNotificationImpl(wrapper.getStatusValue() , wrapper.getAuthority() ));
	 		}
		 }
		
 		SetView<AuthorityNotification> differenceOldNew = Sets.difference(oldNotifications, newNotifications);
 		SetView<AuthorityNotification> differenceNewOld = Sets.difference(newNotifications , oldNotifications );

		 for(AuthorityNotification notif : differenceOldNew)
		 {
			 // remove the user
			 getNotificationSubscriptionService().removeNotification(getActionNode().getNodeRef(), notif.getAuthority());
		 ...
		 {
			 String info = MessageFormat.format("status {0} for user {1}", new Object[]{notif.getAuthority(),notif.getNotificationStatus()});
 			 logRecord.addInfo(info );
			 // remove the user
			 getNotificationSubscriptionService().setNotificationStatus(getActionNode().getNodeRef(), notif.getAuthority(),notif.getNotificationStatus());
		 ...
		 if(logger.isDebugEnabled())
		 {
			 logger.debug("Notifications successfull update for the node " + getActionNode().getNodeRef()
					 ...
					 + "\n\tAfter:    " + getNotificationSubscriptionService().getNotifications(getActionNode().getNodeRef())
					 );
		 }
	...
	public SelectItem[] pickerCallback(final int filterIndex, final String contains)
	{
		...
		{
			result = ProfileUtils.buildMailableProfileItems(getActionNode(), contains, alreadyInvitedAuthorities, logger);
		...
		{
			result = PermissionUtils.buildInvitedUserItems(getActionNode(), contains, false, alreadyInvitedAuthorities, logger);

			if(logger.isDebugEnabled())
        	{
				logger.debug("The User search is performed successfully and return " + result + ". Filter Index: " + filterIndex + ". Expression: " + contains + "." );
        	}
		}
		else
		{
    ...
    {
    	final IGRootProfileManagerService profileManager = getProfileManagerServiceFactory().getIGRootProfileManagerService();
    	final NodeRef igRoot = getManagementService().getCurrentInterestGroup(getActionNode().getNodeRef());
    	final UIGenericPicker picker = (UIGenericPicker) event.getComponent().findComponent("define-notif-picker");
        final UISelectOne profilePicker = (UISelectOne) event.getComponent().findComponent("define-notif-statuses");
            ...
            if (status != null)
            {
                ...
                for (String authority : results)
                {
                    ...
                    if (foundExisting == false)
                    {
                        ...
                        {
                        	final Profile profileFromGroup = profileManager.getProfileFromGroup(igRoot, authority);
	...
	{
    	final List<String> invitedList = new ArrayList<String> (this.userNotifications.size());
    ...
    private void fillDatas()
    {
		...
		userNotifications = new ArrayList<NotificationWrapper>(notifications.size());

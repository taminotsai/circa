...
public class DefineNewPropertyWizard extends BaseWaiWizard {
	...
	public String next()
	{
		...
		if(step == 2)
		{
			...
			if(getTranslationDataModel().getRowCount() < 1)
			{
				...
				{
					logger.debug("Impossible to add a Dynamic property to the Interest Group " + getActionNode().getName() + " because the list of translation is empty.");
				}
		...
		else if(step == 3)
		{
			...
			if(type.equals(DynamicPropertyType.SELECTION.name()))
			{
				...
				if(trimMultiValue.length() < 1)
				{
					...
					if(logger.isDebugEnabled())
					{
						logger.debug("Impossible to add a Dynamic property to the Interest Group " + getActionNode().getName() + " because the available values list is empty.");
					}
	...
	protected String finishImpl(FacesContext context, String outcome) throws Exception
	{
        ...
        final StringBuffer formatedValues = new StringBuffer("");
        if(type.equals(DynamicPropertyType.SELECTION.name()))
        {
        	final StringTokenizer tokens = new StringTokenizer(validValues.trim(), String.valueOf(DynamicPropertyService.MULTI_VALUES_SEPARATOR), false);
        ...
        final DynamicPropertyImpl dynamicProperty = new DynamicPropertyImpl(labels, DynamicPropertyType.valueOf(type), formatedValues.toString());

        String info =  MessageFormat.format("the property {0} has been created - {1}", new Object[]{dynamicProperty.getName(), dynamicProperty.toString()});
		...
		if(logger.isDebugEnabled())
		{
			logger.debug("Property " + dynamicProperty.getName() + " successfully created under the interest group " + getActionNode().getName() +  ". With value: " + dynamicProperty.toString());
		}
	...
	public void addDescription(ActionEvent event)
	{
		...
		final UISelectOne select = (UISelectOne) event.getComponent().findComponent("define-property-language");
        ...
        if(!error)
        {
        	propertyTranslations.add(new DynPropertyWrapper(selValue, selLocale));
        	text = null;
        }
	}
    ...
    public SelectItem[] getLanguages()
    {
        ...
        for(final DynPropertyWrapper wrapper : propertyTranslations)
        {
            ...
            {
                throw new IllegalStateException("The Dynamic property " + wrapper + " has not language defined. ");
            }
        }

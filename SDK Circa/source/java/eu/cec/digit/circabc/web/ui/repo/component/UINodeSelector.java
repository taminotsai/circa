...
public class UINodeSelector extends AbstractItemSelector
{
	...
	public String getParentNodeId(final FacesContext context)
	{
		...
		if (this.navigationId != null && this.navigationId.equals(getRootNode()) == false)
		{
			try
			{
				final ChildAssociationRef parentRef = getFastNodeService(context).getPrimaryParent(new NodeRef(Repository.getStoreRef(), this.navigationId));
	...
	public Collection<NodeRef> getChildrenForNode(final FacesContext context)
	{
		...
		final List<ChildAssociationRef> allKids = getNodeService(context).getChildAssocs(nodeRef, ContentModel.ASSOC_CONTAINS, RegexQNamePattern.MATCH_ALL);
		final NodeService service = getFastNodeService(context);
		// filter out those children that are not spaces
		final List<NodeRef> spaceKids = new ArrayList<NodeRef>();
		QName type;
	...
	private NodeRef getNodeRefFromPath(FacesContext context, String pathString)
	{
		...
		if (tokens.hasMoreTokens()) //root node
		{
			...
			final String rootName = nodeService.getProperty(iterNodeRef, ContentModel.PROP_NAME).toString();
	...
	public void encodeBegin(FacesContext context) throws IOException
	{
		...
		buffer.append("<div id=\"script\" style=\"visibility:hidden\" >");
	...
	public String getItemIcon(FacesContext context, NodeRef ref)
	{
		...
		} else if (type.equals(ContentModel.TYPE_FOLDER) || dictionaryService.isSubClass(type, ContentModel.TYPE_FOLDER))
		{
	...
	{
		this.showContents = Boolean.valueOf(showContents);
	}
	...
	/**
	 * We use a unique noscript field name based on our client Id (client ID concatenate with word "noscript").

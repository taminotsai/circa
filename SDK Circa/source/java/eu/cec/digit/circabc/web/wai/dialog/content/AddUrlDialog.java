...
//            public String execute() throws Throwable
//            {
//               return getErrorOutcomeIntern(exception);
//            }
//         };
//         return txnHelper.doInTransaction(callback, false, true);
//	 }
	...
	protected String finishImpl(final FacesContext context, final String outcome) throws Exception
	{
			...
			if(name.matches(PATTERN_HTML_EXTENSION) == false)
			{
				name = name + HTML;
			}
			...
			if(isParentSpace(actionNode))
			{
				...
				final FileInfo fileInfo = getFileFolderService().create(parentRef, name, ContentModel.TYPE_CONTENT);
	    	...
	    	parameters.put(EditNodePropertiesDialog.NOTIFY_AFTER_SETTING_PROPERTIES,String.valueOf(true));
	...
	protected void checkParent(final Node node)
	{
	...
	*/
	...
	{
		if (ContentModel.TYPE_CATEGORY.equals(node.getType()))
        {
        	return true;
        }
		else
	...
	/**
	 * @param name
	 * @see org.alfresco.web.bean.spaces.CreateSpaceWizard#setName(java.lang.String)
	 */

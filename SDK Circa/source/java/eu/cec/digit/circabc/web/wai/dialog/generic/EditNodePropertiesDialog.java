...
public class EditNodePropertiesDialog extends BaseWaiDialog
{
	...
	protected static final Map<ManagedNodes, String> ICONS_CONFIG = new HashMap<ManagedNodes, String>(10);
	static{
	...
	protected static final Map<ManagedNodes, String> ID_PREFIX = new HashMap<ManagedNodes, String>(5);
	static{
	...
	public void init(Map<String, String> parameters)
	{
		...
		if(parameters != null)
		{
			...
			{
				throw new IllegalArgumentException("This kind of node is not managed by this dialog. Node Type : " + getActionNode().getType().getLocalName() + " . Expected: " + Arrays.toString(ManagedNodes.values()));
			}

			if(ManagedNodes.EMPTY_TRANSLATION.equals(actionNodeType) || ManagedNodes.TRANSLATION.equals(actionNodeType))
			{
			...
			// fix for DIGIT-CIRCABC-2139
  			super.getNodeService().setProperty(getActionNode().getNodeRef(), ContentModel.PROP_AUTO_VERSION_PROPS, false);
		    ...
		    ContentData content = (ContentData)this.editableNode.getProperties().get(ContentModel.PROP_CONTENT.toString());
		    if (content != null)
		    {
	...
	{
  		wasNotifyEnable = !getPolicyBehaviourFilter().disableBehaviour(ContentNotifyAspect.ASPECT_CONTENT_NOTIFY);
		...
		// check text length
		// bugfix DIGIT-CIRCABC-352
		if(hasTextTooLong(context))
		{
			return null;
		}
		final QName type = getActionNode().getType();
		if(ContentModel.TYPE_CONTENT.equals(type) || getDictionaryService().isSubClass(type, ContentModel.TYPE_CONTENT))
		{
		...
		if((getNodeService().hasAspect(this.getActionNode().getNodeRef(), CircabcModel.ASPECT_CATEGORY)) && titleChanged)
		{
		...
		return CircabcNavigationHandler.CLOSE_WAI_DIALOG_OUTCOME + CircabcNavigationHandler.OUTCOME_SEPARATOR
				+ CircabcBrowseBean.PREFIXED_WAI_BROWSE_OUTCOME;		
	}

	public void setPropetyDefinedOutside(final QName propertyDef)
	{
		final Serializable newValue = getNodeService().getProperty(editableNode.getNodeRef(), propertyDef);
...
private void addRemainingProperties(Map<String, Object> editedProps, Map<QName, Serializable> repoProps)
{
	...
	private void logRecordSetInfo(Map<QName, Serializable> repoProps ,Map<String, Object> editableProperties)
	{
		final Set<QName> auditPropeties =new HashSet<QName>();
	...
	protected void editContainerProperties() throws Exception
	{
      ...
      if (name != null)
      {
    	  if(ManagedNodes.ML_CONTAINER.equals(actionNodeType) || ManagedNodes.CATEGORY_HEADER_NODE.equals(actionNodeType))
    	  {
	...
	{
		final String name = (String) this.editableNode.getProperties().get(ContentModel.PROP_NAME.toString());
	...
	protected boolean fieldMandatoryIsNotMissing(FacesContext context)
	{
		...
		{
			final String status = (String) this.editableNode.getProperties().get(DocumentModel.PROP_STATUS.toString());
		...
		{
			final String url = (String) this.editableNode.getProperties().get(DocumentModel.PROP_URL.toString());
			try
			{
	...
	{
		final String name = (String) this.editableNode.getProperties().get(ContentModel.PROP_NAME.toString());
		final String title = (String) this.editableNode.getProperties().get(ContentModel.PROP_TITLE.toString());
		final String desc = (String) this.editableNode.getProperties().get(ContentModel.PROP_DESCRIPTION.toString());
		final String author = (String) this.editableNode.getProperties().get(ContentModel.PROP_AUTHOR.toString());
		final String ref = (String) this.editableNode.getProperties().get(DocumentModel.PROP_REFERENCE.toString());
...
//			}
		...
		if(wasNotifyEnable)
	    {
		 ...
		 if (notify)
			{
				final Set<NotifiableUser> users = getNotificationSubscriptionService().getNotifiableUsers(getActionNode().getNodeRef());
				try
				{
	 ...
	 /**
	   ...
	   public List<UIListItem> getIcons()
	   {
	      ...
	      Config config = Application.getConfigService(FacesContext.getCurrentInstance()).getConfig(iconConfigName);
	      if (config != null)
	      {
	...
	{
		return translate("translation_note", "<b>" + I18NUtil.getContentLocale().getDisplayLanguage() + "</b>");
	}
   ...
   private void setServiceAndActivity()
	{
		...
		default:
			...
			{
				logger.warn("Not defined logging activity and service for Action Node Type " +  actionNodeType.toString());
			}
		}

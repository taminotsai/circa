...
public class IGServicesNode extends NavigableNode
{
	...
	/*package*/IGServicesNode(final NodeRef nodeRef, final InterestGroupNode igNode, final NavigableNodeType trustedType)
	{
	...
	public IGServicesNode(final NodeRef nodeRef, final NodeService nodeService, final Map<QName, Serializable> props)
	{
	...
	private void validate(final InterestGroupNode igNode)
	{
        ...
        {
           throw new IllegalArgumentException("NodeRef must be a child of an Interest Group and must not have the  " + CircabcModel.ASPECT_IGROOT + " aspect applied"); //$NON-NLS-1$ //$NON-NLS-2$
        }
		else if (!getNavigableNodeType().isEqualsOrUnder(NavigableNodeType.IG_ROOT))
        {
           throw new IllegalArgumentException("NodeRef must be a Circabc Interest Group child aspect applied"); //$NON-NLS-1$
        }

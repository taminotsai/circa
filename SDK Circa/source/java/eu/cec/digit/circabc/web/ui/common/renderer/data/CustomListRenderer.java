...
public class CustomListRenderer extends RichListRenderer
{
   ...
   public void encodeChildren(FacesContext fc, UIComponent component) throws IOException
   {
      if (component.isRendered() && component.getChildCount() == 0)
      {
         ...
         for(final ColumnConfig columnConfig: configuration.getColumns())
         {
             ...
             if(first)
             {
                 ...
                 facesColumn.getFacets().put(FACET_SMALL_ICON, generateLink(fc, configuration, columnConfig, var, true));
                 facesColumn.getChildren().add(generateLink(fc, configuration, columnConfig, var, false));
                 facesColumn.getChildren().add(generateLangLabel(fc, var));
         ...
         if(configuration.isDisplayActionColumn())
         {
             ...
             actionsColumn.getFacets().put(FACET_HEADER, generateStaticText(Application.getMessage(fc, MSG_ACTIONS)));
    ...
    private HtmlOutputText generateText(final FacesContext fc, final ColumnConfig columnConfig, final String var)
    {
    ...
    @SuppressWarnings("unchecked")
    private UIActionLink generateLink(final FacesContext fc, final NavigationPreference configuration, final ColumnConfig columnConfig, String var, boolean isIcon)
    {
        final UIActionLink link = new UIActionLink();

        final String linkTarget = configuration.getLinkTarget();
        final boolean isContent = configuration.getService().getType().equalsIgnoreCase(NavigationPreferencesService.CONTENT_TYPE);
        ...
        if(isDownload)
        {
        	link.setValueBinding(ATTR_HREF, buildBinding(fc, var, "alternativeDownloadUrl"));
        ...
        else
        {
            ...
            link.setActionListener(buildBindingMethod(fc, BROWSE_BEAN, METHOD_CLICK_WAI, ActionEvent.class));
        ...
        if(isIcon)
        {
            ...
            {
                link.setValueBinding(ATTR_IMAGE, buildBinding(fc, var, VALUE_SMALL_ICON, DIRECTORY_ICONS , ICON_EXTENSION));
        ...
        else
        {
            ...
            link.setValueBinding(ATTR_TOOLTIP, buildBinding(fc, var, VALUE_NAME, tooltipMessage + ": " , EMPTY));
        }
     ...
     * @param var
     */
    private HtmlOutputLabel generateLangLabel(final FacesContext fc, String var) 
    {
    ...
    {
        final UIParameter actionParam = (UIParameter) fc.getApplication().createComponent(ComponentConstants.JAVAX_FACES_PARAMETER);
     ...
     * @return
     */
    private UIActions buildActions(final FacesContext fc, final String var, final NavigationPreference configuration)
    {
        ...
        if(mandatoryAction != null && mandatoryAction.length() > 0 && !toRenderActions.contains(mandatoryAction))
        {
            toRenderActions.add(mandatoryAction);

            if(logger.isWarnEnabled())
            {
            	logger.warn("Mandatory action forced. Please to add it in the configuration file!:" + mandatoryAction);
            }
        }
    ...
    private MethodBinding buildBindingMethod(final FacesContext fc, final String object, final String value, final Class paramClass)
    {
        return fc.getApplication().createMethodBinding(MessageFormat.format(COMPOSITE_BINDING, object, value), new Class[]{paramClass} );
    }
    ...
    private ValueBinding buildBinding(final FacesContext fc, final String object, final String value, final String prefix, final String suffix)
    {
        return fc.getApplication().createValueBinding(prefix + MessageFormat.format(COMPOSITE_BINDING, object, value) + suffix);
    }
    private ValueBinding buildDifNullBinding(final FacesContext fc, final String object, final String value)
    {
        return fc.getApplication().createValueBinding(MessageFormat.format(BINDING_NOTEQUALS_NULL, object, value));
    }

...
public abstract class BaseWaiDialog extends BaseDialogBean implements WaiDialog ,WaiDialogAsync
{
	...
	public void init(final Map<String, String> parameters)
	{
		...
		if(parameters != null && parameters.get(NODE_ID_PARAMETER) != null)
		{
			...
			try
	        {
				...
				actionNodeDatabaseID = (Long) actionNode.getProperties().get(ContentModel.PROP_NODE_DBID.toString());
	...
	private final class FinishAsyncRun implements Runnable
	{
	   ...
	   public String finishAsync()
	   {
	      ...
	      if (this.isFinishedAsync == false)
	      {
	         this.isFinishedAsync = true;
	         Runnable r = new FinishAsyncRun( context,defaultOutcome,AuthenticationUtil.getFullyAuthenticatedUser()); 
	         try
	         {
	...
	public void updateLogDocument(final NodeRef nodeRef, final LogRecord logRecord)
	{
		...
		else
		{
			...
			{
				logRecord.setIgID((Long) getNodeService().getProperty(categoryNodeRef, ContentModel.PROP_NODE_DBID ));
				logRecord.setIgName((String) getNodeService().getProperty(categoryNodeRef, ContentModel.PROP_NAME));
			}
			else
			{
	...
	protected String getBestTitle(final Node node)
	{
		...
		final String title = (String) getActionNode().getProperties().get(ContentModel.PROP_TITLE.toString());
	...
	protected final FileFolderService getFileFolderService()
	{
		...
		{
			fileFolderService = Services.getAlfrescoServiceRegistry(FacesContext.getCurrentInstance()).getFileFolderService();
		}
	...
	protected final NamespaceService getNamespaceService()
	{
		...
		{
			namespaceService = Services.getAlfrescoServiceRegistry(FacesContext.getCurrentInstance()).getNamespaceService();
		}
	...
	protected final ProfileManagerServiceFactory getProfileManagerServiceFactory()
	{
		...
		{
			profileManagerServiceFactory = Services.getCircabcServiceRegistry(FacesContext.getCurrentInstance()).getProfileManagerServiceFactory();
		}
	...
	protected final SearchService getSearchService()
	{
		...
		{
			searchService = Services.getAlfrescoServiceRegistry(FacesContext.getCurrentInstance()).getSearchService();
		}
	...
	protected final UserService getUserService()
	{
		if(userService == null)
		{
			userService =  Services.getCircabcServiceRegistry(FacesContext.getCurrentInstance()).getUserService();
		}
		return userService;
	}
	...
	protected final PermissionService getPermissionService()
	{
		...
		{
			permissionService = Services.getAlfrescoServiceRegistry(FacesContext.getCurrentInstance()).getPermissionService();
		}
	...
	protected final MailService getMailService()
	{
		if(mailService == null)
		{
			mailService = Services.getCircabcServiceRegistry(FacesContext.getCurrentInstance()).getMailService();
		}
		return mailService;
	}
	...
	protected final MailToMembersService getMailToMembersService()
	{
		...
		{
			MailToMembersService = Services.getCircabcServiceRegistry(FacesContext.getCurrentInstance()).getMailToMembersService();
		}
	...
	protected final MailPreferencesService getMailPreferencesService()
	{
		...
		{
			mailPreferencesService = Services.getCircabcServiceRegistry(FacesContext.getCurrentInstance()).getMailPreferencesService();
		}
	 ...
	 */
	public void setMultilingualContentService(final MultilingualContentService multilingualContentService)
	{
	...
	protected ThreadPoolExecutor getAsyncThreadPoolExecutor() {
		...
		{
			asyncThreadPoolExecutor  = Services.getCircabcServiceRegistry(FacesContext.getCurrentInstance()).getAsyncThreadPoolExecutor();
		}

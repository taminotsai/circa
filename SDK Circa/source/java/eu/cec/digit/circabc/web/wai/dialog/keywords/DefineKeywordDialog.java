...
public class DefineKeywordDialog extends BaseWaiDialog
{
    ...
    public void init(Map<String, String> parameters)
    {
        ...
        if(parameters != null)
        {
        	value = null;
            language = null;
            keywordTranslations = new ArrayList<KeywordWrapper>(30);
    ...
    protected String finishImpl(FacesContext context, String outcome) throws Exception
    {
        ...
        if(getTranslationDataModel().getRowCount() < 1)
        {
            ...
            {
                logger.debug("Impossible to add a keyword to the Interest Group " + getActionNode().getName() + " because the list of translation is empty.");
            }
        ...
        if(logger.isDebugEnabled())
        {
            logger.debug("Trying to add translation to the Interest Group " + getActionNode().getName() + "...");
        }
        ...
        if(keyWordToUpdate == null)
        {
	        ...
	        if(logger.isDebugEnabled())
	        {
	            logger.debug("Keyword  successfully created for the Interest Group " + getActionNode().getName()
        ...
        else
        {
        	...
        	if(logger.isDebugEnabled())
 	        {
 	            logger.debug("Keyword  successfully updated for the Interest Group " + getActionNode().getName()
 	                    + "\n	ig: " + getActionNode().getNodeRef()
    ...
    public void addSelection(ActionEvent event)
    {
        ...
        final UISelectOne select = (UISelectOne) event.getComponent().findComponent(COMPONENT_ADD_KEYWORD_LANGUAGE);
    ...
    public SelectItem[] getLanguages()
    {
        ...
        for(final KeywordWrapper wrapper : keywordTranslations)
        {
            ...
            {
                logger.warn("The Keyword wrapper " + wrapper + " has not language defined. A non multilingual keyword should only be create at the import time.");
                throw new IllegalStateException("The Keyword wrapper " + wrapper + " has not language defined. A non multilingual keyword should only be create at the import time.");
            }
        }
    ...
    protected void fillFieldsForEditMode(Keyword keyword)
    {
    	if(keyword.isKeywordTranslated())
    	{
    		...
    		{
    			keywordTranslations.add(new KeywordWrapper(keyword.getId(), translation.getValue(), translation.getKey()));
    		}
    	}
    	else
    	{

...
public class DirectoryBean extends InterestGroupBean
{
	...
	protected SelectItem[] pageSizes = new SelectItem[] {new SelectItem(10), new SelectItem(20), new SelectItem(50), new SelectItem(100), new SelectItem(200)};
	...
	protected List<Node> usersDisplay = Collections.<Node> emptyList();
	protected List<SearchResultRecord> ldapUsers = Collections.<SearchResultRecord> emptyList();
	...
	private ExportTypeEnum _exportType = ExportTypeEnum.CSV;
	
	private List<SelectItem> exportTypes = null;
    ...
    public void init(final Map<String, String> parameters)
    {
		...
		// set the permissions
		this.hasAdvancedOption = currentIg.hasPermission(DirectoryPermissions.DIRMANAGEMEMBERS.toString());
		// Reset var
		...
		this.profileMap = getProfileManagerServiceFactory().getIGRootProfileManagerService().getProfileMap(igRootNodeRef);
		...
		String element;
		Profile value;
		for (final Iterator iter = profileMap.keySet().iterator(); iter.hasNext();)
		{
			element = (String) iter.next();
			// Remove the generic profile
			if (element.equals(CircabcConstant.GUEST_AUTHORITY) || element.equals(CircabcRootProfileManagerService.ALL_CIRCA_USERS_PROFILE_NAME))
			{
				continue;
			}
			value = (Profile) profileMap.get(element);
			items.add(new SelectItem(element,  value.getProfileDisplayName()));
		}

		profiles = items.toArray(new SelectItem[items.size()]);
    ...
    private void writeCSV(List<SearchResultRecord> userList, ServletOutputStream outStream) throws IOException
    {
    ...
    private void writeXML(List<SearchResultRecord> userList, ServletOutputStream outStream) throws XMLStreamException
    {		
    ...
    private void writeXLS(List<SearchResultRecord> userList, ServletOutputStream outStream) throws IOException {
	...
	public void export()
	{
		...
		String extension = _exportType.equals(ExportTypeEnum.Excel) ? "xls" : _exportType.toString().toLowerCase();
		...
		if (this.member.equals("allcircabcuser"))
		{
	    	...
	    	userList = new ArrayList<SearchResultRecord>(users.size());
			for (Node node : users) {
				...
				{
					username = props.get("{http://www.cc.cec/circabc/model/user/1.0}ecasUserName") == null ? "" : props.get("{http://www.cc.cec/circabc/model/user/1.0}ecasUserName").toString();
				}
		...
		{
			logger.error("Error exporting file of type " + _exportType.toString() ,ex);
		}
		finally
		{
			if (outStream != null)
	...
	public String search()
	{
		...
		this.usersDisplay = Collections.<Node> emptyList();
		this.ldapUsers = Collections.<SearchResultRecord> emptyList();
		...
		if (this.domain == null || this.member == null || this.name == null)
		{
			...
			{
				logger.error("search : Piracy warning from |" + ((NavigationBean) FacesHelper.getManagedBean(FacesContext.getCurrentInstance(), NavigationBean.BEAN_NAME)).getCurrentUser().getUserName() + "|");
			}
			return OUTCOME;
		}
		else
		{
			...
			{
				if(logger.isErrorEnabled())
					logger.error("search |3| : Piracy warning from |" + ((NavigationBean) FacesHelper.getManagedBean(FacesContext.getCurrentInstance(), NavigationBean.BEAN_NAME)).getCurrentUser().getUserName() + "|");
				return OUTCOME;
			}
		}
	}
	...
	private String searchAllUsers() {
		...
		this.ldapUsers  = getUserService().getUsersByMailDomain(this.name.trim(), this.domain);
	...
	private String searchMembers() {
		...
		Map<String, Profile> invitedUsersProfiles  = igRootProfileManagerService.getInvitedUsersProfiles(this.igRootNodeRef);

		if (this.profile == null)
		{
			if(logger.isErrorEnabled())
				logger.error("search : Piracy warning from |" + ((NavigationBean) FacesHelper.getManagedBean(FacesContext.getCurrentInstance(), NavigationBean.BEAN_NAME)).getCurrentUser().getUserName() + "| Get a profile value null with members search");
			return OUTCOME;
		}
		if (!this.profile.equals(EMPTY_STRING))
		{
			...
			{
				authorities = getAuthorityService().getContainedAuthorities(AuthorityType.USER, emptyProfile.getPrefixedAlfrescoGroupName(), false);
			}
			else
			{
				if (logger.isErrorEnabled())
			...
			{
				if(logger.isErrorEnabled())
					logger.error("search |2| : Piracy warning from |" + ((NavigationBean) FacesHelper.getManagedBean(FacesContext.getCurrentInstance(), NavigationBean.BEAN_NAME)).getCurrentUser().getUserName() + "|");
				return OUTCOME;
			}
		} else
		{
		...
		usersDomain = new ArrayList<MapNode>(authoritiesSize);
		...
		this.usersDisplay = new ArrayList<Node>(authoritiesSize);
		if ((search.equals(EMPTY_STRING)))
		{
			...
			else
			{
				...
				for (final MapNode node : usersDomain)
					{
						currentDomain = node.get(UserModel.PROP_DOMAIN).toString();
		...
		else
		{
			...
			for (final MapNode node : usersDomain)
			{
				final String firstName = node.get(ContentModel.PROP_FIRSTNAME) == null ? "" : node.get(ContentModel.PROP_FIRSTNAME).toString().toUpperCase();
				final String lastName = node.get(ContentModel.PROP_LASTNAME) == null ? "" : node.get(ContentModel.PROP_LASTNAME).toString().toUpperCase();
				final String userName = node.get(ContentModel.PROP_USERNAME) == null ? "" : node.get(ContentModel.PROP_USERNAME).toString().toUpperCase() ;
				final String email =  node.get(ContentModel.PROP_EMAIL) == null ? "" : node.get(ContentModel.PROP_EMAIL).toString().toUpperCase();
				
				if (this.domain.equalsIgnoreCase(PermissionUtils.ALLUSERS))
				{

					if ((firstName.contains(search)) || (lastName.contains(search)) || (userName.contains(search)) || (email.contains(search)))
					{
				...
				{
					final  String currentDomain = node.get(UserModel.PROP_DOMAIN).toString();
					if (this.domain.equalsIgnoreCase(currentDomain))
					{
						if ((firstName.contains(search)) || (lastName.contains(search)) || (userName.contains(search)) || (email.contains(search)))
						{
	...
	public SelectItem[] getExportTypes()
	{
		if (exportTypes == null)
		{
			exportTypes = new ArrayList<SelectItem>(3);
			...
			exportTypes.add(new SelectItem(ExportTypeEnum.Excel.toString(), ExportTypeEnum.Excel.toString()));			
		}		
		return exportTypes.toArray(new SelectItem[exportTypes.size()]);
	}
	
	public String getExportType()
	{
		return _exportType.toString();
	}	
	...
	public void setExportType(String value)
	{
		if (value != null)
		{
			_exportType = ExportTypeEnum.valueOf(value);
		}
	}	

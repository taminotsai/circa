...
public class InterestGroupNode extends NavigableNode
{
	...
	public static  final List<String> VALID_SERVICES = Collections.unmodifiableList(Arrays.asList(LIBRARY_SERVICE, NEWSGROUP_SERVICE, SURVEY_SERVICE, DIRECTORY_SERVICE, INFORMATION_SERVICE, EVENT_SERVICE));
	...
	public InterestGroupNode(final NodeRef nodeRef, final CategoryNode categoryNode)
	{
		...
		if (!NavigableNodeType.IG_ROOT.equals(getNavigableNodeType()))
        {
		   if(logger.isErrorEnabled())
		   {
			   ...
			   logger.error("Actual nodeType: " + getNavigableNodeType() != null ? getNavigableNodeType().toString() : null);
		   }
           throw new IllegalArgumentException("NodeRef must be a Circabc Interest Group and have the " + CircabcModel.ASPECT_IGROOT + " aspect applied");
        }
	...
	private Map<QName, IGServicesNode> getChildsByAspectNotSecured()
	{
		...
		try
		{
			...
			if(childsByAspectNotSecured == null)
			{
				 ...
				 final List<ChildAssociationRef> assocs = getServiceRegistry().getNodeService().getChildAssocs(this.nodeRef);
			     childsByAspectNotSecured = new HashMap<QName, IGServicesNode>(assocs.size());
			     ...
			     for(final ChildAssociationRef assoc : assocs)
			     {
			    	...
			    	if(type != null)
			    	{
						...
						{
							case LIBRARY:
								childsByAspectNotSecured.put(LIBRARY_ASPECT, new IGServicesNode(node.getNodeRef(), this, NavigableNodeType.LIBRARY));
								break;
							case NEWSGROUP:
								childsByAspectNotSecured.put(NEWSGROUP_ASPECT, new IGServicesNode(node.getNodeRef(), this, NavigableNodeType.NEWSGROUP));
								break;
							case SURVEY:
								childsByAspectNotSecured.put(SURVEY_ASPECT, new IGServicesNode(node.getNodeRef(), this, NavigableNodeType.SURVEY));
								break;
							case INFORMATION:
								childsByAspectNotSecured.put(INFORMATION_ASPECT, new IGServicesNode(node.getNodeRef(), this, NavigableNodeType.INFORMATION));
								break;
							case EVENT:
								childsByAspectNotSecured.put(EVENT_ASPECT, new IGServicesNode(node.getNodeRef(), this, NavigableNodeType.EVENT));
								break;
	...
	private Map<QName, IGServicesNode> getChildsByAspect()
	{
		...
		{
			 final List<ChildAssociationRef> assocs = getServiceRegistry().getNodeService().getChildAssocs(this.nodeRef);
		     childsByAspect = new HashMap<QName, IGServicesNode>(assocs.size());
		    	...
		    	if(type != null)
		    	{
		    		switch (type)
					{
						case DIRECTORY:
						    ...
						    {
								childsByAspect.put(DIRECTORY_TYPE, new IGServicesNode(node.getNodeRef(), this, NavigableNodeType.DIRECTORY));
						    };
						    break;
	...
	public List<NavigableNode> getNavigableChilds()
	{
		...
		List<NavigableNode> childs = new ArrayList<NavigableNode>();
   ...
   private NodePropertyResolver resolverEvent = new NodePropertyResolver() {
	...
	private NodePropertyResolver resolverContactInfo = new NodePropertyResolver() {
		...
		public Object get(Node node) {
	    	  Object contactInfo = node.getProperties().get(CircabcModel.PROP_CONTACT_INFORMATION.toString());
	    	  return (contactInfo == null  || contactInfo.toString().length() < 1)
	    	  			? "<i>" + Application.getMessage(FacesContext.getCurrentInstance(), MSG_NO_CONTACT_INFO) + "</i>"
	...
	private NodePropertyResolver resolverSafeDescription = new NodePropertyResolver() {
		...
		public Object get(Node node)
		{
	    	  	...
	    	  	{
	    	  		return  "<i>" + Application.getMessage(FacesContext.getCurrentInstance(), MSG_NO_DESCRIPTION) + "</i>";
	    	  	}
	    	  	else
	    	  	{
		...
		private static final long serialVersionUID = -5141066212483817911L;
		   ...
		   private NodePropertyResolver resolverNeedModeDetails = new NodePropertyResolver() {
				...
				public Object get(Node node)
				{
			    	  	...
			    	  	if(lDesc == null || lDesc.toString().length() < 100)
			    	  	{
			    	  		return  false;
			    	  	}
			    	  	else

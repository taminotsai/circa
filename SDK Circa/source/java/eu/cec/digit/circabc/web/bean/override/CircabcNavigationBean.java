 ...
 */
public class CircabcNavigationBean extends NavigationBean implements Serializable, IContextListener
{
    ...
    private Stack<String> visitedNodes =  new  Stack<String>();
    ...
    /**
     * If the current current node is an Interest Group or any of its child, this method activate
     * any valid IG service.
     *
    ...
    public void activateIGService(final String service)
    {
        if(getCurrentIGRoot() != null)
        {
            if(isIgServiceValid(service))
            {
                ...
                if(memIgServiceId != null && getCurrentIGService() != null && !currentIgService.getId().equals(memIgServiceId))
                {
            ...
            {
                throw new IllegalArgumentException("The service specified doesn't exist: " + service + ". It must be contains in " + Arrays.toString(InterestGroupNode.VALID_SERVICES.toArray()));
            }
        }
        else
        {
            throw new IllegalStateException("You cant activate an Interest Group Service if the current node is not a Interest Group");
        }
    ...
    public String getCircabcUrl()
    {
    	...
    	final String caption = Application.getMessage(fc, ECAS_REDIRECTION_PAGE_BUTTON_CAPTION);
    	final String tooltip = Application.getMessage(fc, ECAS_REDIRECTION_PAGE_BUTTON_CAPTION);
    	...
    	final String circabcUrl = A_HREF + WebClientHelper.getCircabcUrl() + containerUrl + TOOLTIP + tooltip + A_HREF_CLOSE + caption + A_CLOSING_TAG;
    	return circabcUrl;
    }
    ...
    public CircabcRootNode getCircabcHomeNode()
    {
        if (this.circabcHomeNode == null)
        {
           ...
           else
           {
               ...
               throw new IllegalStateException("The application can't be running without the circabc root node.");
           }
        }
    ...
    public Node getPreviousNode()
    {
...

    	{
	        ...
	        {
	        	final NodeRef nodeRef = new NodeRef(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE, visitedNodes.peek());
    ...
    public String getCurrentUserName()
    {
    	if (userName == null)
    	{
	    	userName = PermissionUtils.computeUserLogin(super.getCurrentUser().getPerson(), getNodeService());
    	}
        return userName;
    }
    ...
    public List<IBreadcrumbHandler> getLocation()
    {
    	...
    	{
    		final List<IBreadcrumbHandler> elements = new ArrayList<IBreadcrumbHandler>(parents.size());
    ...
    public void updateCircabcNavigationContext()
    {
        ...
        memCatHeaderId = (this.getCurrentCategoryHeader() == null) ? memCatHeaderId : getCurrentCategoryHeader().getId();
        memCategoryId  = (this.getCurrentCategory() == null) ? memCategoryId : getCurrentCategory().getId();
        memIgRootId    = (this.getCurrentIGRoot() == null) ? memIgRootId : getCurrentIGRoot().getId();
        memIgServiceId = (this.getCurrentIGService() == null) ? memIgServiceId : getCurrentIGService().getId();
        ...
        else
        {
            ...
            if(currentNodeType == null || NavigableNodeType.NOT_CIRCABC_CHILD.equals(currentNodeType) || CIRCABC_ROOT.equals(currentNodeType))
            {
            	// nothing more to do.
            }
            else
            {
                ...
                switch (this.currentNodeType)
                {
                    ...
                    default:
                        ...
                        {
                            final NodeRef igNodeRef = getManagementService().getCurrentInterestGroup(currentNode.getNodeRef());

                            if(igNodeRef != null)
                            {
                            	if(getPermissionService().hasPermission(igNodeRef, PermissionService.READ).equals(AccessStatus.ALLOWED))
                            	{
                                ...
                                {
                                    this.currentIgService = new IGServicesNode(currentNode.getNodeRef(), currentIGRoot);
                                }
        ...
        else if(NavigableNodeType.NOT_CIRCABC_CHILD.equals(currentNodeType) && wasInCircabc == false)
        {
        	// we are just entered in circabc
        	UICircabcContextService.getInstance(FacesContext.getCurrentInstance()).circabcEntered();
        }

        if(memCatHeaderId != null && getCurrentCategoryHeader() != null && !currentCategoryHeader.getId().equals(memCatHeaderId))
        {
        	// we are just leaving
        	UICircabcContextService.getInstance(FacesContext.getCurrentInstance()).categoryHeaderChanged();
        }
        if(memCategoryId != null && getCurrentCategory() != null && !currentCategory.getId().equals(memCategoryId))
        {
        	UICircabcContextService.getInstance(FacesContext.getCurrentInstance()).categoryChanged();
        }
        if(memIgRootId != null && getCurrentIGRoot() != null && !currentIGRoot.getId().equals(memIgRootId))
        {
     ...
     */
    public final void setMultilingualContentService(final MultilingualContentService multilingualContentService)
    {

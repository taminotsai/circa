...
public class SelfResendDialog extends CreateCircabcUserWizard
{
        ...
        if(validateFields(true,true))
        {
    		...
    		final RetryingTransactionCallback<String> callback = new RetryingTransactionCallback<String>()
    		{
    			public String execute() throws Throwable
    			{
                    ...
                    {
                        logger.error("The user " + getUserName() + " can't resent password due to an error: " + e.getMessage());
     ...
     *
	 *	@param to
     *            user email for sending email
     *	@param fullName
     *            User full name (first name and last name)
     *  @param password
     *            New password to send user
     */
	protected boolean notifyUser(final String to, final String fullName,final String userName, final char[] password) throws Exception
    {
        if (to != null && to.length() != 0)
        {
        	final NodeRef circabcRoot = getManagementService().getCircabcNodeRef();

        	final Map<String, Object> model = getMailPreferencesService().buildDefaultModel(circabcRoot, null, null);
    		...
    		final MailWrapper mail = getMailPreferencesService().getDefaultMailTemplate(circabcRoot, MailTemplate.RESEND_OWN_PASSWORD);

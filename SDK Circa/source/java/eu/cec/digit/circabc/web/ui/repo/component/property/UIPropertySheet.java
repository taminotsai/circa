...
/**
 * OVERRIDE to take a WAI UIProperty.
...
public class UIPropertySheet extends org.alfresco.web.ui.repo.component.property.UIPropertySheet
{
	   ...
	   /**
	    * Workaround, since the configuration is bugged and we can't set the name of certain kind
	    * of node being read only, we hardcoded this config.
	    *
	    * @param node
	    * @return
	    */
	   private boolean isNodeNameSecured(Node node)
	   {
		   ...
		   return type != null && (type.isIGServiceOrAbove() && !type.equals(NavigableNodeType.IG_ROOT) && !type.equals(NavigableNodeType.CATEGORY_HEADER));
	   }
	   ...
	   private void createComponentsFromConfig(FacesContext context, Collection<ItemConfig> items, boolean nameReadOnly)
	      throws IOException
	   {
	      for (ItemConfig item : items)
	      {
	         ...
	         if (propSheetItem != null)
	         {
	            ...
	            final String labelId = item.getDisplayLabelId() == null ? shortName : item.getDisplayLabelId();
	            final ResourceBundle circabcWebBundle = Application.getBundle(context).getBundle(CIRCABC_MESSAGE_BUNDLE);
	            ...
	            catch(java.util.MissingResourceException ex)
	            {
	            	...
	            	{
	            		logger.info("Can't find resource for bundle java.util.PropertyResourceBundle, key: "  + labelId);
	            	}
	            }

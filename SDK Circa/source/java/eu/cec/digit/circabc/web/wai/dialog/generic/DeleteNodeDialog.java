...
public class DeleteNodeDialog extends BaseWaiDialog
{
	...
	protected String finishImpl(final FacesContext context, final String outcome) throws Exception
	{
		try
		{
			...
			if (igRoot != null)
			{
				getNodeService().setProperty(getActionNode().getNodeRef() , CircabcModel.PROP_IG_ROOT_NODE_ID_ARCHIVED,igRoot.getId());
			}
			...
			if(getActionNode() != null && actionNodeType != null)
			{
				if(ManagedNodes.ML_CONTAINER.equals(actionNodeType))
		        {
		            if (logger.isDebugEnabled())
		                logger.debug("Trying to delete multilingual container: " + getActionNode().getId() + " and its translations" );

		            final NodeRef pivot = getMultilingualContentService().getPivotTranslation(getActionNode().getNodeRef());
		        ...
		        {
		            if (logger.isDebugEnabled())
		                logger.debug("Trying to delete " + actionNodeType.toString() +  " node: " + getActionNode().getId());

		            parent = getNodeService().getPrimaryParent(getActionNode().getNodeRef()).getParentRef();

		            final String  nodeName = (String) this.getNodeService().getProperty( getActionNode().getNodeRef(), ContentModel.PROP_NAME );
    ...
    {
       if(getActionNode() != null && getNodeService().exists(getActionNode().getNodeRef()) && actionNodeType != null)
       {
           return translate("delete_" + actionNodeType.toString() + "_dialog_confirmation", getActionNode().getName());
       }
       else
       {

...
public class AppointmentDetailsBean extends InterestGroupBean
{
    ...
    {
    	return translate((isMeeting() ? MSG_MEETING_PAGE_TITLE : MSG_EVENT_PAGE_TITLE), getAppointment().getTitle());
    }
    ...
    public String getCloseOutcome()
    {
    	if(lastNodeId != null)
    	{
    		...
    		ViewHandler viewHandler = fc.getApplication().getViewHandler();
  	       	UIViewRoot viewRoot = viewHandler.createView(fc, CircabcNavigationHandler.WAI_DIALOG_CONTAINER_PAGE);
	...
	{
		return AppointmentWebUtils.translate(getAppointment().getOccurenceRate().getEveryTimesOccurence());
	}

	public String getUserStatus()
	{
		return AppointmentWebUtils.translate(getEventService().getMeetingStatus(getCurrentNode().getNodeRef(), getNavigator().getCurrentUser().getUserName()));
	}
    ...
    public void init(Map<String, String> parameters)
    {
    	...
    	{
    		this.closeOutcome = CircabcNavigationHandler.CLOSE_WAI_DIALOG_OUTCOME + CircabcNavigationHandler.OUTCOME_SEPARATOR + this.closeOutcome;
    	}
    	...
    	if(TYPE_EVENT.isMatch(getCurrentNode().getType()))
    	{
        	...
        	this.isMeeting  = Boolean.valueOf(this.appointment instanceof Meeting);
        	this.appointmentNode = new MapNode(eventItem);
        	final Boolean isRecurrent = !MainOccurence.OnlyOnce.equals(appointment.getOccurenceRate().getMainOccurence());
    ...
    public String getSpaceDisplayPath()
    {
    	...
    	if(ref != null)
    	{
    		final String parentPath = (String) getBrowseBean().resolverSimpleDisplayPath.get(new Node(ref));
    ...
    public List<AudienceUser> getAudienceDetail()
    {
    	...
    	final List<AudienceUser> audienceUser = new ArrayList<AudienceUser>(statuses.size());
    	...
    	for(Map.Entry<String, MeetingRequestStatus> entry : statuses.entrySet())
    	{
    		...
    		status = (MeetingRequestStatus.NotApplicable.equals(entry.getValue())
    						? ""
    						: translate(AppointmentWebUtils.MSG_PREFIX_REQUEST_STATUS + entry.getValue().name().toLowerCase()));
    		...
    		if(getPersonService().personExists(authority))
    		{
    			...
    			audienceUser.add(new AudienceUser(
    		...
    		else
    		{
    			...
    			audienceUser.add(new AudienceUser("", "", authority, status));
    		}
    	}
    ...
    public String getOccurenceAsString()
    {
    	...
    	switch (occurenceRate.getMainOccurence())
		{
			...
			case EveryTimes:
				occurStr = translate(OCCUR_EVERY_TIME, occurenceRate.getEvery(), getDisplayEveryTimesOccurence(), occurenceRate.getTimes());
				break;
		}
	...
	protected final PersonService getPersonService()
	{
		...
		{
			this.personService = Services.getAlfrescoServiceRegistry(FacesContext.getCurrentInstance()).getPersonService();
		}
	...
	protected final ContentFilterLanguagesService getContentFilterLanguagesService()
	{
		...
		{
			this.contentFilterLanguagesService = Services.getAlfrescoServiceRegistry(FacesContext.getCurrentInstance()).getContentFilterLanguagesService();
		}

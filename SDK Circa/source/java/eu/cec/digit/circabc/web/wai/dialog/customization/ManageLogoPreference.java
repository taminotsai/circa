...
public class ManageLogoPreference extends BaseWaiDialog
{
	...
	private void reset() throws IllegalStateException
	{
		...
		try
		{
			final DefaultLogoConfiguration defaultConfiguration = getLogoPreferencesService().getDefault(ref);
			resetLogos(ref);
			...
			if(defaultConfiguration.getLogo() != null && defaultConfiguration.getLogo().getReference() != null)
			{
			...
			{
				final InterestGroupNode interestGroup = (InterestGroupNode) getNavigator().getCurrentIGRoot();
				this.serviceConfigs = new ArrayList<ServiceConfigWrapper>();
	...
	protected String finishImpl(FacesContext context, String outcome) throws Throwable
	{
		try
		{
			...
			else
			{
				...
				getLogoPreferencesService().setMainPageLogoConfig(currentLocationRef,
						this.logoDisplayedOnMainPage.booleanValue(),
						height.intValue(),
						width.intValue(),
						this.mainPageSizeForced.booleanValue(),
						this.mainPageLogoAtLeft.booleanValue());

				getLogoPreferencesService().setOtherPagesLogoConfig(currentLocationRef,
						this.logoDisplayedOnAllPages.booleanValue(),
						getSafeSize(this.otherPagesLogoHeight).intValue(),
						getSafeSize(this.otherPagesLogoWidth).intValue(),
						this.otherPagesSizeForced.booleanValue());
				...
				if(serviceConfigs != null && isServiceConfigAllowed())
				{
					for(final ServiceConfigWrapper wrapper: serviceConfigs)
					{
						...
						else
						{
							...
							{
								getLogoPreferencesService().setDefault(wrapper.getService().getNodeRef(), wrapper.getSelectedLogo());
							}
							if(wrapper.needToChangeDisplay())
							{
								getLogoPreferencesService().setOtherPagesLogoConfig(
										wrapper.getService().getNodeRef(),
										wrapper.getDisplay().booleanValue(),
		...
		catch(final Throwable t)
		{
	         ...
	         {
	        	 logger.error("Unexpected error during settings of logo definition on " + getActionNode().getNodePath(), t);
	         }
	...
	public List<LogoWrapper> getLogoDefinitions()
	{
		...
		final List<LogoWrapper> wrappers = new ArrayList<LogoWrapper>(logos.size());
	...
	protected final LogoPreferencesService getLogoPreferencesService()
	{
		...
		{
			this.logoPreferencesService = Services.getCircabcServiceRegistry(FacesContext.getCurrentInstance()).getLogoPreferencesService();
		}
	...
	protected final InterestGroupLogoBean getInterestGroupLogoBean()
	{
		...
		{
			interestGroupLogoBean = (InterestGroupLogoBean) Beans.getBean(InterestGroupLogoBean.BEAN_NAME);
		}
	...
	public final void setIconLink(NodeRef iconLink)
	{
		...
		if(iconLink != null)
		{
			...
			else if(isDuplicateLogoName(currentRef, name).booleanValue() == false)
			{
				InputStream inputStream = null;

				try
				{
					inputStream = getContentService().getReader(iconLink, ContentModel.PROP_CONTENT).getContentInputStream();
	...
	public final void setIconFile(String name)
	{
		...
		if (fileBean != null)
		{
			final NodeRef currentRef = getActionNode().getNodeRef();
			InputStream inputStream = null;
			if(isDuplicateLogoName(currentRef, name).booleanValue() == false)
			{
				try
				{
	...
	protected final ContentService getContentService()
	{
		...
		{
			contentService = Services.getAlfrescoServiceRegistry(FacesContext.getCurrentInstance()).getContentService();
		}

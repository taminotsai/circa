...
public class UserDetailsImpl implements Serializable, UserDetails
{
	...
	/*package*/ UserDetailsImpl(final NodeRef personRef, final UserDetailsBusinessImpl userDetailSrv)
	{
	 ...
	 * TODO set notification status as a preference!
	 */
	 ...
	 * TODO set visibility as a preference!
	 */
	...
	/**
	 * @return	The user id used for display. The user identifier entered by user when it authenticate itself if different that the UserName.
	 */
	...
	public boolean isPersonalDataHidden()
	{
		if(personalDataHidden == null)
		{
			...
			{
				personalDataHidden =  !getVisibility().booleanValue();
			}
		}

		return personalDataHidden.booleanValue();
	}

	//--  Utils

	private final Serializable getProperty(final QName qname)
	{
		return getProperty(qname, true);
	}

	private final Serializable getProperty(final QName qname, final boolean checkUpdated)
	{
	...
	private final Serializable getPreference(final QName qname)
	{
		return getPreference(qname, true);
	}

	private final Serializable getPreference(final QName qname, final boolean checkUpdated)
	{
	...
	private final void setProperty(final QName qname, final Serializable value)
	{
		if(updatedProperties == null)
		{
			updatedProperties = new HashMap<QName, Serializable>(12);
		}
	...
	private final void setPreference(final QName qname, final Serializable value)
	{
		if(updatedPreferences == null)
		{
			updatedPreferences = new HashMap<QName, Serializable>(12);
		}

...
/**
 * Business manager that perform checking of the type and state of nodes. (ie: is a folder? is the document locked? is a post?)
 *
 * @author Yanick Pignot
 */
public class NodeTypeManager
{
	private static final List<QName> DIRECTORY_CHILDREN_TYPE = Arrays.<QName>asList(
							CircabcModel.TYPE_INTEREST_GROUP_PROFILE,
							ContentModel.TYPE_PERSON);

	private static final List<QName> IG_SERVICES_ROOT_ASPECTS = Arrays.<QName>asList(
	...
	private static final List<QName> IG_SERVICES_CHILD_ASPECTS = Arrays.<QName>asList(
	...
	private static final List<QName> LINKS_TYPE = Arrays.<QName>asList(
	...
	{
		return isFromType(nodeRef, ContentModel.TYPE_CONTENT) && !hasAspect(nodeRef, DocumentModel.ASPECT_URLABLE);
	}
	...
	{
		return hasAspect(nodeRef, CircabcModel.ASPECT_CIRCABC_MANAGEMENT) || isMultilingualContent(nodeRef);
	}
	...
	private boolean isFromType(final NodeRef nodeRef, final QName typeQname, final boolean orSubtype)
	{
		if(nodeRef == null)
		{
			return false;
	...
	private boolean isFromAnyType(final NodeRef nodeRef, final List<QName> typeQnames, final boolean orSubtype)
	{
		if(nodeRef == null)
		{
			return false;
	...
	private boolean isFromAnyType(final List<QName> typeQnames, boolean orSubtype, final QName nodeType)
	{
		if(nodeType == null)
		{
			return false;

...
public class MailMeContentBusinessImpl implements MailMeContentBusinessSrv
{
	 ...
	 */
	public boolean send(final NodeRef nodeRef)
	{
		...
		final NodeRef person = userManager.getPerson(userName);
		...
		try
		{
			...
			{
				final MailWrapper mail = mailPreferencesService.getDefaultMailTemplate(targetIfLink, MailTemplate.MAIL_ME_CONTENT);
				return mailService.sendNode(targetIfLink, getFromAddress(), to, null,  mail.getSubject(model, locale), mail.getBody(model, locale), true);
			}
			else
			{
				final MailWrapper mail = mailPreferencesService.getDefaultMailTemplate(targetIfLink, MailTemplate.MAIL_ME_DOSSIER);
				return mailService.send(getFromAddress(), to, null,  mail.getSubject(model, locale), mail.getBody(model, locale), true);
			}
		}
	...
	{
		Serializable langObject = userManager.getUserPreference(person, UserService.PREF_INTERFACE_LANGUAGE);
		Locale locale;

...
public class PropertiesBusinessImpl implements PropertiesBusinessSrv
{
	...
	public Map<String, PropertyItem> getProperties(final NodeRef nodeRef)
	{
		...
		final Map<String, Serializable> stringKeyedProps = new HashMap<String, Serializable>(nodeProps.size());
		...
		final Map<String, PropertyItem> propertyItems = new LinkedHashMap<String, PropertyItem>();
		...
		for(final Map.Entry<String, ItemConfig> prop: propsConfig.entrySet())
		{
			...
			propertyItems.put(propName, new ConfigurableProperty(propName, stringKeyedProps.remove(propName), prop.getValue()));
		}

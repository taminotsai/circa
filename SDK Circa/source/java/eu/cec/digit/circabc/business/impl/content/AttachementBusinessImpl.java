...
public class AttachementBusinessImpl implements AttachementBusinessSrv
{
	...
	public NodeRef addTempAttachement(final String name, final File file)
	{
		...
		return contentManager.createContent(referer, name, DocumentModel.ASSOC_HIDDEN_REFERENCES, DocumentModel.TYPE_HIDDEN_ATTACHEMENT_CONTENT, file, false);
	}
	...
	public NodeRef addAttachement(final NodeRef referer, final NodeRef refered)
	{
		...
		if(temporaryFileManager.isTempFile(refered))
		{
			...
			ChildAssociationRef assoc = nodeService.moveNode(refered, referer, DocumentModel.ASSOC_HIDDEN_REFERENCES, DocumentModel.TYPE_HIDDEN_ATTACHEMENT_CONTENT);
	...
	public List<Attachement> getAttachements(final NodeRef referer)
	{
		...
		final List<Attachement> attachements = new ArrayList<Attachement>();

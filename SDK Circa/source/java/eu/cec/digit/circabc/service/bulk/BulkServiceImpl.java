...
public class BulkServiceImpl implements BulkService {
	...
	private static final String FILE_REFERENCED_NOT_PRESENT_IN_ZIP = "file_referenced_not_present_in_zip";
	...
	public List<CompressedEntry> getCompressedEntries(
			final NodeRef containerNodeRef, final File compressedFile,
			final List<ValidationMessage> messages) {
		if (logger.isDebugEnabled()) {
			logger.debug("upload file:" + compressedFile.getPath() + " to:"
					+ containerNodeRef);
		}
		final List<CompressedEntry> compressedEntries = zipService
				.getCompressedEntries(compressedFile, messages);
		return compressedEntries;
	}

	/**
	 * Extract "index" from the compressed and create a List of entries
	 * "IndexRecord"
	 */
	public List<IndexRecord> getIndexRecords(final File compressedFile,
			final List<ValidationMessage> messages) throws IOException {
		final List<IndexRecord> indexRecords = new ArrayList<IndexRecord>();
		...
		try {
			...
			// file exist)
			final boolean exist = zipService.extract(compressedFile,
					IndexService.INDEX_FILE, temporaryIndexFile, messages);
			...
			if (exist) {
				...
				indexService.getIndexRecords(temporaryIndexFile, indexRecords,
						messages);
			} else {
				messages.add(new ValidationMessageImpl(0, "",
						"No index file provided", ErrorType.Warning));
			}
		} finally {
	...
	public List<UploadedEntry> upload(
			...
			final List<ValidationMessage> messages) 
	{
		final List<UploadedEntry> uploadedEntries = new ArrayList<UploadedEntry>();
		if (logger.isDebugEnabled()) 
		{
			logger.debug("upload file:" + compressedFile.getPath() + " to:"
					+ containerNodeRef);
		}
		final List<String> excludedFileName = new ArrayList<String>(1);
		...
		final Map<String, NodeRef> extractedFiles = zipService.extract(libraryNodeRef, containerNodeRef, compressedFile, excludedFileName, indexRecords, messages);
		...
		for (final Map.Entry<String, NodeRef> entry : extractedFiles.entrySet()) 
		{
			...
			if(indexRecord != null && indexRecord.getOriLang() == null) 
			{
				...
				setMetaDatas(libraryNodeRef, containerNodeRef, entry.getValue(), indexRecord, fileName, messages);
		...
		{
			indexRecord = getIndexRecord(indexRecords, entry.getKey());
			if(indexRecord != null && indexRecord.getOriLang() != null && indexRecord.getOriLang().equalsIgnoreCase("Y")) 
			{
				fileName = (String) nodeService.getProperty(entry.getValue(), ContentModel.PROP_NAME);
				setMetaDatas(libraryNodeRef, containerNodeRef, entry.getValue(), indexRecord, fileName, messages);
				uploadedEntry = new UploadedEntryImpl(fileName,entry.getKey());
		...
		{
			if(indexRecord2 != null && indexRecord2.getNoContent() != null && indexRecord2.getNoContent().equalsIgnoreCase("Y")) 
			{
	...
	private void setMetaDatas(final NodeRef libraryNodeRef,
			final NodeRef containerNodeRef,
			final NodeRef nodeRef, final IndexRecord indexRecord,
			final String fileName, final List<ValidationMessage> messages) {
		...
		if (indexRecord != null) {
			...
			if(nodeRef == null) {
				...
				if (indexRecord.getNoContent() != null
					...
					if (indexRecord.getRelTrans() != null
						...
						if (relatedTranslationNodeRef != null) {
							...
							try {
								...
								currentNode = multilingualContentService.addEmptyTranslation(relatedTranslationNodeRef, emptyTranslationName, I18NUtil.parseLocale(indexRecord.getDocLang()));
							} catch(final FileExistsException ex) {
								final String ERROR_DESCRIPTION = "bulk_upload_existing_empty_translation";
								messages.add(new ValidationMessageImpl(indexRecord
										.getRowNumber(), fileName,
										I18NUtil.getMessage(ERROR_DESCRIPTION),
												ErrorType.Fatal));
							}
						} else {
							final String ERROR_DESCRIPTION = "bulk_upload_invalid_translation_empty_multi_lingual_doc";
							messages.add(new ValidationMessageImpl(indexRecord
									.getRowNumber(), fileName,
									I18NUtil.getMessage(ERROR_DESCRIPTION),
											ErrorType.Fatal));
					...
					if(currentNode == null) {
						final String ERROR_DESCRIPTION = "bulk_upload_file_not_exist_either_in_zip_and_in_repository";
						messages.add(new ValidationMessageImpl(indexRecord
								.getRowNumber(), fileName,
								I18NUtil.getMessage(ERROR_DESCRIPTION),
										ErrorType.Fatal));
			...
			if (indexRecord.getTitle() != null && indexRecord.getTitle().length() > 0) {
				setPropertiesGeneric(currentNode, fileName, nodeProperties,
						ContentModel.PROP_TITLE, indexRecord.getTitle(),
						indexRecord, messages);
			}
			// Description
			if (indexRecord.getDescription() != null && indexRecord.getDescription().length() > 0) {
				setPropertiesGeneric(currentNode, fileName, nodeProperties,
						ContentModel.PROP_DESCRIPTION, indexRecord
								.getDescription(), indexRecord, messages);
			}
			...
			if (indexRecord.getStatus() != null && indexRecord.getStatus().length() > 0) {
				...
				if (valid) {
					setPropertiesGeneric(currentNode, fileName, nodeProperties,
							DocumentModel.PROP_STATUS, indexRecord.getStatus(),
							indexRecord, messages);
				} else {
			...
			if (indexRecord.getIssueDate() != null && indexRecord.getIssueDate().length() > 0) {
				try {
					...
					setPropertiesGeneric(currentNode, fileName, nodeProperties,
							DocumentModel.PROP_ISSUE_DATE, parsed, indexRecord,
							messages);
			...
			if (indexRecord.getReference() != null && indexRecord.getReference().length() > 0) {
				setPropertiesGeneric(currentNode, fileName, nodeProperties,
						DocumentModel.PROP_REFERENCE, indexRecord
								.getReference(), indexRecord, messages);
			}

			// SecurityRanking
			if (indexRecord.getSecurityRanking() != null && indexRecord.getSecurityRanking().length() > 0) {
				boolean valid = false;
				for (final String securityRanking : DocumentModel.SECURITY_RANKINGS) {
					if (securityRanking
							.equals(indexRecord.getSecurityRanking())) {
						valid = true;
						break;
					}
				}
				if (valid) {
					setPropertiesGeneric(currentNode, fileName, nodeProperties,
							DocumentModel.PROP_SECURITY_RANKING, indexRecord
									.getSecurityRanking(), indexRecord,
							messages);
				} else {
			...
			if (indexRecord.getDocLang() != null && indexRecord.getDocLang().length() > 0) {
				if (contentFilterLanguagesService.getFilterLanguages()
					...
					if (indexRecord.getOriLang() != null &&
						...
						//Langue PIVOT
						multilingualContentService.makeTranslation(currentNode,
						...
						if (indexRecord.getRelTrans() == null
							...
							final String ERROR_DESCRIPTION = "bulk_upload_param_doc_lang_on_a_document_not_translated";
							messages.add(new ValidationMessageImpl(indexRecord
									.getRowNumber(), fileName,
									I18NUtil.getMessage(ERROR_DESCRIPTION),
									ErrorType.Warning));
			...
			if (indexRecord.getRelTrans() != null
				...
				if (relatedTranslationNodeRef != null) {
					//final String relatedTranslationName = (String)nodeService.getProperty(relatedTranslationNodeRef, ContentModel.PROP_NAME);
					if (indexRecord.getNoContent() != null
							&& indexRecord.getNoContent().equals("Y")) {
						//Empty Translation allready created
					} else {
						if (currentNode != null) {
							multilingualContentService.addTranslation(currentNode,
									relatedTranslationNodeRef, I18NUtil.parseLocale(indexRecord.getDocLang()));
						}
					}
				} else {
					final String ERROR_DESCRIPTION = "bulk_upload_invalid_translation_multi_lingual_doc";
...
//			 Author
			if (indexRecord.getAuthor() != null && indexRecord.getAuthor().length() > 0) {
				if(currentNode == pivotNode && pivotNode != null) {
					//Set the Author value on the logical document if the current document (currentNode) is the pivot language
					final NodeRef logicalMlDocument = multilingualContentService.getTranslationContainer(pivotNode);
					setPropertiesGeneric(logicalMlDocument, fileName, nodeProperties,
									ContentModel.PROP_AUTHOR, indexRecord.getAuthor(),
									indexRecord, messages);
				} else {
				...
				if(relatedTranslationNodeRef != null) {
					...
					if(indexRecord.getOriLang() != null &&
					   indexRecord.getOriLang().equalsIgnoreCase("Y")) {
						...
						//Dans les autre cas...
						final String ERROR_DESCRIPTION = "bulk_upload_add_translator_with_no_related_translation";
			...
			if (indexRecord.getExpirationDate() != null && indexRecord.getExpirationDate().length() > 0) {
				try {
					final Date parsed = sdf.get().parse(indexRecord
							.getExpirationDate());
					if(currentNode == pivotNode && pivotNode != null) {
						//Set the Expiration Date value on the logical document
						final NodeRef logicalMlDocument = multilingualContentService.getTranslationContainer(pivotNode);
						setPropertiesGeneric(logicalMlDocument, fileName, nodeProperties,
								DocumentModel.PROP_EXPIRATION_DATE, parsed,
								indexRecord, messages);
					} else {
	...
	private void setPropertiesGeneric(final NodeRef nodeRef,
			final String fileName,
			final Map<QName, Serializable> nodeProperties, final QName key,
			final Serializable value, final IndexRecord indexRecord,
			final List<ValidationMessage> messages) {
		if (value != null && !value.equals("")) {
			...
			if (oldValue != null && oldValue.toString().length() > 0) {
				if (logger.isTraceEnabled()) {
					logger
							.trace("oldValue:" + oldValue + " new value:"
									+ value);
				}
			...
			try {
				if (logger.isTraceEnabled()) {
					logger.trace("set new value:" + value + " on noderef:"
							+ nodeRef + " that represent the file:" + fileName);
				}
				nodeService.setProperty(nodeRef, key, value);
			} catch (final Exception ex) {
				messages.add(new ValidationMessageImpl(indexRecord
						.getRowNumber(), fileName,
						"Error while setting property " + key + " with value:"
								+ value + " on file:" + fileName + " cause:"
								+ ex.getMessage(), ErrorType.Fatal));
			}
		}
	}
	...
	private List<NodeRef> getAllChildsNodeRefs(
		...
		final List<NodeRef> childsNodeRefs = new ArrayList<NodeRef>();

		TypeDefinition typeDef;

		if (nodeService.exists(nodeRef)
				&& AccessStatus.ALLOWED.equals(permissionService.hasPermission(
			...
			if (typeDef != null) {
				...
				if (ContentModel.TYPE_CONTENT.equals(type)
					...
					if (!childsNodeRefs.contains(nodeRef)
				...
				if (logger.isWarnEnabled()) {
					logger.warn("Found invalid object in database: id = "
							+ nodeRef + ", type = " + type);
				}
			}
		}
	...
	private void createFileLinkRepresentation(final NodeRef nodeRef, final List<NodeRef> childsNodeRefs, final List<NodeRef> allNodeRefs) {
		// only display the user has the permissions to navigate to
		// the target of the link
		final NodeRef destRef = (NodeRef) nodeService.getProperty(
				nodeRef, ContentModel.PROP_LINK_DESTINATION);
		if (destRef != null
				&& new Node(destRef)
						.hasPermission(PermissionService.READ) == true) {
			if (!childsNodeRefs.contains(nodeRef)
					&& !allNodeRefs.contains(nodeRef)) {
	...
	private void createFolderLinkRepresentation(final NodeRef nodeRef, final List<NodeRef> childsNodeRefs, final List<NodeRef> allNodeRefs) {
		// only display the user has the permissions to navigate to
		// the target of the link
		final NodeRef destRef = (NodeRef) nodeService.getProperty(
				nodeRef, ContentModel.PROP_LINK_DESTINATION);
		if (destRef != null
				&& new Node(destRef)
						.hasPermission(PermissionService.READ) == true) {
			final List<ChildAssociationRef> listChildAssocs = nodeService
					.getChildAssocs(destRef);
			if (!childsNodeRefs.contains(nodeRef)
					&& !allNodeRefs.contains(nodeRef)) {
				childsNodeRefs.add(nodeRef);
			}
			...
			for (final ChildAssociationRef childAssoc : listChildAssocs) {
				tempNodeRefs = (List<NodeRef>) getAllChildsNodeRefs(
						allNodeRefs, childAssoc.getChildRef());
				// remove duplicated
	...
	private void createFolderRepresentation(final NodeRef nodeRef, final List<NodeRef> childsNodeRefs, final List<NodeRef> allNodeRefs) {
	...
	public List<IndexRecord> getMetaData(final List<NodeRef> nodeRefs) {
		final List<IndexRecord> indexRecords = new LinkedList<IndexRecord>();
		IndexRecord indexRecord;
		int rowNumber = 0;
		final List<NodeRef> allNodeRefs = new ArrayList<NodeRef>();
		...
		for (final NodeRef nodeRef : allNodeRefs) {
			...
			if (isContent(nodeRef)) {
				...
				//SecurityRanking
				if(multilingualContentService.isTranslation(nodeRef)) {
					final NodeRef translationContainer = multilingualContentService.getTranslationContainer(nodeRef);
					securityRanking = (String) nodeService.getProperty(translationContainer, DocumentModel.PROP_SECURITY_RANKING);
				} else {
					securityRanking = (String) nodeService.getProperty(nodeRef, DocumentModel.PROP_SECURITY_RANKING);
				}
				//TODO check why securityRanking is null with tranlated document
				if(securityRanking != null && !DocumentModel.SECURITY_RANKINGS.get(0).equals(securityRanking)) {
					//ignore not public security ranking
					continue;
				}
				...
				if (indexRecord != null) {
					...
					final String title = (String) nodeService.getProperty(
							nodeRef, ContentModel.PROP_TITLE);
					if (title != null && title.length() > 0) {
						indexRecord.setTitle(title);
					}

					// Description
					final String description = (String) nodeService
					...
					final Locale docLang = (Locale) nodeService.getProperty(
							nodeRef, ContentModel.PROP_LOCALE);
					if (docLang != null) {
					...
					final String author = (String) nodeService.getProperty(
							nodeRef, ContentModel.PROP_AUTHOR);
					if (author != null && author.length() > 0) {
						indexRecord.setAuthor(author);
					}

					// Keywords
					final List<Keyword> keywords = keywordsService
							.getKeywordsForNode(nodeRef);
					...
					for (final Keyword keyword : keywords) {
						if(keyword.isKeywordTranslated()) {
							...
							for(final Locale locale : locales) {
								...
								keywordBuilder.append(" ").append(keyword.getMLValues().getValue(locale)).append(";");
							}
						} else {
					...
					final String status = (String) nodeService.getProperty(
							nodeRef, DocumentModel.PROP_STATUS);
					...
					final Date issueDate = (Date) nodeService.getProperty(
							nodeRef, DocumentModel.PROP_ISSUE_DATE);
					if (issueDate != null) {
					...
					final String reference = (String) nodeService.getProperty(
							nodeRef, DocumentModel.PROP_REFERENCE);
					...
					if(multilingualContentService.isTranslation(nodeRef)) {
						...
						if(pivotNodeRef == nodeRef) {
							...
							final NodeRef translationContainer = multilingualContentService.getTranslationContainer(nodeRef);
							expirationDate = (Date) nodeService.getProperty(translationContainer, DocumentModel.PROP_EXPIRATION_DATE);
						}
					} else {
						expirationDate = (Date) nodeService.getProperty(nodeRef, DocumentModel.PROP_EXPIRATION_DATE);
					}
					if (expirationDate != null) {
						indexRecord.setExpirationDate(sdf.get()
					...
					if(multilingualContentService.isTranslation(nodeRef)) {
						...
						if(nodeRef.equals(pivot)) {
							...
							if (nodeService.hasAspect(nodeRef,
													  ContentModel.ASPECT_MULTILINGUAL_EMPTY_TRANSLATION)) {
								final String translator = (String) nodeService.getProperty(nodeRef, ContentModel.PROP_AUTHOR);
	...
	public void validateEntries(final List<IndexRecord> indexFileEntries, final List<UploadedEntry> uploadedEntries, final List<ValidationMessage> messages) {
		...
		for(final IndexRecord indexRecord : indexFileEntries) {
			...
			if(!found) {
				if(indexRecord.getNoContent() != null && indexRecord.getNoContent().equalsIgnoreCase("Y")) {
					if(logger.isTraceEnabled()) {
					...
					messages.add(new ValidationMessageImpl(indexRecord
							.getRowNumber(), indexRecord.getName(), I18NUtil.getMessage(FILE_REFERENCED_NOT_PRESENT_IN_ZIP)
							, ErrorType.Fatal));
				}
			}
	...
	private IndexRecord getIndexRecord(final List<IndexRecord> indexRecords,
			final String fileName) {
	...
	private String getFileName(final String relativePath) {
		...
		if (pathElements.length > 1) {
			...
			System.arraycopy(pathElements, 0, folderElements, 0,
	...
	private NodeRef searchFile(final NodeRef parentNodeRef,
		...
		if (pathElements.length > 1) {
			...
			System.arraycopy(pathElements, 0, folderElements, 0,
					pathElements.length - 1);
			final List<String> folderElementsList = Arrays
					.asList(folderElements);
			// fileName
			fileName = pathElements[pathElements.length - 1];
			destinationFolderNodeRef = searchFolder(parentNodeRef,
					folderElementsList);
		} else {
			// Reference to the document directly
			fileName = relativePath;
		...
		if(destinationFolderNodeRef != null) {
			if(logger.isDebugEnabled()) {
				try {
					...
					if(logger.isTraceEnabled()) {
						final List<SimplePath> childs = managementService.getChildsPath(destinationFolderNodeRef);
			...
			fileNodeRef = nodeService.getChildByName(destinationFolderNodeRef, ContentModel.ASSOC_CONTAINS, fileName);
		}
		return fileNodeRef;
	}

	private NodeRef searchFolder(final NodeRef parentNodeRef,

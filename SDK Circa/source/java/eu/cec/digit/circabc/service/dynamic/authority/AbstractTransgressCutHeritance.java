...
public abstract class AbstractTransgressCutHeritance implements DynamicAuthority, InitializingBean {
	...
	protected Map<QName, QName> mandatoryAspects = new HashMap<QName, QName>();
	...
	public void afterPropertiesSet() throws Exception {
		...
		transgressCutInheritancePermRef = modelDAO.getPermissionReference(CircabcModel.ASPECT_CIRCABC_MANAGEMENT,
						permissionToCheck);
	}
	...
	public boolean hasAuthority(final NodeRef nodeRef, final String userName) {
		...
		return AuthenticationUtil.runAs(new RunAsWork<Boolean>() {
			...
			public Boolean doWork() throws Exception {
				NodeRef nodeToEvaluate = nodeRef;
				if (logger.isDebugEnabled())
					logger.debug("***  " + nodeService.getProperty(nodeRef, ContentModel.PROP_NAME) + " (" + nodeRef.getId()
									+ ") " + "   ***  ");
				...
				if(mandatoryAspect == null) {
					if (logger.isDebugEnabled())
						logger.debug("The node doesn't have one of the mandatory haspects:" + mandatoryAspects);
					return false;
				}

				return isAdmin(nodeToEvaluate, userName, authorityService.getContainingAuthorities(AuthorityType.GROUP, userName,
								true), modelDAO.getGrantingPermissions(transgressCutInheritancePermRef), mandatoryAspect);
			}

			public boolean isAdmin(final NodeRef nodeRef, final String userName, final Set<String> containingGroupAuthorities,
							final Set<PermissionReference> grantingPermissions, final QName mandatoryAspect) {
				if (nodeRef == null) {
					return false;
				} else {
					final NodePermissionEntry permissions = permissionsDaoComponent.getPermissions(nodeRef);
					...
					for (final PermissionEntry permissionEntry : permissions.getPermissionEntries()) {
						if (permissionEntry.getAuthority().equals(userName)
							...
							if (logger.isDebugEnabled())
								logger.debug("(" + circabcService + ")" + nodeService.getProperty(nodeRef, ContentModel.PROP_NAME) + " --> "
												+ permissionEntry.getPermissionReference().getName() + " from   "
												+ permissionEntry.getPosition() + " for " + permissionEntry.getAuthority());

							adminOnService = grantingPermissions.contains(permissionEntry.getPermissionReference());
						}

						if (adminOnService == true) {
							break;
						}
					}
					...
					if (adminOnService == true || hasTopAspect == true) {
						return adminOnService;
					} else {
						//recursive Test: Check the parent
						return isAdmin(nodeService.getPrimaryParent(nodeRef).getParentRef(), userName,
										containingGroupAuthorities, grantingPermissions, mandatoryAspect);
					}
				}
	...
	public Set<PermissionReference> requiredFor() {
		if(permissionReference == null) {
			permissionReference = new HashSet<PermissionReference>();
			for(final String permission : this.permissions) {
				permissionReference.addAll(modelDAO.getGranteePermissions(modelDAO.getPermissionReference(CircabcModel.ASPECT_CIRCABC_MANAGEMENT,
								permission)));
			}
			if (logger.isDebugEnabled())

...
import eu.cec.digit.circabc.service.bulk.validation.validator.ExpirDateValidator;
import eu.cec.digit.circabc.service.bulk.validation.validator.IndexValidator;
import eu.cec.digit.circabc.service.bulk.validation.validator.IssueDateValidator;
import eu.cec.digit.circabc.service.bulk.validation.validator.LangValidator;
import eu.cec.digit.circabc.service.bulk.validation.validator.NameValidator;
import eu.cec.digit.circabc.service.bulk.validation.validator.NoContentValidator;
import eu.cec.digit.circabc.service.bulk.validation.validator.OriLangValidator;
import eu.cec.digit.circabc.service.bulk.validation.validator.OverwriteValidator;
import eu.cec.digit.circabc.service.bulk.validation.validator.SecurityRankingValidator;
import eu.cec.digit.circabc.service.bulk.validation.validator.StatusValidator;
...
public static final  List<String> HEADERS_LIST = Arrays.asList(IndexServiceImpl.HEADERS);
	...
	private static final List<IndexValidator> VALIDATORS = new ArrayList<IndexValidator>();
	...
	public void getIndexRecords(final File indexFile, final List<IndexRecord> indexRecords, final List<ValidationMessage> messages) throws IOException {
		...
		final Map<Integer, String> fileHeaderPositions = new HashMap<Integer, String>();
		...
		if((headerRow = reader.readNext()) != null) {
			...
			for(int i = 0; i < headerRow.length; i++) {
				fileHeaderPositions.put(Integer.valueOf(i), headerRow[i]);
				if(logger.isDebugEnabled()) {
			...
			while ((nextRow = reader.readNext()) != null) {
				...
				if(isEmptyLine(nextRow)) {
					...
					messages.add(new ValidationMessageImpl(rowNumber, fileName,
		    	...
		    	for(int i = 0; i < nextRow.length; i++) {
		    		...
		    		if(nextRow[i] != null && nextRow[i].length() != 0) {
		    			if(Headers.NAME.equals(fileHeaderPositions.get(i))) {
			    			...
			    			if(logger.isDebugEnabled()) {
			    				logger.debug("Line:" + rowNumber + " Column:" + (i+1) + " HeaderName:" + headerRow[i] + " Value:" + nextRow[i]);
			    			}
		    			}
	...
	private IndexHeaders getAllHeaders() {
		...
		{
			localIndexHeaders.addHeader(indexHeaders.getHeader(Headers.ATTRIPREFIX + String.valueOf(i+1) ));
		}
	...
	public void generateIndexRecords(final File indexFile, final List<IndexRecord> indexRecords) throws IOException {
		...
		try {
			...
			writer = new CSVWriter(bw, '\t', CSVWriter.NO_QUOTE_CHARACTER, CSVWriter.NO_ESCAPE_CHARACTER, "\r\n");

			final String[] headers = new String[indexHeaders.getHeaders().size()];

			int i=0;
			final StringBuffer allHeaders = new StringBuffer();
			...
			for(final IndexRecord indexRecord : indexRecords) {
				final StringBuffer allValues = new StringBuffer();
				i = 0;
				for(final IndexHeader indexHeader : indexHeaders.getHeaders()) {
					indexEntry = indexRecord.getEntry(indexHeader.getHeaderName());
					if(indexEntry == null || indexEntry.getValue() == null || indexEntry.getValue().length() == 0) {
						...
						if(logger.isDebugEnabled()) {
							logger.debug("Value " + indexEntry.getValue() + " found for header:" + indexHeader.getHeaderName());
						}
	...
	private void validate(final int rowNumber, final IndexRecord indexRecord, final List<ValidationMessage> messages) {

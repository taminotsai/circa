...
import static eu.cec.digit.circabc.model.EventModel.PROP_EVENT_ABSTRACT;
import static eu.cec.digit.circabc.model.EventModel.PROP_EVENT_AUDIENCE;
import static eu.cec.digit.circabc.model.EventModel.PROP_EVENT_DATE;
import static eu.cec.digit.circabc.model.EventModel.PROP_EVENT_EMAIL;
import static eu.cec.digit.circabc.model.EventModel.PROP_EVENT_END_TIME;
import static eu.cec.digit.circabc.model.EventModel.PROP_EVENT_INVITATION_MESSAGE;
import static eu.cec.digit.circabc.model.EventModel.PROP_EVENT_INVITED_USERS;
import static eu.cec.digit.circabc.model.EventModel.PROP_EVENT_LANGUAGE;
import static eu.cec.digit.circabc.model.EventModel.PROP_EVENT_LOCATION;
import static eu.cec.digit.circabc.model.EventModel.PROP_EVENT_NAME;
import static eu.cec.digit.circabc.model.EventModel.PROP_EVENT_OCCURENCE_RATE;
import static eu.cec.digit.circabc.model.EventModel.PROP_EVENT_PHONE;
import static eu.cec.digit.circabc.model.EventModel.PROP_EVENT_START_DATE;
import static eu.cec.digit.circabc.model.EventModel.PROP_EVENT_START_TIME;
import static eu.cec.digit.circabc.model.EventModel.PROP_EVENT_TIMEZONE;
import static eu.cec.digit.circabc.model.EventModel.PROP_EVENT_TITLE;
import static eu.cec.digit.circabc.model.EventModel.PROP_EVENT_URL;
import static eu.cec.digit.circabc.model.EventModel.PROP_KIND_OF_EVENT;
...
public class AppointmentImpl implements Appointment  {
	...
	private List<String> invitedUsers = new ArrayList<String>();
	...
	public Date getEndTimeAsDate() {
		...
		if (date != null)
		{
			return AppointmentUtils.convertTimeOfDayDateValueToDate(endTime, date) ;
		}
		else
		{
			return AppointmentUtils.convertTimeOfDayToDate(endTime) ;
		}
	}
	...
	public Date getStartTimeAsDate() {
		if (date != null)
		{
			return AppointmentUtils.convertTimeOfDayDateValueToDate(startTime, date) ;
		}
		else
		{
			return AppointmentUtils.convertTimeOfDayToDate(startTime) ;
		}
	}
	...
	public void setEndTimeAsDate(Date value) {
		...
		endTime = AppointmentUtils.convertDateToTimeOfDay( value);
	}
	...
	public void setStartTimeAsDate(Date value) {
		...
		startTime = AppointmentUtils.convertDateToTimeOfDay( value);
	}
	...
	public void init(Map<QName, Serializable> properties )
	{
		...
		this.setStartDate(AppointmentUtils.convertDateToDateValue ((Date) properties.get(PROP_EVENT_START_DATE)));

		this.setStartDate(AppointmentUtils.convertDateToDateValue ((Date) properties.get(PROP_EVENT_START_DATE)));

		this.setStartTime(new TimeOfDay( properties.get(PROP_EVENT_START_TIME).toString() ));
		this.setEndTime( new TimeOfDay(properties.get(PROP_EVENT_END_TIME).toString())) ;
		...
		this.audience =  new HashMap<String, MeetingRequestStatus>();
		final MeetingRequestStatus defaultSatus = (this instanceof Meeting) ? MeetingRequestStatus.Pending : MeetingRequestStatus.NotApplicable;
	...
	public List<PropertyMap> getEventDatesProperties(AppointmentType appointmentType)
	{
		...
		final List<PropertyMap> listOfProperties = new ArrayList<PropertyMap>(eventDates.size());
		...
		for (DateValue value : eventDates) {
			...
			Date javaDate = AppointmentUtils.convertTimeOfDayDateValueToDate(this.startTime, value);
	...
	public List<DateValue> getEventDates()
	{
		...
		List<DateValue> result = new ArrayList<DateValue>();
		int limit = LIMIT_OCCURENCE_GENERATION;
		try {
			RecurrenceIterator ri = RecurrenceIteratorFactory.createRecurrenceIterator(rrule,startDate, TimeZone.getTimeZone(timeZoneId) );
	...
	private String getRRuleString() {
		...
		case Times:
			...
			case TuesdayThursday:
				...
				break ;
			case MonthlyByDate :
				result = "RRULE:FREQ=MONTHLY;COUNT="+ occurenceRate.getTimes()+";BYMONTHDAY="+ dayOfMonth;
				break ;
			case MonthlyByWeekday :
				result =  "RRULE:FREQ=MONTHLY;COUNT="+ occurenceRate.getTimes() + ";BYDAY=1" + getWeekDay2Letters(dayOfWeek) ;
				break ;
			case Yearly:
		...
		case EveryTimes:
			...
			case days :
				result =  "RRULE:FREQ=DAILY;INTERVAL="+occurenceRate .getEvery()+";COUNT=" +occurenceRate.getTimes();
				break;
			case weeks:
				result =  "RRULE:FREQ=WEEKLY;INTERVAL="+occurenceRate .getEvery()+";COUNT=" +occurenceRate.getTimes();
				break;
			case months :
				result =  "RRULE:FREQ=MONTHLY;INTERVAL="+occurenceRate .getEvery()+";COUNT=" +occurenceRate.getTimes();
				break;
			default:
				break;
			}

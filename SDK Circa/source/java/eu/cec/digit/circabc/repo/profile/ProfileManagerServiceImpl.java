...
public abstract  class ProfileManagerServiceImpl implements ProfileManagerService
{
	...
	private static  final QName PROP_PROFILE_EXPORTED = QName.createQName(CircabcModel.CIRCABC_CONTENT_MODEL_1_0_URI, "isExported");
	private static  final QName PROP_PROFILE_IMPORTED = QName.createQName(CircabcModel.CIRCABC_CONTENT_MODEL_1_0_URI, "isImported");
	private static  final QName PROP_PROFILE_IMPORTED_REF = QName.createQName(CircabcModel.CIRCABC_CONTENT_MODEL_1_0_URI, "importedNodeRef");
	protected static final String CACHE_NAME = "eu.cec.digit.circabc.service.dynamic.authority.circabcDynamicAuthority";
	 ...
	 */
	public final void setProfileMapCache(final SimpleCache<String, Map<String, Profile>> profileMapCache)
	{
	...
	/**
	 * Return the Aspect QualifiedName. This is used in order to verify that
	 * Service is called on right node
	 *
	 * @return
	 */
	...
	private final QName getProfileAssocQName()
	{
		final QName profileAssocQName = QName.createQName(CIRCABC_CONTENT_MODEL_1_0_URI, getProfilePrefix() + "ProfileAssoc");
		return profileAssocQName;
	}

	private final QName getProfileQName()
	{
		/** node type for the profile */
		final QName profileQName = QName.createQName(CIRCABC_CONTENT_MODEL_1_0_URI, getProfilePrefix() + "Profile");
		return profileQName;
	}

	private final QName getProfileNameQName()
	{
		/** type for the profileName */
		final QName profileQName = QName.createQName(CIRCABC_CONTENT_MODEL_1_0_URI, getProfilePrefix() + "ProfileName");
		return profileQName;
	}

	private final QName getProfileGroupNameQName()
	{
		/** type for the profileGroupName */
		final QName profileGroupNameQName = QName.createQName(CIRCABC_CONTENT_MODEL_1_0_URI, getProfilePrefix()
				+ "ProfileGroupName");
		return profileGroupNameQName;
	}

	/** Relation name between profile and the service */
	private final QName getServiceAssocQName()
	{
		final QName serviceAssocQName = QName.createQName(CIRCABC_CONTENT_MODEL_1_0_URI, getProfilePrefix() + "ServiceAssoc");
		return serviceAssocQName;
	}

	private final QName getServiceQName()
	{
		/** node type for the profile */
		final QName serviceQName = QName.createQName(CIRCABC_CONTENT_MODEL_1_0_URI, getProfilePrefix() + "Service");
		return serviceQName;
	}

	private final QName getServiceNameQName()
	{
		/** type for the serviceName */
		final QName serviceQName = QName.createQName(CIRCABC_CONTENT_MODEL_1_0_URI, getProfilePrefix() + "ServiceName");
		return serviceQName;
	}

	private final QName getPermissionSetQName()
	{
		/** type for the permissionSetQName */
		final QName servicePermissionSetQName = QName.createQName(CIRCABC_CONTENT_MODEL_1_0_URI, getProfilePrefix()
				+ "PermissionSet");
	...
	private NodeRef createProfileNode(final NodeRef nodeRef, final Profile profile)
	{
		...
		if (!CircabcConstant.GUEST_AUTHORITY.equalsIgnoreCase(profile.getProfileName()))
		{
			...
			{
				prefixedProfileGroupName = authorityService.getName(AuthorityType.GROUP, profile.getAlfrescoGroupName());
			}
			else
			{
		...
		contentProps.put(PROP_PROFILE_IMPORTED, profile.isImported());
		contentProps.put(PROP_PROFILE_EXPORTED, profile.isExported());
		contentProps.put(PROP_PROFILE_IMPORTED_REF, profile.getImportedNodeRef());
		...
		final ChildAssociationRef profileAssociation = nodeService.createNode(nodeRef, getProfileAssocQName(), QName.createQName(
				CIRCABC_CONTENT_MODEL_1_0_URI, profile.getProfileName()), getProfileQName(), contentProps);
	...
	private NodeRef updateProfileNode(final NodeRef nodeRef, final Profile profile)
	{
		...
		else
		{
			...
			try
			{
				...
				{
					// the stored group name is the prefixed name with GROUP_
					//prefixedProfileGroupName = authorityService.getName(AuthorityType.GROUP, profile.getProfileName());
				...
				final String curentPrefixedProfileGroupName = (String) nodeService.getProperty(profileNodeRef, getProfileGroupNameQName());

				if (curentPrefixedProfileGroupName != null && prefixedProfileGroupName.equals(curentPrefixedProfileGroupName))
				{
				...
				else
				{
					...
					nodeService.setProperty(profileNodeRef, getProfileGroupNameQName(), prefixedProfileGroupName);
				}
	...
	{
		final List<ChildAssociationRef> listOfProfilesAssoc = nodeService.getChildAssocs(nodeRef, getProfileAssocQName(),
	...
	{
		final Map<String, NodeRef> profileNodes = new HashMap<String, NodeRef>();
		final List<ChildAssociationRef> listOfProfilesAssoc = nodeService.getChildAssocs(nodeRef, getProfileAssocQName(),
				RegexQNamePattern.MATCH_ALL);
		NodeRef profileNodeRef;
		String profileName;
	...
	{
		final List<NodeRef> profileNodes = new ArrayList<NodeRef>();
		final List<ChildAssociationRef> listOfProfilesAssoc = nodeService.getChildAssocs(nodeRef, getProfileAssocQName(),
				RegexQNamePattern.MATCH_ALL);
		NodeRef profileNodeRef;
		for (final ChildAssociationRef profileAssoc : listOfProfilesAssoc)
	...
	// Service
	private NodeRef createServiceNode(final NodeRef profileNodeRef, final String serviceName, final Set<String> permissionSet)
	{
		final Map<QName, Serializable> contentProps = new HashMap<QName, Serializable>();
		contentProps.put(getServiceNameQName(), serviceName);
		final ArrayList<String> permissionList = new ArrayList<String>(permissionSet);
		...
		final ChildAssociationRef serviceAssociation = nodeService.createNode(profileNodeRef, getServiceAssocQName(), QName
				.createQName(CIRCABC_CONTENT_MODEL_1_0_URI, serviceName), getServiceQName(),
				contentProps);
		return serviceAssociation.getChildRef();
	}

	private NodeRef updateServiceNode(final NodeRef nodeRef, final String serviceName, final Set<String> permissionSet)
	{
	...
	{
		final List<ChildAssociationRef> listOfServiceAssoc = nodeService.getChildAssocs(profileNodeRef, getServiceAssocQName(),
				RegexQNamePattern.MATCH_ALL);
	...
	{
		final Map<String, NodeRef> serviceNodes = new HashMap<String, NodeRef>();
		final List<ChildAssociationRef> listOfServicesAssoc = nodeService.getChildAssocs(profileNodeRef, getServiceAssocQName(),
				RegexQNamePattern.MATCH_ALL);
		NodeRef serviceNodeRef;
		String serviceName;
	...
	private Set<String> getServicePermissionSet(final NodeRef profileNodeRef, final String serviceName)
	{
		...
		final Set<String> permissionSet = new HashSet<String>(permissionList);
		return permissionSet;
	}

	private Map<String, Set<String>> getServicePermissionSet(final NodeRef profileNodeRef,
			final Map<String, NodeRef> servicesNodeRef)
	{
		final Map<String, Set<String>> servicePermissionSet = new HashMap<String, Set<String>>();
		// NodeRef serviceNodeRef;
	 ...
	 *
	 */
	public void addProfile(final NodeRef nodeRef, final String profileName, final String authorityGroupName, final MLText title, final MLText description, final Map<String, Set<String>> servicesPermissions)
			throws ProfileException
	{
		addProfileImpl(nodeRef, profileName, authorityGroupName, title, description, servicesPermissions, false, null);
	}
	 ...
	 *
	 */
	private Profile addProfileImpl(final NodeRef nodeRef, final String profileName, final String authorityGroupName, final MLText title, final MLText description, final Map<String, Set<String>> servicesPermissions, final boolean imported, final NodeRef fromIgNoderef)
			throws ProfileException
	{
		profileMapCache.remove(nodeRef.toString());
		Profile profile = null;
		...
		else
		{
			...
			profile = new ProfileImpl(nodeRef, profileName, authorityGroupName, this.getInvitedUsersGroupName(nodeRef), title, description, serviceRegistry, circabcRootProfileManagerService);

			if(imported)
			{
				profile.setImported(true);
				profile.setImportedNodeRef(fromIgNoderef);
				profile.setImportedNodeName((String)nodeService.getProperty(fromIgNoderef, ContentModel.PROP_NAME));
			}
	 ...
	 *
	 */
	public void addProfile(final NodeRef nodeRef, final String profileName, final String authorithyGroupName, final Map<String, Set<String>> servicesPermissions)
			throws ProfileException
	{
	...
	protected Map<String, Set<String>> filterServicesForNode(final Map<String, Set<String>> servicesPermissions)
	{
		final Map<String, Set<String>> filteredServicesPermissions = new HashMap<String, Set<String>>();
	...
	public void deleteProfile(final NodeRef nodeRef, final String profileName) throws ProfileException
	{
		...
		else
		{
			...
			{
				final Set<String> authorithyContainedInProfile = authorityService
					.getContainedAuthorities(null, alfrescoGroupName, true);
	...
	public Profile getProfile(final NodeRef nodeRef, final String profileName)
	{
		...
		if (profile == null)
		{
			throw new ProfileException(profileName, "Profile not exist " + " for profiles: " + profileMap);
		}
		else
		{
			return profile;
	...
	abstract public List<Profile> getProfilesRecursivly(final NodeRef nodeRef);
	...
	private Profile createProfile(final NodeRef profileNodeRef)
	{
		...
		try
		{
			...
			try
			{
				final Serializable value = properties.get(PROP_PROFILE_IMPORTED);
				if(value != null)
				{
			...
			{
				if(logger.isErrorEnabled()) logger.error("isImported can't be evaluated on node:" + profileNodeRef, e);
			}
			...
			try
			{
				final Serializable value = properties.get(PROP_PROFILE_IMPORTED_REF);
				if(value != null)
				{
					importedNodeRef = (NodeRef) value;
					profile.setImportedNodeRef(importedNodeRef);
					profile.setImportedNodeName((String)nodeService.getProperty(importedNodeRef, ContentModel.PROP_NAME));
				}
			}
			catch(final Exception e)
			{
				if(logger.isErrorEnabled()) logger.error("importedNodeRef can't be evaluated on node:" + profileNodeRef, e);
			}

			try
			{
				final Serializable value = properties.get(PROP_PROFILE_EXPORTED);
				if(value != null)
				{
			...
			{
				if(logger.isErrorEnabled()) logger.error("isExported can't be evaluated on node:" + profileNodeRef, e);
			}
			...
			{
				curentProfileGroupName = curentPrefixedProfileGroupName.substring(GROUP_UNDESCORE.length(), curentPrefixedProfileGroupName
						.length());
	...
	public void renameProfile(final NodeRef nodeRef, final String oldProfileName, final String newProfileName)
			throws ProfileException
	{
		profileMapCache.remove(nodeRef.toString());
		...
		if (logger.isDebugEnabled())
		{
			logger.debug("renameProfile(oldName:" + oldProfileName + ", newName:" + newProfileName + ", node:" + nodeRef);
		}
	}
	...
	public void addApplicantPerson(final NodeRef nodeRef, final String userID, final String message, final String firstName, final String lastName)
		throws ProfileException
	{
		addApplicantPerson(nodeRef, userID, message, firstName, lastName, new Date());
	}

	public void addApplicantPerson(final NodeRef nodeRef, final String userID, final String message, final String firstName, final String lastName, final Date date)
			throws ProfileException
	{
		if (hasApplicantFeature())
		{
			// get the applicantMap and add the user in it
			Map<String, Applicant> applicantMap = (Map<String, Applicant>) nodeService.getProperty(nodeRef,
					getApplicantUsersMapQName());
			if (applicantMap == null)
			{
				applicantMap = new HashMap<String, Applicant>();
			}
			...
			if (logger.isDebugEnabled())
			{
				...
				logger.debug("The user " + userID + " has requested to become a member of the interest group '" + igName + "'");
			}
		}
	}
	...
	public void removeApplicantPerson(final NodeRef nodeRef, final String userID) throws ProfileException
	{
		if (hasApplicantFeature())
		{
			...
			final Map<String, Applicant> applicantMap = (Map<String, Applicant>) nodeService.getProperty(nodeRef,
					getApplicantUsersMapQName());
			...
			if (logger.isDebugEnabled())
			{
				...
				logger.debug("The application of " + userID + " has been refused to become a member of '" + igName + "'");
			}
		}
	}

	public final void addPersonToProfile(final NodeRef nodeRef, final String userID, final String profileName)
			throws ProfileException
	{
		...
		final CircabcRootProfileManagerService circabcRootProfileManagerService = profileManagerServiceFactory.getCircabcRootProfileManagerService();
		if (personProfile != null && !personProfile.equalsIgnoreCase(circabcRootProfileManagerService.getAllCircaUsersGroupName()) )

			throw new ProfileException(profileName, "User " + userID + " have already a profile in the group");
		...
		{
			throw new ProfileException(profileName, "addPersonToProfile failed because profile name doesn't exist on the node");
		}

		if (logger.isDebugEnabled())
		{
			logger.debug("addPersonToProfile(userID:" + userID + ", profileName:" + profileName + " node:" + nodeRef);
		}
	}

	public final void changePersonProfile(final NodeRef nodeRef, final String userID, final String targetProfileName)
			throws ProfileException
	{
		if (getProfilePrefix() == null)
		{
		...
		if (profile != null)
		{
			...
			if (originalProfileName == null)
			{
				...
				// any
				// profile
				throw new ProfileException(targetProfileName, "movePerson failed because user " + userID + " is not invited yet on the node:" + nodeRef);
			}
			final String originalProfileGroupName = this.getProfile(nodeRef, originalProfileName).getPrefixedAlfrescoGroupName();
			if (originalProfileGroupName == null)
			{
				// that mean that the user can not be moved becaus he wasn't in
				// any
				// profile
				throw new ProfileException(targetProfileName, "movePerson failed because original group wasn't found");
			}
		...
		{
			throw new ProfileException(targetProfileName,
					"addPersonToProfile failed because profile name doesn't exist on the node");
		}

		if (logger.isDebugEnabled())
		{
			logger.debug("changePersonProfile(userID:" + userID + ", targetProfileName" + targetProfileName + " node:" + nodeRef);
		}
	}
	...
	public void uninvitePerson(final NodeRef nodeRef, final String userID) throws ProfileException
	{
		...
		// TODO Verify if Alfresco don't do already this
	...
	public final String getPersonProfile(final NodeRef nodeRef, final String userID)
	{
		...
		final Set<String> containingAuthoritiesForUser = authorityService.getContainingAuthorities(AuthorityType.GROUP, userID, true);

		final Map<String, Profile> profileMap = getProfileMap(nodeRef);
		final CircabcRootProfileManagerService circabcRootProfileManagerService = profileManagerServiceFactory.getCircabcRootProfileManagerService();
		final String allCircaUsersGroupName = circabcRootProfileManagerService.getAllCircaUsersGroupName();
		...
		if (logger.isDebugEnabled())
		{
			logger.debug("return: " + personProfile + " getPersonProfile(userID:" + userID + " node:" + nodeRef);
		}
	...
	public final String getPersonProfileGroup(final NodeRef nodeRef, final String userID)
	{
		...
		// retrieve what was the original profile of the user
		final Set<String> containingAuthoritiesForUser = authorityService.getContainingAuthorities(AuthorityType.GROUP, userID,
				true);
		...
		for (final Profile profile : profileMap.values())
		{
			if (containingAuthoritiesForUser.contains(profile.getPrefixedAlfrescoGroupName()))
			{
				if (logger.isDebugEnabled())
				{
					logger.debug("return: " + profile.getPrefixedAlfrescoGroupName() + " getPersonProfileGroup(userID:" + userID
							+ " node:" + nodeRef);
				}
		...
		if (logger.isDebugEnabled())
		{
			logger.debug("return: " + null + " getPersonProfileGroup(userID:" + userID + " node:" + nodeRef);
		}
		return null;
	}

	/**
	 * try to find the parent node (recursivly) who has the current Aspect
	 * Current Aspect depend on the Implementation of this class (IGRoot,
	 * Category, CircaBC, ...)
	 */
	...
	public final Set<String> getPersonInProfile(final NodeRef nodeRef, final String profileName)
	{
		...
		Set<String> personsInProfile = new HashSet<String>();
		for (final Profile profile : profileMap.values())
		{
			if (profileName.equals(profile.getProfileName()))
			{
				if (logger.isDebugEnabled())
				{
					logger.debug("find: " + profile.getProfileName() + " group: " + profile.getPrefixedAlfrescoGroupName()
							+ " on node:" + nodeRef);
				}
				if (profileManagerServiceFactory.getCircabcRootProfileManagerService().getPrefixedAllCircaUsersGroupName().equals(profile.getPrefixedAlfrescoGroupName())
				...
				if(authorityService.authorityExists(profile.getPrefixedAlfrescoGroupName())) {
					personsInProfile = authorityService.getContainedAuthorities(AuthorityType.USER, profile.getPrefixedAlfrescoGroupName(), true);
					break;
				} else {
					if(logger.isWarnEnabled())
					{
	...
	public String createMasterGroup(final NodeRef nodeRef) {
		...
		if (hasMasterParentGroup()) {
			final NodeRef parentNodeRef = nodeService.getPrimaryParent(nodeRef).getParentRef();
			final ProfileManagerService parentProfileManagerService = getProfileManagerServiceFactory().getProfileManagerService(
					parentNodeRef);
			...
			if (parentSubsGroupName != null) {
				...
				prefixedParentSubsGroupName = authorityService.getName(AuthorityType.GROUP, parentSubsGroupName);
			}
		}
		
		// get name of the folder
		final String folderName = ((String) nodeService.getProperty(nodeRef, ContentModel.PROP_NAME)).replaceAll(" ", "");
		...
		String createdAuthority = authorityService.createAuthority(AuthorityType.GROUP, masterGroupName, masterGroupName, authorityService.getDefaultZones());
		
		if (prefixedParentSubsGroupName != null && createdAuthority != null) {
			String prefixedMasterGroupName = authorityService.getName(AuthorityType.GROUP, masterGroupName);
	...
	/**
	 * Create a new GROUP of InvitedUsers inside the "Parent" RegistredUsers (if
	 * exist)
	 *
	 * @param nodeRef
	 * @return
	...
	public String createInvitedUsersGroup(final NodeRef nodeRef) {
		...
		// Migration 3.1 -> 3.4.6 - 09/12/2011
		// createAuthority() method changed for version 3.4 
		String createdAuthority = authorityService.createAuthority(AuthorityType.GROUP, invitedUsersGroupName, invitedUsersGroupName, authorityService.getDefaultZones());
		
		if (createdAuthority != null) {
			String prefixedInvitedUsersGroupName = authorityService.getName(AuthorityType.GROUP, invitedUsersGroupName);
	...
	/**
	 * Create a new GROUP of RegistredUsers inside the "Parent" RegistredUsers
	 * (if exist)
	 *
	 * @param nodeRef
	 * @return
	...
	public String createSubsGroup(final NodeRef nodeRef) {
		...
		// Migration 3.1 -> 3.4.6 - 09/12/2011
		// createAuthority() method changed for version 3.4 
		String createdAuthority = authorityService.createAuthority(AuthorityType.GROUP, subsGroupName, subsGroupName, authorityService.getDefaultZones());
		
		if (createdAuthority != null) {
			String prefixedInvitedUsersGroupName = authorityService.getName(AuthorityType.GROUP, subsGroupName);
			authorityService.addAuthority(prefixedMasterGroupName, prefixedInvitedUsersGroupName);
		}
	...
	/**
	 * Return a Set with users that are related to this node Invited + SubsGroup
	 * Users
	 *
	 * @param nodeRef
	 * @return
	...
	public final Set<String> getMasterUsers(final NodeRef nodeRef)
	{
		...
		final String prefixedUserGroupName = authorityService.getName(AuthorityType.GROUP, userGroupName);
		Set<String> usersSet = Collections.emptySet();

		if(authorityService.authorityExists(prefixedUserGroupName))  
		{
			usersSet = authorityService.getContainedAuthorities(AuthorityType.USER, prefixedUserGroupName, false);
		}
		else
		{
	...
	public final Set<String> getInvitedUsers(final NodeRef nodeRef)
	{
		...
		{
			usersSet = cachedUserSet;
		}
		return usersSet;
	}
	public Map<String, Profile> getInvitedUsersProfiles(NodeRef rootNode)
	{
		Map<String, Profile> resultMap = new HashMap<String, Profile>();
		...
		if(authorityService.authorityExists(prefixedUserGroupName))  
		{
			groupSet = authorityService.getContainedAuthorities(AuthorityType.GROUP, prefixedUserGroupName, true);
		}
		else
		{
			if (logger.isErrorEnabled())
			{
				logger.error("Error get contained authorities for group " + prefixedUserGroupName + " authority does not exists" );
			}
		}
		for (String group : groupSet)
		{
			if(authorityService.authorityExists(prefixedUserGroupName))  
			{
				Set<String>	usersSet = authorityService.getContainedAuthorities(AuthorityType.USER, group, true);
	...
	public final Set<String> getAllSubUsers(final NodeRef nodeRef) {
		...
		final ProfileManagerService parentProfileManagerService = getProfileManagerServiceFactory().getProfileManagerService(parentNodeRef);
		...
		if (authorityService.authorityExists(prefixedUserGroupName))
		{
			parentSubsGroupUsersSet =authorityService.getContainedAuthorities(AuthorityType.USER, prefixedUserGroupName, false);
		}
		else
		{
			if (logger.isErrorEnabled())
	...
	{
		final String allUsersGroupName = (String) nodeService.getProperty(nodeRef, getMasterGroupQName());
	...
	{
		final String invitedUsersGroupName = (String) nodeService.getProperty(nodeRef, getInvitedUsersGroupQName());

		if (logger.isDebugEnabled())
		{
			logger.debug("return:" + invitedUsersGroupName + " + getInvitedUsersGroupName(node:" + nodeRef);
		}
		return invitedUsersGroupName;
	}
	...
	{
		final String applicantGroupName = (String) nodeService.getProperty(nodeRef, getApplicantUsersMapQName());

		if (logger.isDebugEnabled())
		{
			logger.debug("return:" + applicantGroupName + " + getApplicantUsersGroupName(node:" + nodeRef);
		}
	...
	public final QName getMasterGroupQName()
	{
		...
		final QName allUsersGroup = QName.createQName(CIRCABC_CONTENT_MODEL_1_0_URI, getProfilePrefix() + "MasterGroup");
	...
	public final QName getInvitedUsersGroupQName()
	{
		...
		final QName invitedUsersGroup = QName.createQName(CIRCABC_CONTENT_MODEL_1_0_URI, getProfilePrefix()
				+ "InvitedUsersGroup");
	...
	public final QName getSubsGroupQName()
	{
		...
		final QName subsGroup = QName.createQName(CIRCABC_CONTENT_MODEL_1_0_URI, getProfilePrefix() + "SubsGroup");
		return subsGroup;
	}
	...
	public final QName getApplicantUsersMapQName()
	{
		...
		final QName applicantUsersGroup = QName.createQName(CIRCABC_CONTENT_MODEL_1_0_URI, getProfilePrefix()
				+ "ApplicantUsersProperty");
	...
	abstract public String getProfilePrefix();
	...
	public List<Profile> getExportedProfiles(final NodeRef nodeRef)
	{
		...
		final List<Profile> exportedProfiles = new ArrayList<Profile>();
	...
	private List<Profile> getImportedProfilesRecursivly(final NodeRef nodeRef)
	{
		...
		final List<Profile> importedProfiles = new ArrayList<Profile>();
		for(final Profile profile : allProfiles) {
			if(profile.isImported()) {
	...
	public void importProfile(final NodeRef toIgNoderef, final NodeRef fromIgNoderef, final String fromProfileName, final Map<String, Set<String>> servicesPermissions)
	{
		...
		final Profile prof = addProfileImpl(toIgNoderef,
				fromIgName + "_" + targetProfile.getProfileName(),
				targetProfile.getPrefixedAlfrescoGroupName(),
				targetProfile.getTitle(),
				targetProfile.getDescription(), servicesPermissions, true, fromIgNoderef);

		NodeRef childRef;
		ProfileManagerService childService;
		for(final ChildAssociationRef assoc: nodeService.getChildAssocs(toIgNoderef, ContentModel.ASSOC_CONTAINS, RegexQNamePattern.MATCH_ALL))
		{
		...
		if(profile != null)
		{
			...
			else
			{
				if(export)
				{
					...
					{
						final NodeRef profileNodeRef = getProfileNode(nodeRef, profile.getProfileName());
						nodeService.setProperty(profileNodeRef, PROP_PROFILE_EXPORTED, Boolean.TRUE);
				...
				else
				{
					if(profile.isExported() == true)
					{
						...
						{
							throw new ProfileException(profileName, "Impossible to unexport: the Profile is imported");
						}
						else
						{
							final NodeRef profileNodeRef = getProfileNode(nodeRef, profile.getProfileName());
							nodeService.setProperty(profileNodeRef, PROP_PROFILE_EXPORTED, Boolean.FALSE);
	...
	public void addProfileTitles(final NodeRef nodeRef, final String profileName, final MLText newTitles)
	{
		final Profile profile = getProfile(nodeRef, profileName);
	...
	public void addProfileDescriptions(final NodeRef nodeRef, final String profileName, final MLText newDescriptions)
	{
		final Profile profile = getProfile(nodeRef, profileName);
		...
		final NodeRef profileNodeRef = getProfileNode(nodeRef, profile.getProfileName());
		nodeService.setProperty(profileNodeRef, ContentModel.PROP_DESCRIPTION, profile.getDescription());
	...
	public List<NodeRef> getRefInExportedProfile(final NodeRef nodeRef, final String profileName)
	{
		...
		final List<NodeRef> refs = new ArrayList<NodeRef>();
	...
	{
		resetCache(CACHE_NAME);
	}

	private void resetCache(final String cacheName)
	{
		final Cache cache = dynamicAuthorithiesCacheManager.getCache(cacheName);
		...
		if(cache != null)
		{
			...
			if(logger.isDebugEnabled())
			{
				logger.debug("Cache " + cacheName + " successfuly reseted");
			}
		}
		else
		{
			logger.error("Cache " + cacheName + " not found !!!");
		}
	}

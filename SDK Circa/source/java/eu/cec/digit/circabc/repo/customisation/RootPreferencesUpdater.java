...
public class RootPreferencesUpdater
{
    ...
    /*package*/ void updateSpace(final NodeRef rootSpace)
    {
        ...
        ParameterCheck.mandatoryString("The config condition (evaluator='string-compare' condition='??')", configCondition);
        ...
        {
            throw new ConfigException("Impossible to update space: " + rootSpace + " with the config condition " + configCondition + ". " + e.getMessage(), e);
        }

    }

    private void createNodes(final NodeRef parentSpace, final Collection<ConfigElement> configElements) throws Exception
    {
        ...
        for(final ConfigElement element: configElements)
        {
            ...
            if(elementName.equals(ELEM_SPACE))
            {
                final String name = element.getAttribute(ATTR_NAME);
                if(isNameValid(name))
                {
                    NodeRef spaceRef = nodeService.getChildByName(parentSpace, ContentModel.ASSOC_CONTAINS, name);

                    if(spaceRef == null)
                    {
                        final FileInfo fileInfo = fileFolderService.create(parentSpace, name, ContentModel.TYPE_FOLDER);
                        spaceRef = fileInfo.getNodeRef();

                        if(logger.isDebugEnabled())
                        {
                            logger.debug("Space '" + name + "' successfully create under " + nodeService.getProperty(parentSpace, ContentModel.PROP_NAME) + ". With the reference " + spaceRef);
                        }
                    }
                    else
                    {
                        if(logger.isDebugEnabled())
                        {
                            logger.debug("Space '" + name + "' successfully found under " + nodeService.getProperty(parentSpace, ContentModel.PROP_NAME));
                        }
                    }
            ...
            else if(elementName.equals(ELEM_FILE))
            {
                final String name = element.getAttribute(ATTR_NAME);
                if(isNameValid(name))
                {
                    NodeRef contentRef = nodeService.getChildByName(parentSpace, ContentModel.ASSOC_CONTAINS, name);
                    final boolean newContent = contentRef == null;
                    if(newContent)
                    {
                        final FileInfo fileInfo = fileFolderService.create(parentSpace, name, ContentModel.TYPE_CONTENT);
                        contentRef = fileInfo.getNodeRef();

                        if(logger.isDebugEnabled())
                        {
                            logger.debug("Content '" + name + "' successfully create under " + nodeService.getProperty(parentSpace, ContentModel.PROP_NAME) + ". With the reference " + contentRef);
                        }
                    }
                    else
                    {
                        if(logger.isDebugEnabled())
                        {
                            logger.debug("Content '" + name + "' successfully found under " + nodeService.getProperty(parentSpace, ContentModel.PROP_NAME));
                        }
                    }
                    ...
                    if(forceUpdate == false)
                    {
                        ...
                        if(revision == null)
                        {
                        	throw new ConfigException("In force update is null or false, a revision number is mandatory (attribute 'revision')");
                        }
                        if (revision.intValue() < 1)
                        {
                        	throw new ConfigException("The revision number is invalid. Found: " + revision + " and any strictly positive number expected.");
                        }
                    }
                    ...
                    if(newContent || forceUpdate || updateContent(contentRef, revision))
                    {
                        ...
                        final ContentWriter writer = contentService.getWriter(contentRef, ContentModel.PROP_CONTENT, true);
                        ...
                        {
                        	permissionService.setPermission(contentRef, CircabcConstant.GUEST_AUTHORITY, guestPerm, true);
                        }

                        if(isNameValid(registredPerm))
                        {
                        	permissionService.setPermission(contentRef, CircabcRootProfileManagerService.ALL_CIRCA_USERS_AUTHORITY, registredPerm, true);
                        }
                    }
                    else
                    {
    ...
    private void updateRevision(Integer revision, String name, NodeRef contentRef)
	{
		if(revision != null)
		{
			final Map<QName, Serializable> revisionProps = Collections.singletonMap(CircabcModel.PROP_REVISION_NUMBER, (Serializable) revision);
			...
			if(nodeService.hasAspect(contentRef, CircabcModel.ASPECT_REVISIONABLE))
			{
				nodeService.addProperties(contentRef, revisionProps);
				 ...
				 if(logger.isDebugEnabled())
	             {
					 logger.debug("Content '" + name + "' is already revisionable, the revision number is updated: " + revision);
	             }
			}
			else
			{
				...
				if(logger.isDebugEnabled())
	            {
					logger.debug("Content '" + name + "' successfully setted as revisionable with the revision number: " + revision);
	            }
			}
    ...
    {
		if(revision == null || revision < 1 || nodeService.hasAspect(contentRef, CircabcModel.ASPECT_REVISIONABLE) == false)
		{
			return true;
		}
		else
		{
			final Integer oldRevision = (Integer) nodeService.getProperty(contentRef, CircabcModel.PROP_REVISION_NUMBER);
     ...
     */
    private void applyInlineEditable(final ConfigElement element, final String name, NodeRef contentRef) throws InvalidNodeRefException, InvalidAspectException
    {
        if(isTrue(element.getAttribute(ATTR_EDITONLINE)))
        {
            ...
            {
                nodeService.addAspect(contentRef,
                        ApplicationModel.ASPECT_INLINEEDITABLE,
                        Collections.<QName, Serializable>singletonMap(ApplicationModel.PROP_EDITINLINE, (Serializable) Boolean.TRUE));
     ...
     * @throws InvalidAspectException
     */
    private void applyVersionable(final ConfigElement element, final String name, NodeRef contentRef) throws InvalidNodeRefException, InvalidAspectException
    {

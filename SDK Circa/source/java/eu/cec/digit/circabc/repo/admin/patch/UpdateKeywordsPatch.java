...
import org.alfresco.repo.admin.patch.impl.BaseReindexingPatch;
...
public class UpdateKeywordsPatch extends BaseReindexingPatch
{
	...
	{
		 private static final String HBM_SELECT_QNAME = " select qname from "
		       + QNameEntity.class.getName() + " as qname "
		       + " join qname.namespace as ns "
		       + " where qname.localName = :localName"
		       + " and ns.safeUri = :uri";


		 private static final String HBM_SELECT_NODES = "select node from "
		       + NodeImpl.class.getName() + " as node "
			 ...
			 HibernateCallback callback = new HibernateCallback()
			 {
				 ...
				 public Object doInHibernate(Session session)
				 {
					 ...
					 if(qnameEntity != null)
					 {
	                	 ...
	                	 for (Node node : nodes)
						 {
							final NodePropertyValue value = (NodePropertyValue) node.getProperties().get(qnameIDString);
	                    	 if(value != null && !isNodeRefOrListOf(value))
	                    	 {
	                    		 NodePropertyValue newPropertyValue = new NodePropertyValue(DataTypeDefinition.NODE_REF, (Serializable) new ArrayList<NodeRef>());
...
//	                    		 pmk.setQnameId(qnameId);
		                    	 ...
		                    	 if(UpdateKeywordsPatch.logger.isDebugEnabled())
		                    	 {
		                    		 UpdateKeywordsPatch.logger.debug("  --  The keyword of the node " + node.getNodeRef() + " has been updated "
		                    				 + "\n\tOld value " + value
                     ...
                     return Integer.valueOf(modifiedElement);
				 }
			 };
			 ...
			 if(UpdateKeywordsPatch.logger.isDebugEnabled())
        	 {
        		 UpdateKeywordsPatch.logger.debug(updateCount + " keyword was badly setted... The model is now respected.");
        	 }

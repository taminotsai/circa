...
public class MailPreferencesServiceImpl implements MailPreferencesService
{
	...
	public void addMailTemplate(final NodeRef ref, final MailTemplate forTemplate, final String name, final String body, final String subject, final Locale language) throws  CustomizationException
	{
		...
		try
		{
			...
			if(nodePreferencesService.customizationFileExists(ref, TEMPLATES_ROOT_FOLDER, MAILS_TEMPLATE_FOLDER, forTemplate.getTemplateDirectoryName(), cleanName))
			{
				final NodeRef existingRef = nodePreferencesService.getCustomization(ref, TEMPLATES_ROOT_FOLDER, MAILS_TEMPLATE_FOLDER, forTemplate.getTemplateDirectoryName(), name);

				Map<Locale, NodeRef> translations = multilingualContentService.getTranslations(existingRef);
				if(translations.containsKey(language))
				{
					throw new CustomizationException("A template with name " + name + " and language " + language + " already exists.");
				}
				...
				while(nodePreferencesService.customizationFileExists(ref, TEMPLATES_ROOT_FOLDER, MAILS_TEMPLATE_FOLDER, forTemplate.getTemplateDirectoryName(), name))
				{
					translationName = computeName(translationName, ++tries, language);
				}

				mailRef = nodePreferencesService.addCustomizationFile(ref,
						TEMPLATES_ROOT_FOLDER,
						MAILS_TEMPLATE_FOLDER,
						forTemplate.getTemplateDirectoryName(),
						translationName, body);
		...
		{
			throw new IllegalStateException("Problem during adding mail with name " + forTemplate.getTemplateDirectoryName() + "/" + name + ". Is the datadictionary customization folder up to date?");
		}
	}

	public void replaceDefaultMailTemplate(final NodeRef ref, final MailTemplate forTemplate, final String body, final String subject, final Locale language) throws CustomizationException
	{
		addMailTemplate(ref, forTemplate, forTemplate.getDefaultTemplateName(), body, subject, language);
	}
	...
	public List<MailWrapper> getMailTemplates(final NodeRef ref, final MailTemplate mailTemplate)
	{
		...
		{
			logger.debug("Trying to retreive the " + mailTemplate + " mail templates for the node " + ref + ".");
		}

		try
		{
			final List<NodeRef> templatesNodes = nodePreferencesService.getConfigurationFiles(ref,
					TEMPLATES_ROOT_FOLDER,
					MAILS_TEMPLATE_FOLDER,
					mailTemplate.getTemplateDirectoryName());

			final Map<NodeRef, MailWrapper> mailMap = new HashMap<NodeRef, MailWrapper>(templatesNodes.size());
			MailWrapper wrapper;
			...
			final List<MailWrapper> mails = new ArrayList<MailWrapper>(mailMap.size());
		...
		{
			throw new IllegalStateException("Problem during getting mails in folder: " + mailTemplate.getTemplateDirectoryName() + ". Is the datadictionary customization folder up to date?", ex);
		}
	 ...
	 */
	private MailWrapper computeWrapper(final MailTemplate mailTemplate, final NodeRef nodeRef) throws InvalidNodeRefException
	{
		MailWrapper wrapper;
		...
		else if(multilingualContentService.getPivotTranslation(nodeRef).equals(nodeRef) == true)
		{
			...
			wrapper = new MailWrapperImpl(multilingualContentService.getTranslations(nodeRef), mailTemplate, templateService, nodeService, pivot);
		}
		else
		{
	...
	public MailWrapper getDefaultMailTemplate(final NodeRef ref, final MailTemplate mailTemplate)
	{
		...
		if(logger.isDebugEnabled())
		{
			logger.debug("Trying to retreive the default " + mailTemplate + " mail templates for the node " + ref + ".");
		}
		...
		try
		{
			...
			{
				throw new CustomizationException("No default mail mail template in folder: " + mailTemplate.getTemplateDirectoryName() + ". Is the datadictionary customization folder up to date?");
			}
			...
			if(logger.isDebugEnabled())
			{
				logger.debug(wrapper + " is the default mail template for template: " + mailTemplate.getTemplateDirectoryName() );
			}
	...
	public Map<String, Object> buildDefaultModel(final NodeRef currentRef, final NodeRef otherPerson, final TemplateImageResolver imageResolver)
    {
    	final Map<String, Object> model = new HashMap<String, Object>(12, 1.0f);
        ...
        if(currentRef != null)
        {
        	...
        	if (ContentModel.TYPE_FOLDER.equals(type) || dictionaryService.isSubClass(type, ContentModel.TYPE_FOLDER))
        	{
        		model.put(MailTemplate.KEY_SPACE, currentRef);
            }
        	else if (ContentModel.TYPE_CONTENT.equals(type) || dictionaryService.isSubClass(type, ContentModel.TYPE_CONTENT))
            {
	...
	/* TODO replace the creation of a template User by a custom representation of the TemplateNode object.
	 */
	...
	public NodeRef getDisclamerLogo(final NodeRef ref)
	{
		...
		try
		{
			...
			if(templateNodeRef == null)
			{
				throw new CustomizationException("No default mail disclamer logo in folder: " + DISCLAMER_FOLDER  + "/" + LOGO_FOLDER + ". Is the datadictionary customization folder up to date?");
			}
		...
		{
			throw new IllegalStateException("Problem during getting mail disclamer logo in folder: " + DISCLAMER_FOLDER  + "/" + LOGO_FOLDER  + ". Is the datadictionary customization folder up to date?", ex);
		}
	...
	private String computeName(final String originalName, final int tries, final Locale locale)
	{
		...
		return fileName + "_" + locale.getLanguage() + ((tries == 0) ? "" : "(" + tries + ")" )+ extension;
	}
